...
python -m PyInstaller --noconfirm --clean --onefile --windowed --name "HR Enterprise Network Server" --icon "%CD%\HR_Enterprise.ico" --hidden-import win32clipboard --hidden-import win32con --hidden-import qrcode --hidden-import zk --collect-submodules zk --collect-all qrcode --collect-all reportlab --add-data "%CD%\fonts;fonts" --add-data "%CD%\assets;assets" --add-data "%CD%\postgresql;postgresql" --distpath "dist\network" --workpath "build\network" --specpath "build\network" HR_NETWORK_SERVER.py
if errorlevel 1 exit /b 1
