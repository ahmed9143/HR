...
Every fresh install starts with:
...
The first login is forced onto the change-password screen; nothing else is
reachable until the password is changed. The default applies only when the
users table is empty, so upgrading or restoring an existing installation never
resets anyone's password. The login page shows the default only while it is
still unchanged.

Set `HR_BOOTSTRAP_PASSWORD` (8+ characters) before the first start to use a
different first-run password.

**Why this is fixed rather than random:** the Windows build runs without a
console window. A random password was printed to a console that did not exist
and written into the hidden `C:\ProgramData\HR Enterprise\Data` folder, so a
new machine had a password nobody could see.
