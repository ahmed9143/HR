...
<form method="post" action="/zkteco/devices/delete" style="display:inline" onsubmit="return confirm('حذف الجهاز نهائيًا؟ سجلات الحضور القديمة تبقى محفوظة.');">{csrf_field(u)}<input type="hidden" name="id" value="{r['id']}"><button class="btn bad">🗑 حذف</button></form>
        ...
        # SECURITY: never echo the stored credential back into the form. The
        # value used to be rendered in full, so it sat in the page source, the
        # browser cache and any saved copy of the page. The field is now always
        # blank; leaving it blank on save keeps the existing password.
        _has_pass = bool(edit_row and (edit_row['comm_password'] or ''))
        f_pass = ''
...
<div class="field"><label>كلمة مرور الاتصال (اختياري)</label><input type="password" name="comm_password" value="" autocomplete="new-password" placeholder="{'•••••••• (محفوظة — اتركها فارغة للإبقاء عليها)' if _has_pass else 'بدون كلمة مرور'}"></div>
...
<tbody>{trs or '<tr><td colspan="8">لا توجد أجهزة مضافة بعد. أضف أول جهاز من النموذج بالأعلى.</td></tr>'}</tbody></table></div>'''
    ...
    def zk_device_save(self, u, f):
        ...
        pw_input = self.fval(f, 'comm_password').strip()
        # Blank means "unchanged": re-read and keep whatever is already stored,
        # because the form can no longer show it.
        if pw_input:
            pw = g['encrypt_secret'](pw_input)
        elif did:
            _c = db()
            _row = _c.execute('SELECT comm_password FROM zk_devices WHERE id=?', (did,)).fetchone()
            _c.close()
            pw = (_row['comm_password'] if _row else '') or ''
        else:
            pw = ''
        try:
