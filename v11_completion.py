...
def install_v11(g):
    ...
    def v11_excel(self,u):
        ...
        body=f'''<div class="top"><div class="title"><h1>📊 Excel Grid</h1><p>Range selection, drag, keyboard navigation, clipboard, fill, undo/redo and column resize.</p></div></div>
        ...
        <script>
        const HEADERS={hjson}; let data=[HEADERS.slice(),Array(HEADERS.length).fill('')],anchor=null,dragging=false,undo=[],redo=[];
        ...
        H.send(page('Excel Grid',body,u,'import'))
    ...
    def mapping(u):
        ...
        H.send(page('Smart Mapping',body,u,'import'))

    def mapping_save(u,f):
        if not can(u,'employees.edit'): return
        try: mp=json.loads(f.get('mapping','{}'))
        except Exception: return H.send(page('Mapping','<div class="card"><div class="alert">Invalid mapping JSON.</div></div>',u),400)
        ...
        c.execute('INSERT INTO mapping_templates_v11(name,user_name,company_key,signature,mapping_json,confidence,version,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(name,user_name,company_key) DO UPDATE SET signature=excluded.signature,mapping_json=excluded.mapping_json,confidence=excluded.confidence,version=excluded.version,updated_at=excluded.updated_at',(name,u['username'],company,sig,json.dumps(mp,ensure_ascii=False),conf,ver,now(),now())); c.commit(); c.close(); audit(u['username'],u['role'],'Save import mapping','Imports',name,f'confidence={conf}'); H.redirect('/v11/mapping')
    ...
    def intelligence(u):
        ...
        H.send(page('Intelligence',body,u,'notifications'))

    def notification_action(u,f):
        ...
        c.execute('INSERT INTO notification_actions(notification_id,action,actor,created_at) VALUES(?,?,?,?)',(nid,action,u['username'],now())); c.commit(); c.close(); audit(u['username'],u['role'],'Notification action','Notifications',str(nid),action); H.redirect('/v11/intelligence')
    ...
    def devices(u):
        ...
        H.send(page('Devices',body,u,'devices'))

    def device_action(u,f):
        ...
        c.execute('INSERT INTO device_admin_events(device_id,action,actor,created_at,details) VALUES(?,?,?,?,?)',(did,action,u['username'],now(),name)); c.commit(); c.close(); audit(u['username'],u['role'],'Device action','Devices',did,action); H.redirect('/v11/devices')
    ...
    def workflow(u):
        ...
        H.send(page('Workflow',body,u,'requests'))

    def workflow_transition(u,f):
        ...
        if not r: c.close(); return H.redirect('/v11/workflow')
    ...
    def global_search(u):
        ...
        H.send(page('Search',body,u,'employees'))
