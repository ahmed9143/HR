...
with tempfile.TemporaryDirectory(prefix='hr_scope_get_') as td:
    ...
    try:
        ...
        for path in ['/contracts','/training']:
            ...
            if 'Blocked' in b or '>EB<' in b or '/employee/profile/EB' in b: failures.append(f'{path}: leaked DeptB employee')
        st,b=req(om,base,'/id-cards')
        if st!=200: failures.append(f'/id-cards: expected 200 got {st}')
        if 'EB</a>' in b or 'Blocked' in b or '/id-card/EB' in b: failures.append('/id-cards leaked DeptB employee')
        ...
        st,_=req(om,base,'/qr/generate-all','POST',{'_csrf':csrf})
        if st!=403: failures.append(f'/qr/generate-all non-admin: expected 403 got {st}')
        st,_=req(om,base,'/employee/photo/upload','POST',{'_csrf':csrf,'emp_code':'EA'})
        if st!=403: failures.append(f'/employee/photo/upload non-admin: expected 403 got {st}')
        for path in ['/hr-inbox','/assets']:
            st,b=req(om,base,path)
            if st!=200: failures.append(f'{path}: expected 200 got {st}')
        st,b=req(om,base,'/assets/save','POST',{'_csrf':csrf,'emp_code':'EB','asset_type':'Laptop','asset_no':'X'})
        if st!=403: failures.append(f'/assets/save out-of-scope: expected 403 got {st}')
        st,b=req(om,base,'/assets/save','POST',{'_csrf':csrf,'emp_code':'EA','asset_type':'Laptop','asset_no':'A-1'})
        if st!=403: failures.append(f'/assets/save manager without edit permission: expected 403 got {st}')
        ...
        if st not in (200,302,303): failures.append(f'admin qr generate: unexpected status {st}')
        st,b= req(oa,base,'/qr/generate-all?offset=0&limit=1','POST',{'_csrf':csrf_a})
        if st!=200 or ('جاري التنفيذ' not in b and 'عملية في الخلفية' not in b and 'جاري التحضير' not in b): failures.append(f'admin qr batch endpoint failed: {st}')
