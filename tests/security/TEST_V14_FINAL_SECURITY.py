...
ROOT=_HR_ROOT; PORT='8995'
def req(op,base,path,method='GET',data=None):
    try:
        r=op.open(urllib.request.Request(base+path,data=(urllib.parse.urlencode(data).encode() if data else None),method=method),timeout=5); return r.status,r.read().decode('utf-8','replace')
...
with __import__('tempfile').TemporaryDirectory(prefix='hr_v14_') as td:
    ...
    try:
        ...
        if failures:
            print('FAIL'); [print(' -',x) for x in failures]; print('See server logs for details.'); sys.exit(1)
