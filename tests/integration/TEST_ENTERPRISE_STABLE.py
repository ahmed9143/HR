...
with tempfile.TemporaryDirectory(prefix='hr_enterprise_') as td:
    ...
    try:
        ...
        # QR generation is intentionally asynchronous; wait on the job API instead of
        # assuming the POST itself finished the work.
        for _ in range(100):
            try:
                if op.open('http://127.0.0.1:8971/qr/image/Q100').status==200: break
            except Exception: pass
            time.sleep(.05)
        else: raise AssertionError('QR background job did not finish')
        ...
        try:
            ...
            # pyzbar needs the native libzbar library. The QR image is still
            # produced and its DB row verified above; only the visual decode
            # round-trip is skipped.
            print('SKIP: pyzbar/libzbar not installed - QR decode round-trip not verified')
            raise SystemExit(0)
        import io
        ...
        post('/qr/regenerate',{'_csrf':csrf,'emp_code':'Q100'});
        decoded2=None
        for _ in range(100):
            try:
                png2=op.open('http://127.0.0.1:8971/qr/image/Q100').read();
                cand=decode(Image.open(io.BytesIO(png2)));
                if cand:
                    candidate_token=cand[0].data.decode().rsplit('/qr/verify/',1)[1]
                    if candidate_token!=token:
                        decoded2=cand; break
            except Exception: pass
            time.sleep(.05)
        else: raise AssertionError('QR regenerate background job did not finish')
        token2=decoded2[0].data.decode().rsplit('/qr/verify/',1)[1]; assert token2!=token
