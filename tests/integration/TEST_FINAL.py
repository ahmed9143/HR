...
# The six BUILD_*.bat scripts were consolidated into BUILD_WINDOWS.bat, which
# builds from HR_Enterprise.spec and HR_Enterprise.iss exactly like CI does.
required=['START_HR.bat','START_NETWORK_SERVER.bat','START_CLIENT.bat','BUILD_WINDOWS.bat',
          'HR_Enterprise.spec','HR_Enterprise.iss','.github/workflows/windows-build.yml',
...
with tempfile.TemporaryDirectory(prefix='hr75test_') as td:
    ...
    try:
        ...
        _expected = (_HR_ROOT/'VERSION.txt').read_text(encoding='utf-8').strip().split()[0]
        assert health and health['ok'] and health['version'].startswith(_expected), (health, _expected)
