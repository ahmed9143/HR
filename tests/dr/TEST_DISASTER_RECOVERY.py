...
def remove_with_retry(path, tries=25, delay=0.2):
    """os.remove() that tolerates a brief Windows file-lock lag.

    app.stop() waits for the server process to exit, but on Windows the OS
    can take a moment longer to release its handle on a WAL-mode SQLite
    file after the process is gone (antivirus scanning, delayed handle
    teardown, etc). Retrying briefly avoids a spurious WinError 32 here
    without masking a real "still running" bug -- if the file is still
    locked after ~5s that's a genuine problem and the last error is raised.
    """
