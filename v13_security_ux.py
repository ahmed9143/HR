import os, time, io, json, csv, types
...
def install(g):
    ...
    def bulk_id_cards(self,u):
        ...
        # Bulk QR uses the same server-side form+redirect contract as
        # /employee/operations: POST /qr/generate-all -> 302 to the job page.
        # The previous inline onclick="genAllQr(this)" referenced a function that
        # no longer exists (its <script> body had been replaced by a comment),
        # so the button raised ReferenceError and sent no request at all.
        gen_btn=(f'<form method="post" action="/qr/generate-all" style="display:inline">'
                 f'{csrf_field(u)}'
                 f'<button class="btn ok" type="submit">⚡ توليد QR للموظفين داخل النطاق ({missing} بدون QR)</button>'
                 f'</form>') if missing and is_admin(u) else ''
        qr_version=int(time.time())
        links=''.join(f'<div class="bulk-card"><div class="bulk-head"><a href="/employee/profile/{quote(code)}">{esc(code)}</a><a class="btn gray" href="/id-card/{quote(code)}">فتح البطاقة</a></div><iframe loading="lazy" src="/id-card/{quote(code)}?embed=1&v={qr_version}" title="ID Card {esc(code)}"></iframe></div>' for code in codes)
        body=f'''<div class="top no-print"><div class="title"><h1>🪪 بطاقات الموظفين</h1><p>{len(codes)} بطاقة · البطاقات تُحمّل بشكل تدريجي بدل تجميد الصفحة.</p></div><div class="actions">{gen_btn}<button class="btn" onclick="window.print()">🖨 طباعة</button></div></div><div class="bulk-grid">{links or '<div class="card">لا توجد بطاقات ضمن نطاقك.</div>'}</div><style>.bulk-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(430px,1fr));gap:16px}}.bulk-card{{background:#fff;border:1px solid #e4e7ec;border-radius:18px;overflow:hidden;box-shadow:0 8px 25px rgba(16,24,40,.06)}}.bulk-head{{display:flex;justify-content:space-between;align-items:center;padding:12px 14px;background:#f8fafc}}.bulk-card iframe{{width:100%;aspect-ratio:860/540;height:auto;border:0;display:block}}@media print{{.bulk-grid{{display:block}}.bulk-card{{break-inside:avoid;margin-bottom:18px}}}}</style>'''
    ...
    def post2(self):
        p=urlparse(self.path).path
        # AUTHORIZATION OWNERSHIP: /qr/generate-all belongs to stable_final,
        # which loads after this module and therefore always wins. The guard
        # that lived here (is_admin) never executed, while stable_final
        # enforced a different rule. Two contradictory checks on one route is
        # how the QR token defect survived two rounds of "fixed", so the dead
        # branch is deleted rather than left to reappear on an import-order
        # change. Owner: stable_final, permission qr.bulk_generate.
