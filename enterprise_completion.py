import os, io, json, base64, secrets, hashlib, mimetypes, socket, threading, time, uuid
...
(function(){
  ...
  camBtn.addEventListener('click',function(){ stream ? stopCam() : startCam(); });
    ...
    def id_cards_bulk(self,u):
        ...
        <script>function genAllQr(btn){{btn.disabled=true;btn.textContent="جاري البدء…";var fd=new FormData();fd.append('_csrf','{esc(u.get("csrf",""))}');fetch('/qr/generate-all',{{method:'POST',body:fd}}).then(r=>r.json()).then(d=>{{if(!d.ok)throw new Error(d.error||'خطأ');location.href='/bulk/jobs/progress?id='+encodeURIComponent(d.job.id);}}).catch(e=>{{alert('تعذر بدء التوليد: '+e.message);btn.disabled=false;btn.textContent="⚡ توليد QR";}});}}</script>'''
