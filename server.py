...
def _bootstrap_admin_password():
    """Return a first-run admin password WITHOUT embedding a known credential.

    Resolution order:
      1. HR_BOOTSTRAP_PASSWORD environment variable (min 12 chars).
      2. An existing data/INITIAL_ADMIN_PASSWORD.txt from a previous first run.
      3. A newly generated cryptographically random password, written to that
         file with 0600 permissions and printed once to the console.

    Existing installations are never reset: the caller only invokes this when
    the users table is empty. The account is always created with
    must_change_password=1, and consume_bootstrap_credential() deletes the
    file as soon as the administrator rotates the password.
    """
    supplied = os.environ.get('HR_BOOTSTRAP_PASSWORD', '').strip()
    if supplied:
        if len(supplied) < 12:
            raise RuntimeError('HR_BOOTSTRAP_PASSWORD must be at least 12 characters.')
        return supplied
    ...
    if os.path.exists(path):
        try:
            ...
            print('[WARNING] Could not read existing bootstrap password file:', repr(e))
    ...
    try:
        ...
        print('[WARNING] Could not persist initial admin password:', repr(e))
    print('=' * 72)
    print('[HR] FIRST RUN: initial admin password generated.')
    print('[HR] Username: admin')
    print('[HR] Password file:', path)
    print('[HR] Log in, change the password immediately; the file is then deleted.')
    print('=' * 72)
    return value
...
class H(BaseHTTPRequestHandler):
    ...
    def handle_one_request(self):
        ...
        try:
            ...
            # Drain the still-unread body before closing. The size check
            # above rejects before a single byte of the body is read, so an
            # oversized POST is left sitting entirely in the OS receive
            # buffer. On platforms with a smaller default TCP receive
            # window (observed on Windows) the client's send() call blocks
            # waiting for window space that nobody will ever open, since we
            # never call rfile.read() again -- it deadlocks until the
            # client's own socket timeout fires. Reading (and discarding)
            # up to the declared Content-Length here releases that write so
            # the client gets its 413 response instead of a hang.
            try:
                remaining = min(getattr(self, '_oversized_content_length', 0), 4*1024*1024)
                while remaining > 0:
    ...
    def _raw(self):
        ...
        if not hasattr(self,'_raw_body'):
            ...
            if n>limit:
                ...
                self._oversized_content_length=n
