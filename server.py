...
# BUG FIX (real, reproduced): the project vendors a full offline copy of the
# `qrcode` library at vendor/qrcode (for machines with no internet/pip access —
# exactly the offline-LAN use case this app targets). But enterprise_completion.py
# does `import qrcode` BEFORE production_ops.py ever adds vendor/ to sys.path
# (that insertion happened later, at server.py's bottom-of-file install order),
# so on any machine that doesn't happen to have `qrcode` pip-installed globally,
# `import qrcode` silently failed -> qrcode=None -> every QR feature (issue,
# regenerate, ID card QR, QR scanner) was disabled with no visible error.
# Inserting the vendor path here, before any other module in this file is
# imported, guarantees the vendored copy is always found first.
_VENDOR_DIR=os.path.join(os.path.dirname(os.path.abspath(__file__)),'vendor')
if os.path.isdir(_VENDOR_DIR) and _VENDOR_DIR not in sys.path: sys.path.insert(0,_VENDOR_DIR)
# PyInstaller --windowed builds (no console attached) set sys.stdout/sys.stderr to
# None. Every print() in this file — including the ones inside log_error(), which
# runs on every single request-handling exception — was crashing with
# "AttributeError: 'NoneType' object has no attribute 'write'" the moment it fired,
# instead of writing the error page and returning cleanly to the browser. That turned
# any ordinary error (a bad request, a locked row, anything) into a silently dead
# connection that the browser just waits on forever. This must run before anything
...
HR_DB_URL=os.environ.get('HR_DB_URL','').strip()
...
_RESOURCE_BASE=os.path.abspath(getattr(sys,'_MEIPASS',BASE))
DEFAULT_LOGO=os.path.join(_RESOURCE_BASE,'assets','default_logo.png')
...
SESS={}
LOGIN_ATTEMPTS={}   # retained only for backward compatibility with older layers

# --------------------------------------------------------------------------
# HTTP hardening limits. Every one of these was previously unbounded.
# --------------------------------------------------------------------------
READ_CHUNK           = 1024*256
MAX_BODY_DEFAULT     = int(os.environ.get('HR_MAX_BODY',       str(2*1024*1024)))    # 2 MB for ordinary forms
MAX_BODY_UPLOAD      = int(os.environ.get('HR_MAX_UPLOAD',     str(64*1024*1024)))   # 64 MB for document/photo uploads
MAX_BODY_IMPORT      = int(os.environ.get('HR_MAX_IMPORT',     str(256*1024*1024)))  # 256 MB for bulk folder/Excel import
MAX_HTTP_WORKERS     = int(os.environ.get('HR_MAX_WORKERS',    '48'))
SOCKET_TIMEOUT_SECS  = int(os.environ.get('HR_SOCKET_TIMEOUT', '120'))
_UPLOAD_PATH_HINTS   = ('/upload','/document','/photo','/attach','/logo','/branding','/restore','/id-card')
_IMPORT_PATH_HINTS   = ('/import','/bulk','/folders','/accdb','/migrate')

class RequestTooLarge(Exception):
    """Raised before a body is read when Content-Length exceeds the limit."""

def max_body_for(path):
    p=(path or '').lower()
    if any(h in p for h in _IMPORT_PATH_HINTS): return MAX_BODY_IMPORT
    if any(h in p for h in _UPLOAD_PATH_HINTS): return MAX_BODY_UPLOAD
    return MAX_BODY_DEFAULT

HTTP_SEMAPHORE=threading.BoundedSemaphore(MAX_HTTP_WORKERS)

# --------------------------------------------------------------------------
# METRICS
#
# The recurring "the system is loading forever" reports were impossible to
# diagnose because nothing recorded how long requests took or how often SQLite
# blocked. These counters are cheap, in-process, and exposed on /health/deep.
# --------------------------------------------------------------------------
METRICS={'requests':0,'errors':0,'rejected_503':0,'rejected_413':0,
         'db_lock_waits':0,'db_lock_seconds':0.0,'slow_requests':0,
         'latency_samples':[],'started_at':time.time()}
METRICS_LOCK=threading.Lock()
SLOW_REQUEST_SECS=float(os.environ.get('HR_SLOW_REQUEST_SECS','3.0'))

def metric_inc(key,amount=1):
    try:
        with METRICS_LOCK: METRICS[key]=METRICS.get(key,0)+amount
    except Exception: pass

def metric_latency(seconds):
    try:
        with METRICS_LOCK:
            samples=METRICS['latency_samples']; samples.append(seconds)
            if len(samples)>2000: del samples[:len(samples)-2000]
            if seconds>SLOW_REQUEST_SECS: METRICS['slow_requests']=METRICS.get('slow_requests',0)+1
    except Exception: pass

def metric_percentiles():
    with METRICS_LOCK: samples=sorted(METRICS['latency_samples'])
    if not samples: return {'p50':0,'p95':0,'p99':0,'n':0}
    def pct(q):
        idx=min(len(samples)-1,int(round(q*(len(samples)-1))))
        return round(samples[idx],4)
    return {'p50':pct(.50),'p95':pct(.95),'p99':pct(.99),'n':len(samples)}
BRIDGE_TOKENS={}
AUDIT_CTX=threading.local()
DB_INIT_LOCK=threading.RLock()
_CACHE_LOCK=threading.RLock()   # guards _ROLE_PERMS_CACHE and _EMP_SCOPE_CACHE
# Explicit invalidation covers roles_save(), but permissions can also change out
# of band: a direct database edit, a second process, or a restore from backup.
# Without an upper bound a revoked permission would stay granted for the life of
# the process. Short enough that an out-of-band revoke applies on its own; long
# enough that can() still avoids essentially every per-request DB connection.
_ROLE_PERMS_TTL=int(os.environ.get('HR_ROLE_PERMS_TTL','60'))
...
# STRUCTURED LOGGING
#
# Diagnosis previously depended on print('[ERROR]', ...) to a console nobody
# reads on a Windows service, with no level, no component, no rotation and no
# way to correlate a failure with the job or request that caused it.
#
# hr_log() emits one JSON object per line to data/logs/hr.log with rotation,
# and mirrors WARNING and above to the console. Secrets are redacted by key
# name so a careless call site cannot leak a password into the log.
# --------------------------------------------------------------------------
import logging, logging.handlers

_SECRET_KEYS = ('password', 'passwd', 'pwd', 'token', 'secret', 'key',
                'csrf', 'comm_password', 'authorization', 'cookie')
LOG_LEVEL = os.environ.get('HR_LOG_LEVEL', 'INFO').upper()
LOG_MAX_BYTES = int(os.environ.get('HR_LOG_MAX_BYTES', str(5 * 1024 * 1024)))
LOG_BACKUPS = int(os.environ.get('HR_LOG_BACKUPS', '5'))
_HR_LOGGER = None

def _redact(key, value):
    k = str(key).lower()
    if any(marker in k for marker in _SECRET_KEYS):
        return '***redacted***'
    text = str(value)
    ...
    if not logger.handlers:
        try:
            ...
            fh = logging.handlers.RotatingFileHandler(
                os.path.join(LOGDIR, 'hr.log'), maxBytes=LOG_MAX_BYTES,
                backupCount=LOG_BACKUPS, encoding='utf-8')
            fh.setFormatter(logging.Formatter('%(message)s'))
            logger.addHandler(fh)
        except OSError as e:
            print('[HR] WARNING: file logging unavailable:', e)
...
def hr_log(component, event, level='ERROR', exc=None, **fields):
    """One structured line. `component` is the subsystem (db, qr, job, backup,
    zkteco, auth, http); `event` is a stable machine-readable identifier."""
    payload = {
    ...
    try:
        _hr_logger().log(getattr(logging, level, logging.ERROR),
                         json.dumps(payload, ensure_ascii=False))
    except Exception:
        # Logging must never be the thing that breaks a request.
        print('[HR-LOG-FALLBACK]', component, event, payload.get('error', ''))

def log_error(context,e,request_id='',username='',method='',path='',ip=''):
    tb=traceback.format_exc()
    hr_log('app',context,level='ERROR',exc=e,request_id=request_id,
           username=username,method=method,path=path,ip=ip)
    try:
...
CSS=''':root{--bg:#f3f6fb;--card:#fff;--ink:#101828;--muted:#667085;--brand:#175cd3;--brand2:#0b4aaf;--line:#e4e7ec;--ok:#12b76a;--warn:#f79009;--bad:#f04438;--nav:#0b1220;--shadow:0 12px 36px rgba(16,24,40,.08)}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top right,#eef4ff 0,#f3f6fb 38%,#f7f8fa 100%);font-family:"Segoe UI",Tahoma,Arial,sans-serif;color:var(--ink);direction:rtl}.app{display:flex;min-height:100vh}.side{width:268px;background:linear-gradient(180deg,var(--nav),#0e1728 55%,#101828);color:#fff;padding:18px 14px;position:fixed;right:0;top:0;bottom:0;overflow:auto;box-shadow:-12px 0 30px rgba(16,24,40,.12);z-index:20}.brand{padding:12px 12px 22px;border-bottom:1px solid rgba(255,255,255,.1);margin-bottom:14px}.brand b{font-size:21px}.brand small{display:block;color:#98a2b3;margin-top:6px}.nav a{display:flex;align-items:center;gap:10px;padding:11px 13px;border-radius:11px;color:#d0d5dd;margin:3px 0;font-size:14px}.nav a:hover,.nav a.active{background:linear-gradient(90deg,#18263d,#1b3157);color:#fff;box-shadow:inset 3px 0 0 #53b1fd}.nav>a:first-child{margin-bottom:10px;font-weight:700}.nav-group{margin:4px 0}.nav-group summary{cursor:pointer;list-style:none;padding:9px 10px;font-size:11px;font-weight:800;letter-spacing:.04em;color:#7b8aa3;text-transform:uppercase;display:flex;align-items:center;justify-content:space-between;border-radius:8px}.nav-group summary::-webkit-details-marker{display:none}.nav-group summary:after{content:"‹";display:inline-block;transform:rotate(-90deg);transition:transform .15s;color:#5c6b85}.nav-group[open]>summary:after{transform:rotate(0deg)}.nav-group summary:hover{background:rgba(255,255,255,.05);color:#c2cadb}.main{margin-right:268px;width:calc(100% - 268px);padding:28px 34px}.top{display:flex;align-items:center;justify-content:space-between;margin-bottom:22px;gap:18px}.title h1{margin:0;font-size:30px}.title p{margin:6px 0 0;color:var(--muted)}.actions{display:flex;gap:8px;flex-wrap:wrap}.btn{border:0;border-radius:10px;padding:10px 15px;background:linear-gradient(135deg,var(--brand),#2e90fa);color:#fff;cursor:pointer;font-weight:700;box-shadow:0 5px 14px rgba(23,92,211,.18)}.btn.gray{background:#fff;color:#344054;border:1px solid var(--line);box-shadow:none}.btn.ok{background:var(--ok)}.btn.bad{background:var(--bad)}.btn.warn{background:var(--warn)}.grid{display:grid;gap:17px}.g4{grid-template-columns:repeat(4,1fr)}.g3{grid-template-columns:repeat(3,1fr)}.g2{grid-template-columns:repeat(2,1fr)}.card{background:rgba(255,255,255,.96);border:1px solid var(--line);border-radius:17px;box-shadow:var(--shadow);padding:19px}.metric{position:relative;overflow:hidden}.metric:after{content:"";position:absolute;left:-20px;bottom:-35px;width:110px;height:110px;border-radius:50%;background:#eff8ff}.metric .label{color:var(--muted);font-size:13px;position:relative;z-index:1}.metric .value{font-size:34px;font-weight:800;margin-top:8px;position:relative;z-index:1}.metric .sub{font-size:12px;color:var(--muted);margin-top:6px;position:relative;z-index:1}.table-wrap{overflow:auto}.table{width:100%;border-collapse:separate;border-spacing:0;font-size:13px}.table th{background:#f8fafc;color:#475467;font-weight:750}.table th,.table td{padding:12px 10px;border-bottom:1px solid var(--line);text-align:right;white-space:nowrap}.table tr:hover td{background:#f8fbff}.badge{display:inline-block;border-radius:999px;padding:5px 10px;font-size:12px;font-weight:700}.b-ok{background:#ecfdf3;color:#027a48}.b-warn{background:#fffaeb;color:#b54708}.b-bad{background:#fef3f2;color:#b42318}.b-blue{background:#eff8ff;color:#175cd3}.b-gray{background:#f2f4f7;color:#475467}.timeline{display:flex;flex-direction:column;gap:10px;max-height:420px;overflow:auto}.tl-item{padding:10px 12px;border-radius:10px;background:#f8fafc;border:1px solid var(--line);font-size:13px}.tl-date{display:inline-block;min-width:96px;color:var(--muted);font-weight:700}.gsearch{position:relative}.gsearch input{width:230px;border-radius:10px;border:1px solid #2a3a55;background:#0e1728;color:#fff;padding:9px 12px;font:inherit}.gsearch input::placeholder{color:#7b8aa3}.gsearch kbd{position:absolute;left:8px;top:50%;transform:translateY(-50%);font-size:10px;color:#7b8aa3;border:1px solid #2a3a55;border-radius:5px;padding:1px 5px}.form{display:grid;grid-template-columns:repeat(2,1fr);gap:14px}.field label{display:block;font-size:12px;font-weight:700;margin-bottom:7px;color:#344054}.field input,.field select,.field textarea{width:100%;border:1px solid #d0d5dd;border-radius:10px;padding:11px;background:#fff;font:inherit;outline:none}.field input:focus,.field select:focus,.field textarea:focus{border-color:#84adff;box-shadow:0 0 0 4px #eff4ff}.field textarea{min-height:100px}.full{grid-column:1/-1}.toolbar{display:flex;gap:9px;flex-wrap:wrap;margin-bottom:14px}.toolbar input,.toolbar select{border:1px solid #d0d5dd;border-radius:10px;padding:10px;font:inherit;background:#fff}.chart{height:260px;display:flex;align-items:end;gap:14px;padding:24px 10px 24px}.bar{flex:1;background:linear-gradient(180deg,#53b1fd,#175cd3);border-radius:10px 10px 4px 4px;min-width:28px;position:relative;box-shadow:0 8px 18px rgba(23,92,211,.16)}.bar span{position:absolute;bottom:-28px;right:0;left:0;text-align:center;font-size:11px;color:var(--muted);overflow:hidden;text-overflow:ellipsis}.bar b{position:absolute;top:-23px;right:0;left:0;text-align:center;font-size:11px}.alert{padding:13px 15px;border-radius:11px;background:#fff8e7;border:1px solid #fedf89;color:#7a4e00}.login{min-height:100vh;display:grid;place-items:center;background:radial-gradient(circle at 20% 10%,#2e90fa 0,#175cd3 22%,#101828 70%)}.login-card{width:450px;background:rgba(255,255,255,.98);border-radius:22px;padding:38px;box-shadow:0 30px 90px rgba(0,0,0,.3)}.login-card input{width:100%;padding:13px;border:1px solid #d0d5dd;border-radius:10px;margin:7px 0;font:inherit}.login-card button{width:100%;margin-top:14px}.footer{color:#98a2b3;font-size:11px;text-align:center;margin-top:30px}@media(max-width:1100px){.g4{grid-template-columns:repeat(2,1fr)}}@media(max-width:900px){.side{position:static;width:100%;height:auto}.app{display:block}.main{margin:0;width:100%;padding:18px}.g4,.g3,.g2{grid-template-columns:1fr}.form{grid-template-columns:1fr}}@media print{.side,.actions,.toolbar,.no-print,.ph-actions,.ph-photo-edit,.qr-actions{display:none!important}.main{margin:0;width:100%;padding:0}.card{box-shadow:none}}'''
CSS += '.top{position:sticky;top:0;z-index:10;background:rgba(243,246,251,.94);backdrop-filter:blur(8px);padding:10px 0}.nav a{min-height:42px}.btn:focus-visible,input:focus-visible,select:focus-visible,textarea:focus-visible{outline:3px solid #b2ddff;outline-offset:2px}.mobile-menu{display:none}.login{position:relative;overflow:hidden}.login:before,.login:after{content:"";position:absolute;width:420px;height:420px;border-radius:50%;background:rgba(255,255,255,.07);filter:blur(2px);animation:hrFloat 8s ease-in-out infinite}.login:before{top:-160px;right:-100px}.login:after{bottom:-220px;left:-120px;animation-delay:-3s}.login-card{position:relative;z-index:2;animation:hrLoginIn .55s cubic-bezier(.2,.8,.2,1)}@keyframes hrLoginIn{from{opacity:0;transform:translateY(22px) scale(.98)}to{opacity:1;transform:none}}@keyframes hrFloat{0%,100%{transform:translate3d(0,0,0) scale(1)}50%{transform:translate3d(0,18px,0) scale(1.06)}}@media(prefers-reduced-motion:reduce){.login:before,.login:after,.login-card{animation:none}.mobile-menu{display:none}}@media(max-width:900px){.side{position:fixed;right:-290px;width:280px;height:100vh;transition:right .2s}.side.open{right:0}.main{margin:0}.mobile-menu{display:inline-flex}.top{position:static}}';
# Was inlined as <style>{CSS}</style> on every single response (~7KB re-sent and
# re-parsed by the browser on every click, with zero caching). Served as a real
# static file with a long-lived Cache-Control instead — the browser downloads it
# once per version and reuses it, so page navigation feels lighter immediately.
CSS += '.wizard{max-width:900px;margin:0 auto}.wz-track{display:flex;align-items:center;justify-content:space-between;margin-bottom:26px;padding:0 6px}.wz-step{display:flex;flex-direction:column;align-items:center;gap:8px;position:relative;flex:1;cursor:pointer}.wz-step .dot{width:38px;height:38px;border-radius:50%;background:#fff;border:2px solid var(--line);color:var(--muted);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:15px;transition:.2s}.wz-step .lbl{font-size:12.5px;color:var(--muted);font-weight:700;white-space:nowrap}.wz-step .sub{font-size:10.5px;color:#98a2b3}.wz-step:before{content:"";position:absolute;top:19px;right:calc(50% + 22px);left:calc(-50% + 22px);height:2px;background:var(--line);z-index:0}.wz-step:first-child:before{display:none}.wz-step.done .dot{background:var(--ok);border-color:var(--ok);color:#fff}.wz-step.done:before{background:var(--ok)}.wz-step.active .dot{background:linear-gradient(135deg,var(--brand),#2e90fa);border-color:var(--brand);color:#fff;box-shadow:0 0 0 6px #eff4ff}.wz-step.active .lbl{color:var(--ink)}.wz-panel{display:none;animation:wzIn .25s ease}.wz-panel.active{display:grid;grid-template-columns:repeat(2,1fr);gap:14px}.wz-panel .full{grid-column:1/-1}@keyframes wzIn{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:none}}.wz-actions{display:flex;justify-content:space-between;margin-top:22px;grid-column:1/-1;padding-top:16px;border-top:1px solid var(--line)}.wz-actions .spacer{flex:1}.wz-hint{grid-column:1/-1;background:#eff8ff;border:1px solid #d1e9ff;color:#175cd3;border-radius:10px;padding:10px 13px;font-size:12.5px;font-weight:600}.field.invalid input,.field.invalid select{border-color:var(--bad);box-shadow:0 0 0 4px #fef3f2}@media(max-width:700px){.wz-step .lbl,.wz-step .sub{display:none}.wz-panel,.wz-panel.active{grid-template-columns:1fr}}'
# Flexible sidebar: expand/collapse rail on desktop, slide-in drawer on mobile,
# plus the Employee Profile hero header (photo + QR together, above everything).
CSS += ('.side{display:flex;flex-direction:column;transition:width .2s ease}'
'.brand{display:flex;align-items:center;gap:10px;flex:0 0 auto}'
'.brand-info{overflow:hidden;white-space:nowrap;min-width:0}'
'.brand-info b{display:block;font-size:17px;line-height:1.35;white-space:normal;overflow-wrap:anywhere;word-break:normal}'
# LAYOUT FIX: the brand block used overflow:hidden + text-overflow:ellipsis on a
# single line, so any hospital name longer than the 268px sidebar was cut to
# "المستشفى العسكـ…". Arabic names are long; the name now wraps instead.
'.app.collapsed .brand-info b{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}'
# LAYOUT FIX: the quick-search input was a fixed 230px inside a 268px sidebar
# with 14px padding, so it overflowed and the Ctrl K badge sat on top of the
# placeholder text. It is now fluid, with room reserved for the badge.
'.gsearch form{position:relative;display:block}'
'.gsearch input{width:100%!important;padding-inline-start:12px;padding-inline-end:62px}'
'.gsearch kbd{left:10px;right:auto;pointer-events:none;white-space:nowrap}'
'.side-toggle{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.16);color:#fff;border-radius:9px;width:34px;height:34px;flex:0 0 34px;display:grid;place-items:center;cursor:pointer;font-size:15px;padding:0}'
'.side-toggle:hover{background:rgba(255,255,255,.18)}'
'.gsearch{flex:0 0 auto;overflow:hidden}'
'.nav{flex:1;min-height:0;overflow-y:auto;overflow-x:hidden}'
'.nav a{white-space:nowrap;overflow:hidden}'
'.nav a .ic,.nav-group summary .ic{width:20px;flex:0 0 20px;text-align:center;font-size:15px}'
'.nav a .lbl,.nav-group summary .grp-lbl{overflow:hidden;text-overflow:ellipsis;min-width:0}'
'.nav-group summary{gap:10px}'
'.footer{flex:0 0 auto;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}'
'.side-overlay{display:none}'
'.mobile-menu{display:none}'
'.app.collapsed .side{width:76px}'
'.app.collapsed .main{margin-right:76px;width:calc(100% - 76px)}'
'.app.collapsed .brand{justify-content:center;padding-left:0;padding-right:0}'
'.app.collapsed .brand-info,.app.collapsed .gsearch,.app.collapsed .nav a .lbl,.app.collapsed .nav-group summary .grp-lbl,.app.collapsed .nav-group summary:after,.app.collapsed .footer{display:none}'
'.app.collapsed .nav a,.app.collapsed .nav-group summary{justify-content:center}'
'.app.collapsed .side-toggle{transform:rotate(180deg)}'
'@media(max-width:900px){'
'.mobile-menu{display:grid;place-items:center;position:fixed;top:14px;right:14px;z-index:40;width:42px;height:42px;border-radius:11px;border:0;background:var(--nav);color:#fff;font-size:18px;cursor:pointer;box-shadow:0 6px 18px rgba(16,24,40,.28)}'
'.side{position:fixed;right:-300px;top:0;bottom:0;width:280px;height:100vh;transition:right .22s ease;z-index:45}'
'.side.open{right:0}'
'.side-toggle{display:none}'
'.app.collapsed .side{width:280px}'
'.app.collapsed .main{margin-right:0;width:100%}'
'.side-overlay.show{display:block;position:fixed;inset:0;background:rgba(16,24,40,.5);z-index:42}'
'.main{margin-right:0!important;width:100%!important;padding:64px 14px 18px!important}'
'}'
# Employee Profile hero: photo + name/status/actions + QR identity, always first.
'.profile-hero{display:flex;flex-wrap:wrap;gap:22px;align-items:flex-start;margin-bottom:16px}'
'.profile-hero .ph-photo-wrap{position:relative;width:150px;height:150px;flex:0 0 150px}'
'.profile-hero .ph-photo{width:150px;height:150px;border-radius:22px;object-fit:cover;border:1px solid var(--line);background:#f2f4f7;display:block}'
'.profile-hero .ph-photo-empty{width:150px;height:150px;border-radius:22px;background:#eef2f6;display:grid;place-items:center;font-size:56px;color:#98a2b3}'
'.profile-hero .ph-photo-edit{position:absolute;inset:0;border-radius:22px;display:grid;place-items:center;background:rgba(16,24,40,0);color:#fff;font-size:24px;cursor:pointer;transition:background .15s;gap:6px}'
'.profile-hero .ph-photo-edit:hover{background:rgba(16,24,40,.55)}'
'.profile-hero .ph-info{flex:1;min-width:240px}'
'.profile-hero .ph-info h1{margin:0;font-size:27px}'
'.profile-hero .ph-meta{color:var(--muted);margin-top:6px;font-size:14px}'
'.profile-hero .ph-status{margin-top:10px}'
'.profile-hero .ph-actions{margin-top:14px;display:flex;gap:8px;flex-wrap:wrap}'
'.profile-hero .ph-qr{flex:0 0 auto;width:190px;border:1px solid var(--line);border-radius:16px;padding:14px;background:#f8fafc;text-align:center}'
'.profile-hero .ph-qr img{width:130px;height:130px;border-radius:10px;background:#fff;border:1px solid var(--line);padding:5px}'
'.profile-hero .ph-qr .qr-status{margin-bottom:8px}'
'.profile-hero .ph-qr .qr-actions{display:flex;flex-direction:column;gap:6px;margin-top:10px}'
'@media(max-width:700px){'
'.profile-hero{display:grid;grid-template-columns:1fr 1fr;grid-template-areas:"photo qr" "info info";gap:14px 10px;text-align:center;align-items:start}'
'.profile-hero .ph-photo-wrap{grid-area:photo;justify-self:center}'
'.profile-hero .ph-qr{grid-area:qr;justify-self:center;width:100%;max-width:170px}'
'.profile-hero .ph-info{grid-area:info}'
'.profile-hero .ph-actions{justify-content:center}'
'}'
'@media(max-width:420px){'
'.profile-hero{grid-template-columns:1fr 1fr;gap:10px 8px}'
'.profile-hero .ph-photo-wrap,.profile-hero .ph-photo,.profile-hero .ph-photo-empty{width:100px;height:100px}'
'.profile-hero .ph-qr{max-width:130px;padding:8px}'
'.profile-hero .ph-qr img{width:90px;height:90px}'
'.profile-hero .ph-qr .qr-actions .btn{font-size:12px;padding:8px 10px}'
'}')
CSS_VERSION=hashlib.sha256(CSS.encode()).hexdigest()[:10]

# JS for the flexible sidebar (expand/collapse rail persisted in localStorage,
# icon+tooltip rendering when collapsed, and the mobile slide-in drawer).
# Plain (non f-string) so its { } braces don't need doubling; interpolated
# into the page() template as {SIDE_JS}.
SIDE_JS = '''
(function(){
  function splitIcon(text){
    var m=text.match(/^\\s*(\\p{Extended_Pictographic}(?:[\\uFE0F\\u200D]\\p{Extended_Pictographic}?)*)\\s*/u);
    return m ? m[1] : null;
  }
  function iconize(el,isSummary){
    ...
    if(nodes.length && nodes[0].nodeType===3){
      var t=nodes[0].textContent; var ic=splitIcon(t);
      if(ic){ icon=ic; nodes[0].textContent=t.slice(t.indexOf(ic)+ic.length).replace(/^\\s+/,''); }
    }
    if(!icon) icon = isSummary ? '\\ud83d\\udcc1' : '\\u2022';
    var icSpan=document.createElement('span'); icSpan.className='ic'; icSpan.textContent=icon;
    var lblSpan=document.createElement('span'); lblSpan.className = isSummary ? 'grp-lbl' : 'lbl';
    nodes.forEach(function(n){ lblSpan.appendChild(n); });
    el.innerHTML=''; el.appendChild(icSpan); el.appendChild(lblSpan);
    el.dataset.iconized='1';
  }
  function applyIcons(){
    document.querySelectorAll('.nav a').forEach(function(a){ iconize(a,false); });
    document.querySelectorAll('.nav-group summary').forEach(function(s){ iconize(s,true); });
  }
  var appEl=document.getElementById('appRoot');
  var side=document.getElementById('mainSide');
  var toggle=document.getElementById('sideToggle');
  var mobileToggle=document.getElementById('sideMobileToggle');
  var overlay=document.getElementById('sideOverlay');
  applyIcons();
  new MutationObserver(applyIcons).observe(document.querySelector('.nav'),{childList:true,subtree:true});
  if(appEl && localStorage.getItem('hr_side_collapsed')==='1') appEl.classList.add('collapsed');
  if(toggle) toggle.addEventListener('click', function(){
    appEl.classList.toggle('collapsed');
    localStorage.setItem('hr_side_collapsed', appEl.classList.contains('collapsed') ? '1' : '0');
  });
  function openMobile(){ side.classList.add('open'); overlay.classList.add('show'); }
  function closeMobile(){ side.classList.remove('open'); overlay.classList.remove('show'); }
  ...
  if(side) side.querySelectorAll('a').forEach(function(a){
    a.addEventListener('click', function(){ if(window.innerWidth<=900) closeMobile(); });
  });
})();
'''
...
#
# Replaces the previous in-memory LOGIN_ATTEMPTS dict, which reset on every
# restart, tracked only the IP, and re-armed itself with a fresh 5-attempt
# budget after each lockout (so an attacker got ~60 tries/hour indefinitely).
#
# Now: per-IP AND per-username counters persisted in SQLite, with progressive
# backoff, so a restart no longer clears an attacker's record.
# --------------------------------------------------------------------------
AUTH_MAX_FAILS      = int(os.environ.get('HR_AUTH_MAX_FAILS','5'))
AUTH_BASE_LOCK_SECS = int(os.environ.get('HR_AUTH_LOCK_SECS','300'))
AUTH_MAX_LOCK_SECS  = int(os.environ.get('HR_AUTH_MAX_LOCK_SECS','86400'))
AUTH_WINDOW_SECS    = int(os.environ.get('HR_AUTH_WINDOW_SECS','900'))
_DUMMY_HASH         = None

def _dummy_password_hash():
    """A real hash used to equalise timing when the username does not exist.

    Without this, checkpw() (scrypt, deliberately slow) only ran for existing
    usernames, so response latency leaked which accounts are real.
    """
    global _DUMMY_HASH
    if _DUMMY_HASH is None:
        _DUMMY_HASH = hashpw(secrets.token_urlsafe(24))
    return _DUMMY_HASH

def _auth_lock_state(scope, ident):
    """Return (locked, seconds_remaining) for one scope/identifier pair."""
    if not ident: return (False, 0)
    try:
        c = db(); r = c.execute('SELECT fails,locked_until,last_fail FROM auth_failures WHERE scope=? AND ident=?', (scope, str(ident)[:190])).fetchone(); c.close()
    except Exception as e:
        log_error('auth-lock-read', e); return (False, 0)
    if not r or not r['locked_until']: return (False, 0)
    try: until = datetime.fromisoformat(r['locked_until'])
    except Exception: return (False, 0)
    if datetime.now() >= until: return (False, 0)
    return (True, int((until - datetime.now()).total_seconds()))

def auth_check_locked(ip, username):
    """True/seconds if either the source IP or the target account is locked."""
    for scope, ident in (('ip', ip), ('user', (username or '').strip().lower())):
        locked, wait = _auth_lock_state(scope, ident)
        if locked: return (True, wait, scope)
    return (False, 0, '')

def auth_record_failure(ip, username):
    """Increment both counters; apply progressive backoff once the cap is hit."""
    ts = now()
    for scope, ident in (('ip', ip), ('user', (username or '').strip().lower())):
        ...
        try:
            c = db()
            r = c.execute('SELECT id,fails,first_fail,lock_count FROM auth_failures WHERE scope=? AND ident=?', (scope, ident)).fetchone()
            if not r:
                c.execute('INSERT INTO auth_failures(scope,ident,fails,first_fail,last_fail,lock_count) VALUES(?,?,?,?,?,0)', (scope, ident, 1, ts, ts))
            else:
                fails = int(r['fails'] or 0) + 1
                lock_count = int(r['lock_count'] or 0)
                # Reset the counter if the previous failure window has expired.
                try:
                    if r['first_fail'] and (datetime.now() - datetime.fromisoformat(r['first_fail'])).total_seconds() > AUTH_WINDOW_SECS and fails <= AUTH_MAX_FAILS:
                        fails = 1
                except Exception: pass
                locked_until = None
                if fails >= AUTH_MAX_FAILS:
                    lock_count += 1
                    # Progressive: 5m, 10m, 20m, 40m ... capped at 24h.
                    secs = min(AUTH_BASE_LOCK_SECS * (2 ** (lock_count - 1)), AUTH_MAX_LOCK_SECS)
                    locked_until = (datetime.now() + timedelta(seconds=secs)).isoformat(timespec='seconds')
                    fails = 0
                c.execute('UPDATE auth_failures SET fails=?,last_fail=?,locked_until=?,lock_count=? WHERE id=?', (fails, ts, locked_until, lock_count, r['id']))
            c.commit(); c.close()
        except Exception as e:
            log_error('auth-record-failure', e)

def auth_clear_failures(ip, username):
    """Wipe both counters after a successful authentication."""
    for scope, ident in (('ip', ip), ('user', (username or '').strip().lower())):
        if not ident: continue
        try:
            c = db(); c.execute('DELETE FROM auth_failures WHERE scope=? AND ident=?', (scope, str(ident)[:190])); c.commit(); c.close()
        except Exception as e:
            log_error('auth-clear-failures', e)

def auth_unlock(scope, ident):
    """Administrative unlock."""
    try:
        c = db(); c.execute('DELETE FROM auth_failures WHERE scope=? AND ident=?', (scope, str(ident)[:190])); c.commit(); c.close(); return True
    except Exception as e:
        log_error('auth-unlock', e); return False

def auth_event(username, ip, event, details=''):
    """Append-only security log for every login outcome."""
    try:
        c = db(); c.execute('INSERT INTO security_login_events(user_name,ip,event,created_at,details) VALUES(?,?,?,?,?)', ((username or '')[:120], (ip or '')[:60], event, now(), (details or '')[:400])); c.commit(); c.close()
    except Exception as e:
        log_error('auth-event', e)
def now(): return datetime.now().isoformat(timespec='seconds')
def db():
    # Network mode can use PostgreSQL when HR_DB_URL is provided. Standalone remains
    # SQLite/WAL for zero-dependency local operation.
    if HR_DB_URL:
        from pg_compat import connect as pg_connect
        return pg_connect(HR_DB_URL)
    _t0=time.time()
    c=sqlite3.connect(DB,timeout=10,check_same_thread=False)
    _wait=time.time()-_t0
    if _wait>0.25:
        # Connecting took measurably long: SQLite was busy behind a writer.
        try:
            with METRICS_LOCK:
                METRICS['db_lock_waits']=METRICS.get('db_lock_waits',0)+1
                METRICS['db_lock_seconds']=round(METRICS.get('db_lock_seconds',0.0)+_wait,3)
        except Exception: pass
...
def _seed_default_branding(c):
    """On first run only: if no logo has been configured yet, install the
    hospital's bundled emblem as the default login/sidebar logo, and set a
    matching Arabic tagline. Any logo the admin later uploads via
    Settings > Branding simply overwrites this — this only fills the gap
    before the admin has uploaded anything."""
    try:
        row=c.execute("SELECT value FROM settings WHERE key='company_logo'").fetchone()
        if row and row['value']:
            return  # already configured (by us before, or by the admin)
        if os.path.exists(DEFAULT_LOGO):
            os.makedirs(BRAND,exist_ok=True)
            dest=os.path.join(BRAND,'logo_default.png')
            if not os.path.exists(dest):
                shutil.copyfile(DEFAULT_LOGO,dest)
            rel=os.path.relpath(dest,DATA)
            c.execute("INSERT INTO settings(key,value) VALUES('company_logo',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value",(rel,))
        c.execute("INSERT OR IGNORE INTO settings(key,value) VALUES('login_tagline',?)",('نظام الموارد البشرية — المستشفى العسكري بدمياط الجديدة',))
        c.commit(); _SETTINGS_CACHE.clear()
    except Exception as e:
        log_error('seed-default-branding',e)

# --------------------------------------------------------------------------
# FIRST-RUN ADMIN CREDENTIAL
#
# Fixed, documented default: admin / Admin@12345.
#
# Why not a random password: the Windows build runs with --windowed, so there
# is no console. A per-machine random password was printed to a console that
# does not exist and written into C:\ProgramData\HR Enterprise\Data — a hidden
# folder. Every fresh install therefore had a password nobody could see, which
# is exactly the "wrong password on every new device" failure.
#
# What keeps a known default acceptable:
#   * it is used ONLY when the users table is empty (a brand-new install);
#     existing installations are never reset,
#   * the account is created with must_change_password=1, so the very first
#     login is forced onto the change-password screen and nothing else works
#     until it is changed,
#   * the persistent per-IP + per-account lockout still applies,
#   * the login page warns while the default is still active.
#
# Deployments that want a different first-run password set
# HR_BOOTSTRAP_PASSWORD (minimum 8 characters) before the first start.
# --------------------------------------------------------------------------
DEFAULT_ADMIN_USERNAME = 'admin'
DEFAULT_ADMIN_PASSWORD = 'Admin@12345'

def _bootstrap_admin_password():
    supplied = os.environ.get('HR_BOOTSTRAP_PASSWORD', '').strip()
    if supplied:
        if len(supplied) < 8:
            raise RuntimeError('HR_BOOTSTRAP_PASSWORD must be at least 8 characters.')
        return supplied
    return DEFAULT_ADMIN_PASSWORD

def default_credentials_active():
    """True while the first-run admin has not yet changed the default password.

    Used by the login page to show a one-line setup hint. Once the password is
    rotated this returns False and nothing about the default is shown again.
    """
    try:
        c = db()
        row = c.execute('SELECT must_change_password FROM users WHERE username=?',
                        (DEFAULT_ADMIN_USERNAME,)).fetchone()
        total = c.execute('SELECT COUNT(*) FROM users').fetchone()[0]
        c.close()
        return bool(row and row[0] and total == 1)
    except sqlite3.Error as e:
        hr_log('auth', 'default_credential_check_failed', level='WARNING', exc=e)
        return False

def consume_bootstrap_credential():
    """Delete the first-run password file once the admin has rotated it."""
    path = os.path.join(DATA, 'INITIAL_ADMIN_PASSWORD.txt')
    try:
        if os.path.exists(path):
            os.remove(path)
            print('[HR] Bootstrap credential file consumed and deleted.')
    except Exception as e:
        print('[WARNING] Could not delete bootstrap credential file:', repr(e))
...
def _init_db_schema():
    ...
    CREATE TABLE IF NOT EXISTS auth_failures(id INTEGER PRIMARY KEY,scope TEXT NOT NULL,ident TEXT NOT NULL,fails INTEGER DEFAULT 0,first_fail TEXT,last_fail TEXT,locked_until TEXT,lock_count INTEGER DEFAULT 0);
    CREATE UNIQUE INDEX IF NOT EXISTS idx_auth_failures ON auth_failures(scope,ident);
    ...
    CREATE INDEX IF NOT EXISTS idx_login_events_time ON security_login_events(created_at);
    ...
    CREATE TABLE IF NOT EXISTS system_sessions(id INTEGER PRIMARY KEY,session_id TEXT UNIQUE,user_name TEXT,full_name TEXT,role TEXT,created_at TEXT,last_seen TEXT,ip TEXT,device TEXT,revoked INTEGER DEFAULT 0,csrf TEXT);
    ...
    CREATE TABLE IF NOT EXISTS supplies_items(id INTEGER PRIMARY KEY,name TEXT UNIQUE,unit TEXT DEFAULT 'قطعة',notes TEXT,active INTEGER DEFAULT 1,created_by TEXT,created_at TEXT);
    CREATE TABLE IF NOT EXISTS supplies_quotas(department TEXT PRIMARY KEY,monthly_quota REAL DEFAULT 0,updated_by TEXT,updated_at TEXT);
    CREATE TABLE IF NOT EXISTS supplies_issues(id INTEGER PRIMARY KEY,receipt_no TEXT UNIQUE,emp_code TEXT,emp_name TEXT,department TEXT,item_name TEXT,quantity REAL DEFAULT 1,paper_size TEXT DEFAULT '',notes TEXT,issued_by TEXT,issued_at TEXT);
    ...
    extra_cols={'manager_name':'TEXT','tags':'TEXT','employee_group':'TEXT','birth_date':'TEXT','address':'TEXT','qualification':'TEXT','iban':'TEXT','bank_name':'TEXT','bank_branch':'TEXT','unit':'TEXT','contract_date':'TEXT','contract_amount':'REAL DEFAULT 0','prev_status':'TEXT','archived_at':'TEXT','archived_by':'TEXT','shift_id':'INTEGER','zk_user_id':'TEXT'}
    for col,typ in extra_cols.items():
        if col not in existing: c.execute(f'ALTER TABLE employees ADD COLUMN {col} {typ}')
    # ZKTeco Phase 1: employees.zk_user_id is the single mapping to the employee's
    # ZKTeco identity (shared across any number of devices). NULL = not linked yet;
    # a real value must be unique to one employee. Added here (not in zkteco_core.py)
    # because this is the only point in the startup sequence guaranteed to run after
    # the employees table/column above exist — the zk_* plugin module below loads
    # before _init_db_schema() runs, same as every other feature-pack module.
    c.execute('CREATE UNIQUE INDEX IF NOT EXISTS idx_employees_zk_user_id ON employees(zk_user_id) WHERE zk_user_id IS NOT NULL')
    ...
    for _ix in [
        ...
        'CREATE INDEX IF NOT EXISTS idx_notifications_dedup ON notifications(user_name,title,created_at)',
        ...
        'CREATE INDEX IF NOT EXISTS idx_supplies_issues_dept_date ON supplies_issues(department,issued_at)',
        'CREATE INDEX IF NOT EXISTS idx_supplies_issues_emp ON supplies_issues(emp_code)'
    ]:
        try: c.execute(_ix)
        except sqlite3.Error as _e:
            hr_log('db','silent_db_failure',level='WARNING',exc=_e,where='server.py:694')
    defaults={'company_name':'المستشفى العسكري بدمياط الجديدة','work_start':'09:00','work_end':'17:00','grace_minutes':'15','currency':'EGP','max_late_minutes_month':'60','session_idle_minutes':'30','document_max_mb':'25','network_mode':'standalone','late_alert_threshold':'60','monthly_late_limit_minutes':'120','monthly_late_action':'none','monthly_deduction_amount':'0','required_doc_categories':'عقد,هوية,مؤهل','company_logo':'','eval_weight_attendance':'20','eval_weight_punctuality':'20','eval_weight_productivity':'25','eval_weight_behavior':'15','eval_weight_manager':'20','backup_auto_enabled':'1','backup_time':'23:00','backup_keep_days':'14','theme':'light','accent_color':'blue','backup_keep_weekly':'8','backup_keep_monthly':'12','server_name':'HR-SERVER','server_url':'','offline_cache_minutes':'15','daily_late_limit_minutes':'15','daily_late_action':'none','daily_exceeded_deduction_amount':'0','contract_alert_days':'30','credential_alert_days':'30','training_alert_days':'30','privacy_mode_default':'0','https_enabled':'0','backup_network_path':'','backup_verify_after_create':'1','server_offline_seconds':'120','late_risk_percent':'80','training_alert_days':'30','credential_alert_days':'30'}
    for k,v in defaults.items(): c.execute('INSERT OR IGNORE INTO settings(key,value) VALUES(?,?)',(k,v))
    _seed_default_branding(c)
    
    if not c.execute('SELECT 1 FROM users LIMIT 1').fetchone():
        c.execute('INSERT INTO users(username,password_hash,role,full_name,must_change_password,scope_type) VALUES(?,?,?,?,?,?)',('admin',hashpw(_bootstrap_admin_password()),'SuperAdmin','مدير النظام',1,'all'))
    ...
    # MIGRATION: grant qr.bulk_generate to the roles that could already perform
    # bulk QR generation before it became an explicit permission (the old check
    # was can(u,'employees.edit')). This preserves existing behaviour exactly;
    # revoke it from the Roles screen to restrict bulk QR to administrators.
    for existing in c.execute("SELECT DISTINCT role FROM role_permissions WHERE permission='employees.edit'").fetchall():
        c.execute('INSERT OR IGNORE INTO role_permissions(role,permission) VALUES(?,?)',(existing[0],'qr.bulk_generate'))
    c.commit(); c.close()
    # Do not auto-import the legacy seed workbook. Employees are loaded only from the user's Excel/Paste workflow.


def init():
    """Initialize/migrate SQLite safely even when an old HR process is still closing."""
    last=None
    for attempt in range(8):
        try:
            ...
            if 'locked' not in str(e).lower() and 'busy' not in str(e).lower():
                raise
            time.sleep(min(0.75 * (attempt + 1), 4.0))
    raise sqlite3.OperationalError(f'HR database is still busy after retries: {last}')
...
_SETTINGS_CACHE={}
def setting(k):
    # setting() is called ~3 times on every single page (company_name, company_logo,
    # notifications) plus more inside specific handlers — 51 call sites total. Each
    # call used to open/close a fresh SQLite connection (connect + 4 PRAGMA round
    # trips) just to read one row. settings only changes via the handful of write
    # sites below, which clear this cache, so caching in memory is safe and removes
    # another large chunk of per-request DB connections on top of the can() cache.
    if k in _SETTINGS_CACHE: return _SETTINGS_CACHE[k]
    c=db(); r=c.execute('SELECT value FROM settings WHERE key=?',(k,)).fetchone(); c.close()
    v=r['value'] if r else ''
    _SETTINGS_CACHE[k]=v
    return v

def clear_settings_cache(): _SETTINGS_CACHE.clear()
...
def storage_health():
    ...
    for label,path in [('Data',DATA),('Database',DB),('Employee Files',EMPFILES),('Backups',BACKUPS)]:
        ...
        free_mb=None
        try: free_mb=round(shutil.disk_usage(path if os.path.exists(path) else DATA).free/1024/1024,1)
        except Exception: pass
        checks.append({'name':label,'exists':ok,'writable':writable,'free_mb':free_mb})
    return checks
...
def find_hospital_header(rows, max_scan=30):
    ...
    # Universal mappings: every saved employee mapping is a source of trusted
    # column names.  This keeps the import engine independent from one fixed
    # hospital template while preserving the legacy aliases.
    try:
        cc=db(); saved=cc.execute("SELECT mapping_json FROM import_mappings WHERE kind='employees' ORDER BY created_at DESC").fetchall(); cc.close()
        for rr in saved:
            try:
                mm=json.loads(rr['mapping_json'] or '{}')
                for fk,hname in mm.items():
                    if fk in HOSPITAL_FIELDS and hname: aliases.setdefault(fk,[]).append(str(hname))
            except Exception: pass
    except Exception: pass
    ...
    for ridx,row in enumerate(rows[:max_scan]):
        ...
        score=len(found)
        identity=('name' in found and ('emp_no' in found or 'national_id' in found or 'phone' in found))
        if identity and (score>=3 or dynamic) and (best is None or score>best[0]): best=(score,ridx,found)
...
# --------------------------------------------------------------------------
# PERMISSION CACHE — generation-guarded.
#
# The cache was a bare dict read and written without a lock. The dangerous
# interleaving is not the duplicate DB read (that is idempotent); it is this:
#
#   thread A: _role_perms('HR') misses, reads role_permissions from SQLite
#   thread B: roles_save() commits a revocation and pops 'HR' from the cache
#   thread A: writes its PRE-revocation result back into the cache
#
# Because there is no TTL, the revoked permission then stays granted until the
# process restarts. A generation counter closes it: a populate is only stored
# if no invalidation happened while the read was in flight.
# --------------------------------------------------------------------------
_ROLE_PERMS_CACHE={}
_ROLE_PERMS_GENERATION=0

def _role_perms(role):
    with _CACHE_LOCK:
        entry=_ROLE_PERMS_CACHE.get(role)
        if entry is not None:
            perms,cached_at=entry
            if (time.time()-cached_at)<_ROLE_PERMS_TTL:
                return perms
            _ROLE_PERMS_CACHE.pop(role,None)
        generation=_ROLE_PERMS_GENERATION
    # The DB read happens outside the lock so permission checks never serialise
    # behind SQLite; correctness is restored by the generation check below.
    c=db(); perms={r['permission'] for r in c.execute('SELECT permission FROM role_permissions WHERE role=?',(role,)).fetchall()}; c.close()
    with _CACHE_LOCK:
        if generation==_ROLE_PERMS_GENERATION:
            _ROLE_PERMS_CACHE[role]=(perms,time.time())
        else:
            # Invalidated mid-read: return what we found for this request but do
            # not poison the cache with a possibly stale set.
            hr_log('auth','role_perms_cache_skipped_stale_write',level='DEBUG',role=role)
    return perms

def invalidate_role_perms(role=None):
    """Drop cached permissions and invalidate any populate still in flight."""
    global _ROLE_PERMS_GENERATION
    with _CACHE_LOCK:
        _ROLE_PERMS_GENERATION+=1
        if role is None:
...
def can(u,p):
    # Sidebar/menu rendering alone calls can() ~25 times per page, and it's called
    # ~120 more times across the app. Each call used to open/close a fresh SQLite
    # connection (connect + 4 PRAGMA round trips) just to answer a yes/no question.
    # role_permissions only changes via roles_save(), which invalidates this cache,
    # so caching per-role in memory is safe and removes the overwhelming majority
    # of per-request DB connections — this is the main cause of every-click lag.
    if not u: return False
    if u.get('_no_auth'): return True   # TEMP: authentication bypass (see bottom of file)
    if u.get('role')=='Admin': return True
    return p in _role_perms(u.get('role'))
...
_EMP_SCOPE_CACHE={}
_EMP_SCOPE_TTL=30
_EMP_SCOPE_MAX=int(os.environ.get('HR_EMP_SCOPE_CACHE_MAX','5000'))
def scope_needs_lookup(u):
    """True only when the decision actually depends on the employee's row.

    SuperAdmin / Admin / HR are allowed everything, and self-scope compares the
    code directly — neither needs to read the employees table. Checking the role
    only AFTER the lookup meant every permission check cost a fresh SQLite
    connection even when the answer was already known.
    """
    if u.get('role') in ('SuperAdmin','Admin','HR'): return False
    if u.get('role')=='Employee' or u.get('scope_type')=='self': return False
    return u.get('scope_type') in ('department','unit')

def emp_allowed_row(u,row):
    """Scope check against an already-fetched employee row — no query at all.

    Callers that have just selected the employees they are iterating over
    should use this. missing_documents_report() used to call emp_allowed() once
    per employee, which opened one SQLite connection per employee: rendering
    the dashboard for 5,000 employees issued ~5,000 connections and ~30,000
    statements, and took 4.8 seconds on a cold cache. That is the freeze users
    saw the first time they opened the system each day.
    """
    if u.get('role') in ('SuperAdmin','Admin','HR'): return True
    code=row['emp_code'] if not isinstance(row,str) else row
    if u.get('role')=='Employee' or u.get('scope_type')=='self':
        return code==(u.get('scope_value') or u.get('username'))
    keys=row.keys() if hasattr(row,'keys') else ()
    if u.get('scope_type')=='department':
        return ('department' in keys) and row['department']==u.get('scope_value')
    if u.get('scope_type')=='unit':
        return ('unit' in keys) and row['unit']==u.get('scope_value')
    return True

def emp_allowed(u,code):
    # Decide without touching the database whenever the role or scope already
    # determines the answer.
    if u.get('role') in ('SuperAdmin','Admin','HR'): return True
    if u.get('role')=='Employee' or u.get('scope_type')=='self':
        return code==(u.get('scope_value') or u.get('username'))
    if not scope_needs_lookup(u): return True
    cached=_EMP_SCOPE_CACHE.get(code)
    if cached and (time.time()-cached[2])<_EMP_SCOPE_TTL:
        dept,unit=cached[0],cached[1]
    else:
        c=db(); e=c.execute('SELECT emp_code,department,unit FROM employees WHERE emp_code=?',(code,)).fetchone(); c.close()
        if not e: return False
        dept,unit=e['department'],e['unit']
        with _CACHE_LOCK:
            if len(_EMP_SCOPE_CACHE)>=_EMP_SCOPE_MAX:
                # Evict the oldest third rather than clearing entirely, so a
                # busy server does not repeatedly cold-start the whole cache.
                for stale_key in sorted(_EMP_SCOPE_CACHE,key=lambda k:_EMP_SCOPE_CACHE[k][2])[:_EMP_SCOPE_MAX//3]:
                    _EMP_SCOPE_CACHE.pop(stale_key,None)
            _EMP_SCOPE_CACHE[code]=(dept,unit,time.time())
    if u.get('scope_type')=='department':return dept==u.get('scope_value')
    if u.get('scope_type')=='unit':return unit==u.get('scope_value')
    return True
...
def missing_documents_report(u):
    ...
    c=db()
    # Select the scope columns up front and filter in memory: one query for the
    # whole report instead of one connection per employee.
    emps=c.execute("SELECT emp_code,name,department,unit FROM employees WHERE status='على رأس العمل' ORDER BY name").fetchall()
    emps=[e for e in emps if emp_allowed_row(u,e)]
...
def make_backup(user,label='manual'):
    ...
    src=db(); dst=sqlite3.connect(tmpdb)
    try:
        src.backup(dst, pages=256, sleep=0.05)
        dst.commit()
    finally:
        dst.close(); src.close()
    manifest={'version':APP_VERSION,'created_at':now(),'label':label,'database':'database.db','employee_files':'employee_files/','files':[],'encrypted':backups_encrypted()}
    # Build into a staging path first; the final package is either this file
    # renamed, or its AES-256-GCM encryption.
    ...
    try:
        with zipfile.ZipFile(staging,'w',zipfile.ZIP_DEFLATED) as z:
            ...
            for root,dirs,files in os.walk(EMPFILES):
                for fn in files:
                    fp=os.path.join(root,fn); arc=os.path.relpath(fp,EMPFILES).replace('\\','/')
                    # Stream the checksum instead of open(fp).read(): a folder of
                    # scanned employee documents used to be loaded into RAM in
                    # full, one file at a time, on top of the whole archive.
                    hsh=hashlib.sha256(); size=0
                    with open(fp,'rb') as fh:
                        while True:
                            chunk=fh.read(1024*1024)
                            if not chunk: break
                            hsh.update(chunk); size+=len(chunk)
                    z.write(fp,'employee_files/'+arc)
                    manifest['files'].append({'path':'employee_files/'+arc,'size':size,'sha256':hsh.hexdigest()})
            # QR images live in data/qr/, not data/employee_files/, and were
            # therefore excluded from every backup ever taken. A disaster
            # recovery restored all qr_identities rows but none of the PNGs,
            # leaving every ID card broken with no error anywhere. Proven by
            # tests/dr/TEST_DISASTER_RECOVERY.py ("0/8 image files present").
            qrdir=os.path.join(DATA,'qr')
            if os.path.isdir(qrdir):
                for root,dirs,files in os.walk(qrdir):
                    for fn in files:
                        if fn.startswith('.tmp-'): continue
                        fp=os.path.join(root,fn); arc=os.path.relpath(fp,qrdir).replace('\\','/')
                        hsh=hashlib.sha256(); size=0
                        with open(fp,'rb') as fh:
                            while True:
                                chunk=fh.read(1024*1024)
                                ...
                                hsh.update(chunk); size+=len(chunk)
                        z.write(fp,'qr/'+arc)
                        manifest['files'].append({'path':'qr/'+arc,'size':size,'sha256':hsh.hexdigest()})
            # Branding (logo, ID-card background) is equally unrecoverable
            # from the database alone.
            if os.path.isdir(BRAND):
                for root,dirs,files in os.walk(BRAND):
                    for fn in files:
                        fp=os.path.join(root,fn); arc=os.path.relpath(fp,BRAND).replace('\\','/')
                        hsh=hashlib.sha256(); size=0
                        with open(fp,'rb') as fh:
                            while True:
                                chunk=fh.read(1024*1024)
                                if not chunk: break
                                hsh.update(chunk); size+=len(chunk)
                        z.write(fp,'branding/'+arc)
                        manifest['files'].append({'path':'branding/'+arc,'size':size,'sha256':hsh.hexdigest()})
            manifest['qr']='qr/'; manifest['branding']='branding/'
            z.writestr('manifest.json',json.dumps(manifest,ensure_ascii=False,indent=2))
        if backups_encrypted():
            encrypt_backup_file(staging,package)
            os.remove(staging)
        else:
            print('[HR] WARNING: HR_BACKUP_PLAINTEXT=1 — this backup is NOT encrypted.')
    ...
    _ck=hashlib.sha256()
    with open(package,'rb') as _fh:
        while True:
            _b=_fh.read(1024*1024)
            if not _b: break
            _ck.update(_b)
    checksum=_ck.hexdigest()
    verified,verify_msg=verify_backup_package(package)
    if not verified: raise ValueError('Backup verification failed: '+verify_msg)
    # Optional second copy: a UNC/network folder or mounted backup directory.
    # The local verified backup is always retained even if the network target is unavailable.
    network_target=(setting('backup_network_path') or '').strip()
    network_copy=''
    if network_target:
        try:
            os.makedirs(network_target,exist_ok=True)
            network_copy=os.path.join(network_target,os.path.basename(package))
            shutil.copy2(package,network_copy)
            ok2,msg2=verify_backup_package(network_copy)
            if not ok2: raise ValueError('network backup verification failed: '+msg2)
        except Exception as e:
            log_error('backup-network-copy',e,username=user.get('username','system'),method='BACKUP',path='/backup')
            network_copy=''
    size=os.path.getsize(package); c=db(); cur=c.execute('INSERT INTO system_backups(file_path,created_at,created_by,label,db_size) VALUES(?,?,?,?,?)',(package,now(),user.get('username','system'),label,size)); c.execute('INSERT INTO backup_files(backup_id,path,size,checksum) VALUES(?,?,?,?)',(cur.lastrowid,package,size,checksum));
    if network_copy:
        c.execute('INSERT INTO backup_files(backup_id,path,size,checksum) VALUES(?,?,?,?)',(cur.lastrowid,network_copy,os.path.getsize(network_copy),hashlib.sha256(open(network_copy,'rb').read()).hexdigest()))
    c.commit(); c.close(); prune_backups(); return package
...
# Backup encryption.
#
# Previously HR_Backup_*.zip contained database.db (national IDs, IBANs,
# salaries) plus every employee document in cleartext. Anyone who obtained the
# file — a copied USB stick, an unsecured network share — had the whole HR
# database. Backups are now AES-256-GCM encrypted with a key held outside the
# archive.
# --------------------------------------------------------------------------
BACKUP_MAGIC = b'HRENC1\x00'
...
def _backup_key_path():
    return os.path.join(DATA, 'backup.key')

def backup_key():
    """Resolve the 32-byte backup key.

    HR_BACKUP_KEY (base64 or hex) wins, so the key can live in Windows
    Credential Manager / a secrets store and never touch the data directory.
    Otherwise a key file is generated once with 0600 permissions.
    """
    env = os.environ.get('HR_BACKUP_KEY', '').strip()
    if env:
        try:
            raw = base64.b64decode(env) if len(env) != 64 else bytes.fromhex(env)
        except Exception:
            raise RuntimeError('HR_BACKUP_KEY must be base64 or 64 hex characters.')
        if len(raw) != 32:
            raise RuntimeError('HR_BACKUP_KEY must decode to exactly 32 bytes.')
        return raw
    path = _backup_key_path()
    if os.path.exists(path):
        raw = open(path, 'rb').read().strip()
        try: raw = base64.b64decode(raw)
        except Exception: pass
        if len(raw) == 32: return raw
        raise RuntimeError('backup.key is corrupt; restore it from your key escrow.')
    ...
    print('=' * 72)
    print('[HR] Backup encryption key generated:', path)
    print('[HR] COPY THIS FILE SOMEWHERE SAFE AND OFFLINE.')
    print('[HR] Without it, none of your encrypted backups can ever be restored.')
    print('=' * 72)
    return raw
...
#
# Some credentials must be stored recoverably (the app has to present them to
# a device), so hashing is not an option — they need encryption at rest plus a
# UI that never echoes them back. ZKTeco zk_devices.comm_password was the first
# such value: stored in cleartext and rendered in full inside the device form,
# so it appeared in the page source, in any browser cache, and in every backup.
# --------------------------------------------------------------------------
SECRET_PREFIX = 'enc:v1:'

def _secret_key_path():
    return os.path.join(DATA, 'secret.key')

def app_secret_key():
    """32-byte key for reversible application secrets.

    HR_SECRET_KEY (base64 or hex) takes precedence so the key can live in
    Windows Credential Manager. Otherwise a key file is generated once.
    This is deliberately separate from the backup key: rotating one must not
    invalidate the other.
    """
    env = os.environ.get('HR_SECRET_KEY', '').strip()
    if env:
        try:
            raw = bytes.fromhex(env) if len(env) == 64 else base64.b64decode(env)
        except Exception:
            raise RuntimeError('HR_SECRET_KEY must be base64 or 64 hex characters.')
        if len(raw) != 32:
            raise RuntimeError('HR_SECRET_KEY must decode to exactly 32 bytes.')
        return raw
    ...
    if os.path.exists(path):
        ...
        if len(raw) == 32:
            return raw
        raise RuntimeError('secret.key is corrupt; device passwords cannot be decrypted.')
    raw = secrets.token_bytes(32)
    os.makedirs(DATA, exist_ok=True)
    with open(path, 'wb') as fh:
        fh.write(base64.b64encode(raw))
    protect_secret_file(path)
    print('[HR] Application secret key generated:', path)
    return raw

def encrypt_secret(plaintext):
    """Encrypt a recoverable secret. Returns '' for empty input."""
    if plaintext is None or plaintext == '':
        return ''
    if is_encrypted_secret(plaintext):
        return plaintext
    if not _HAVE_AESGCM:
        raise RuntimeError("Storing device credentials requires the 'cryptography' package.")
    aes = AESGCM(app_secret_key()); nonce = secrets.token_bytes(12)
    blob = nonce + aes.encrypt(nonce, str(plaintext).encode('utf-8'), b'zk-comm-password')
    return SECRET_PREFIX + base64.b64encode(blob).decode('ascii')

def decrypt_secret(stored):
    """Decrypt a stored secret; pass through legacy cleartext unchanged."""
    if not stored:
        return ''
    if not is_encrypted_secret(stored):
        return stored          # legacy row not yet migrated
    if not _HAVE_AESGCM:
        raise RuntimeError("Reading device credentials requires the 'cryptography' package.")
    blob = base64.b64decode(stored[len(SECRET_PREFIX):])
    aes = AESGCM(app_secret_key())
    return aes.decrypt(blob[:12], blob[12:], b'zk-comm-password').decode('utf-8')

def is_encrypted_secret(value):
    return isinstance(value, str) and value.startswith(SECRET_PREFIX)

def mask_secret(stored):
    """What the UI is allowed to show: presence, never the value."""
    return '' if not stored else '********'

def protect_secret_file(path):
    """Restrict a secret file to the current account.

    On Windows, os.chmod() only toggles the read-only attribute — it does NOT
    produce a Unix-like private file, so icacls is used to replace inheritance
    with an explicit grant. The call is idempotent, logged, and never fatal:
    losing access to our own key would be worse than a loose ACL.
    """
    if not os.path.exists(path):
        return False
    if os.name == 'nt':
        account = os.environ.get('USERNAME') or 'CURRENT_USER'
        try:
            # CREATE_NO_WINDOW: the packaged app runs --windowed, and a bare
            # subprocess would flash a console window on every key it protects.
            res = subprocess.run(
            ...
            if res.returncode == 0:
                print(f'[HR] ACL applied to {os.path.basename(path)} (grant: {account}, Administrators)')
                return True
            print(f'[HR] WARNING: could not set ACL on {os.path.basename(path)}: {res.stderr.strip()[:200]}')
            return False
        except (OSError, subprocess.SubprocessError) as e:
            print(f'[HR] WARNING: ACL command failed for {os.path.basename(path)}: {e}')
            return False
    try:
        os.chmod(path, 0o600)
        return True
    except OSError as e:
        print(f'[HR] WARNING: chmod failed for {os.path.basename(path)}: {e}')
        return False

def backups_encrypted():
    """False only when the operator has explicitly opted out."""
    return os.environ.get('HR_BACKUP_PLAINTEXT', '').strip() != '1'

def encrypt_backup_file(src, dst):
    """Stream-encrypt src -> dst with AES-256-GCM in 4 MB chunks."""
    if not _HAVE_AESGCM:
        raise RuntimeError("Backup encryption requires the 'cryptography' package. "
                           "Install it, or set HR_BACKUP_PLAINTEXT=1 to accept unencrypted backups.")
    key = backup_key(); aes = AESGCM(key); base_nonce = secrets.token_bytes(8); idx = 0
    chunk_size = 4 * 1024 * 1024
    with open(src, 'rb') as fi, open(dst, 'wb') as fo:
        fo.write(BACKUP_MAGIC); fo.write(base_nonce)
        while True:
            chunk = fi.read(chunk_size)
            if not chunk: break
            nonce = base_nonce + idx.to_bytes(4, 'big')
            blob = aes.encrypt(nonce, chunk, None)
            fo.write(len(blob).to_bytes(4, 'big')); fo.write(blob)
            idx += 1
        fo.write((0).to_bytes(4, 'big'))

def decrypt_backup_file(src, dst):
    """Inverse of encrypt_backup_file; raises if any chunk fails its GCM tag."""
    if not _HAVE_AESGCM:
        raise RuntimeError("Backup decryption requires the 'cryptography' package.")
    key = backup_key(); aes = AESGCM(key)
    with open(src, 'rb') as fi, open(dst, 'wb') as fo:
        if fi.read(len(BACKUP_MAGIC)) != BACKUP_MAGIC:
            raise ValueError('not an HR encrypted backup')
        base_nonce = fi.read(8); idx = 0
        while True:
            head = fi.read(4)
            if len(head) < 4: raise ValueError('truncated encrypted backup')
            n = int.from_bytes(head, 'big')
            if n == 0: break
            blob = fi.read(n)
            if len(blob) != n: raise ValueError('truncated encrypted backup')
            fo.write(aes.decrypt(base_nonce + idx.to_bytes(4, 'big'), blob, None))
            idx += 1

def is_encrypted_backup(path):
    try:
        with open(path, 'rb') as fh: return fh.read(len(BACKUP_MAGIC)) == BACKUP_MAGIC
    except Exception:
        return False
...
#
# z.extractall() trusts every member name. Even for a self-generated archive
# that is the wrong default: anyone able to drop a file into the backups folder
# could write outside the restore directory via ../ or an absolute path.
# --------------------------------------------------------------------------
SAFE_EXTRACT_MAX_FILES  = int(os.environ.get('HR_ZIP_MAX_FILES', '20000'))
SAFE_EXTRACT_MAX_BYTES  = int(os.environ.get('HR_ZIP_MAX_BYTES', str(4*1024*1024*1024)))
SAFE_EXTRACT_MAX_RATIO  = int(os.environ.get('HR_ZIP_MAX_RATIO', '200'))

def safe_extract_zip(zf, dest, max_files=None, max_bytes=None):
    """Validate every member, then extract. Raises ValueError on anything unsafe."""
    max_files = max_files or SAFE_EXTRACT_MAX_FILES
    max_bytes = max_bytes or SAFE_EXTRACT_MAX_BYTES
    dest_abs = os.path.abspath(dest)
    infos = zf.infolist()
    if len(infos) > max_files:
        raise ValueError(f'archive has {len(infos)} entries (limit {max_files})')
    ...
    for info in infos:
        ...
        if len(name) > 255:
            raise ValueError('archive member name too long')
        # High bits of external_attr hold the Unix mode; 0xA000 == symlink.
        if (info.external_attr >> 16) & 0xF000 == 0xA000:
            raise ValueError('symlink in archive: ' + name)
        target = os.path.abspath(os.path.join(dest_abs, name))
        if target != dest_abs and not target.startswith(dest_abs + os.sep):
            raise ValueError('member escapes destination: ' + name)
        total_out += info.file_size
        total_in += max(info.compress_size, 1)
        if total_out > max_bytes:
            raise ValueError(f'uncompressed size exceeds {max_bytes} bytes')
    if total_in and (total_out / total_in) > SAFE_EXTRACT_MAX_RATIO:
        raise ValueError('compression ratio looks like a zip bomb')
    zf.extractall(dest_abs)
    return len(infos), total_out

def verify_backup_package(path):
    tmp_plain=None
    try:
        if is_encrypted_backup(path):
            tmp_plain=path+'.verify.tmp'
            decrypt_backup_file(path,tmp_plain)
            path=tmp_plain
...
def employee_self_code(u):
    ...
    c=db()
    candidates=[u.get('employee_code') or '',u.get('scope_value') or '',u.get('username') or '']
    row=None
...
def _legacy_shell(title,body,user,active='dashboard'):
    ...
    if can(user,'employees.view'): people.append(('matching','🧩 مراجعة المطابقة','/matching'))
    if user.get('role')=='Employee': people.append(('myhr','👤 حسابي الوظيفي','/myhr'))
    grp('الموظفون',people)
    attend=[]
    if can(user,'attendance.view'): attend.append(('attendance','⏰ الحضور والانصراف','/attendance'))
    if can(user,'leave.create'): attend.append(('leaves','🌴 الإجازات','/leaves'))
    if can(user,'leave.create'): attend.append(('leave-balances','📊 أرصدة الإجازات','/leave-balances'))
    if can(user,'overtime.request'): attend.append(('overtime','🕐 الإضافي','/overtime'))
    if can(user,'shifts.manage'): attend.append(('shifts','🔁 الورديات','/shifts'))
    grp('الحضور والإجازات',attend)
    pay=[]
    ...
    if can(user,'discipline.manage'): pay.append(('discipline','⚠️ الجزاءات والإنذارات','/discipline'))
    if can(user,'reports.view'): pay.append(('reports','📈 التقارير','/reports'))
    grp('المرتبات والتقارير',pay)
    idn=[]
    if can(user,'employees.view'): idn.append(('enterprise','🪪 البطاقات و QR والحسابات','/enterprise'))
    if can(user,'employees.view'): idn.append(('alerts','🚨 تنبيهات الموارد البشرية','/alerts'))
    grp('الهوية والبطاقات',idn)
    sup=[]
    if can(user,'supplies.view'): sup.append(('supplies','📦 لوحة المستلمات','/supplies'))
    if can(user,'supplies.issue'): sup.append(('supplies-issue','🧾 تسجيل استلام جديد','/supplies/issue'))
    if can(user,'supplies.view'): sup.append(('supplies-log','📜 سجل المستلمات','/supplies/log'))
    if can(user,'supplies.manage'): sup.append(('supplies-catalog','🗂 كتالوج الأصناف','/supplies/catalog'))
    if can(user,'supplies.manage'): sup.append(('supplies-quotas','🎚 كوتة الأقسام الشهرية','/supplies/quotas'))
    grp('المستلمات',sup)
    data=[]
    if can(user,'employees.edit'): data.append(('import','📥 استيراد Excel','/import'))
    if can(user,'import.mapping'): data.append(('import-map','🔗 ربط الأعمدة','/import/mapping'))
    if can(user,'documents.manage'): data.append(('documents','📄 المستندات','/documents'))
    grp('البيانات والمستندات',data)
    admin=[]
    if can(user,'users.manage'): admin.append(('users','🔑 المستخدمون','/users'))
    if can(user,'roles.manage'): admin.append(('roles','🛡 الأدوار والصلاحيات','/roles'))
    if can(user,'users.manage'): admin.append(('access','🚪 Access Control','/access'))
    ...
    if can(user,'system.manage'): admin.append(('network','🖥 الشبكة والأجهزة','/network'))
    if can(user,'system.manage'): admin.append(('devices','📱 الأجهزة المتصلة','/devices'))
    if can(user,'backup.manage'): admin.append(('backups','💾 نسخ احتياطي / استرجاع','/backups'))
    if can(user,'system.manage'): admin.append(('diagnostics','🧰 سجل الأخطاء','/diagnostics/errors'))
    if can(user,'audit.view'): admin.append(('audit','📜 سجل المراجعة','/audit'))
    grp('الإدارة والنظام',admin)
    def group_html(label,items):
        opened='open' if active in [k for k,_,_ in items] or label=='الموظفون' else ''
        inner=''.join(f'<a class="{"active" if active==k else ""}" href="{u}">{t}</a>' for k,t,u in items)
        return f'<details class="nav-group" {opened}><summary>{esc(label)}</summary>{inner}</details>'
    links='<a class="active" href="/">🏠 لوحة التحكم</a>'+''.join(group_html(label,items) for label,items in groups)
    ...
    search_box='''<div class="gsearch"><form action="/search" method="get"><input id="gsearch" name="q" placeholder="بحث سريع…" autocomplete="off"><kbd>Ctrl K</kbd></form>
<script>document.addEventListener('keydown',function(e){if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();var el=document.getElementById('gsearch');if(el){el.focus();el.select();}}});</script>'''
    ...
    return f'''<!doctype html><html lang="ar"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="icon" href="/branding/favicon"><title>{esc(title)} — {company}</title><link rel="stylesheet" href="/static/style.css?v={CSS_VERSION}"></head><body><div class="app" id="appRoot"><button id="sideMobileToggle" class="mobile-menu" type="button" aria-label="فتح القائمة الجانبية">☰</button><div id="sideOverlay" class="side-overlay"></div><aside class="side" id="mainSide"><div class="brand"><button id="sideToggle" class="side-toggle" type="button" title="طي/فتح القائمة الجانبية" aria-label="طي القائمة الجانبية">☰</button><div class="brand-info">{('<img src="/branding/logo" alt="logo" style="max-width:92px;max-height:58px;display:block;margin-bottom:10px;border-radius:10px;background:#fff;padding:5px">') if setting('company_logo') else ''}<b>{company}</b><small>{esc(setting("brand_tagline") or "نظام الموارد البشرية")}</small></div></div>{search_box}<nav class="nav">{links}</nav><div class="footer">{esc(user['full_name'])} · {esc(user['role'])}</div></aside><main class="main">{forced}{body}<div class="footer">HR Enterprise · برنامج واحد · بيانات موحدة · {esc(user['full_name'])}</div></main></div>
<script>{SIDE_JS}</script>
<script>
(function(){{
...
# SHELL RESOLUTION
#
# Six modules wrap page() to append their own sidebar links, each capturing
# whatever `page` happened to be at its own import time. A module therefore saw
# only the wrappers installed before it, which is why /id-cards rendered four
# sidebar entries that /users did not.
#
# Two indirections make it uniform without unwinding the chain:
#   _SHELL_BASE  — what the innermost wrapper ultimately renders. The final
#                  layer swaps in its curated navigation here.
#   _PAGE_IMPL   — what page() dispatches to: the OUTERMOST wrapper, so every
#                  caller passes through every wrapper exactly once.
# ---------------------------------------------------------------------------
_SHELL_BASE=_legacy_shell

def _default_page(title,body,user,active='dashboard'):
    return _SHELL_BASE(title,body,user,active)

_PAGE_IMPL=_default_page

def page(title,body,user,active='dashboard'):
    """Render the application shell.

    Always resolves the current implementation at call time. stable_final
    installs its curated navigation by assigning _PAGE_IMPL, and every module
    — including ones loaded before it — then renders the same sidebar.
    """
    return _PAGE_IMPL(title,body,user,active)

def first_run_hint():
    """One-line setup hint, shown only until the default password is changed."""
    if not default_credentials_active():
        return ''
    return ('<div class="alert" style="background:#fff7e6;border-color:#f5c26b;color:#7a4b00">'
            '<b>أول تشغيل:</b> اسم المستخدم <b>admin</b> وكلمة المرور <b>'
            + esc(_bootstrap_admin_password()) +
            '</b>. سيُطلب منك تغييرها فور الدخول.</div>')

def login_page(msg=''):
    return f'''<!doctype html><html lang="ar"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="icon" href="/branding/favicon"><title>تسجيل الدخول</title><link rel="stylesheet" href="/static/style.css?v={CSS_VERSION}"></head><body><div class="login"><form class="login-card" method="post" action="/login">{('<img src="/branding/logo" alt="logo" style="max-width:150px;max-height:80px;display:block;margin:0 auto 14px">') if setting('company_logo') else ''}<h1>نظام الموارد البشرية</h1><p>HR Enterprise Multi-User</p>{('<div class="alert">'+esc(msg)+'</div>') if msg else ''}{first_run_hint()}<input name="username" placeholder="اسم المستخدم" autofocus><input name="password" type="password" placeholder="كلمة المرور"><button class="btn">دخول للنظام</button><div class="footer">في أول تشغيل استخدم حساب المسؤول الذي تم إنشاؤه أثناء الإعداد، ثم غيّر كلمة المرور فورًا.</div></form></div></body></html>'''

SESSION_REVALIDATE_SECS=8   # how often a session is re-checked against the DB (active/role/permission_version)
SESSION_WRITE_SECS=45       # how often system_sessions.last_seen is actually written to disk

# --- Authenticated GET route table -----------------------------------------
# Converted from the original long if-chain in H.do_GET (kept 1:1 in behavior
# and original evaluation order). This table is internal to this module only:
# the v10..production_ops layers still wrap the *whole* H.do_GET method
# reference (old_get=H.do_GET / H.do_GET=get), so they are completely
# unaffected by how this base do_GET dispatches internally.
#
# GET_EXACT: exact path -> handler(self,u). Checked first, O(1) lookup.
# GET_PREFIX: ordered list of (prefix, handler(self,u,path)), checked in
#   order after GET_EXACT misses. Order matters: more specific prefixes
#   (e.g. '/export/import-errors/') must be listed before broader ones
#   (e.g. '/export/') exactly as in the original if-chain.
GET_EXACT_ROUTES = {
    '/system/health.json': lambda self,u: self.system_health_json(u),
    '/bridge/token': lambda self,u: self.bridge_token(u),
    '/bridge/authorize': lambda self,u: self.bridge_authorize(u),
    '/password': lambda self,u: self.password_page(u),
    '/': lambda self,u: self.dashboard(u),
    '/enterprise': lambda self,u: self.enterprise_center(u),
    '/alerts': lambda self,u: self.alerts_page(u),
    '/matching': lambda self,u: self.matching_page(u),
    '/myhr': lambda self,u: self.myhr(u),
    '/requests': lambda self,u: self.requests_page(u),
    '/devices': lambda self,u: self.need(u,'system.manage') and self.devices_page(u),
    '/device/ping': lambda self,u: self.device_ping(u),
    '/employees': lambda self,u: self.employees(u),
    '/employees/views': lambda self,u: self.employees_views(u),
    '/export/employees/selected': lambda self,u: self.need(u,'reports.export') and self.export_selected_employees(u),
    '/documents/paste': lambda self,u: self.need(u,'documents.manage') and self.paste_documents(u),
    '/documents/bridge-upload': lambda self,u: self.need(u,'documents.manage') and self.bridge_upload(u),
    '/employee/new': lambda self,u: self.employee_form(u),
    '/leaves': lambda self,u: self.leaves(u),
    '/leave/new': lambda self,u: self.leave_form(u),
    '/attendance': lambda self,u: self.attendance(u),
    '/reports': lambda self,u: self.reports(u),
    '/import': lambda self,u: self.import_page(u),
    '/users': lambda self,u: self.users(u),
    '/settings': lambda self,u: self.need(u,'settings.manage') and self.settings_page(u),
    '/audit': lambda self,u: self.need(u,'audit.view') and self.audit_page(u),
    '/audit/verify': lambda self,u: self.need(u,'audit.view') and self.audit_verify_page(u),
    '/search': lambda self,u: self.search_page(u),
    '/import/folder-history': lambda self,u: self.need(u,'documents.bulk_import') and self.folder_import_history(u),
    '/backup': lambda self,u: self.need(u,'backup.manage') and self.backup(u),
    '/notifications': lambda self,u: self.notifications(u),
    '/documents': lambda self,u: self.documents(u),
    '/documents/folders': lambda self,u: self.folder_import_page(u),
    '/documents/folders/import-files': lambda self,u: self.folder_import_page(u),
    '/roles': lambda self,u: self.need(u,'roles.manage') and self.roles_page(u),
    '/payroll': lambda self,u: self.need(u,'payroll.view') and self.payroll(u),
    '/access': lambda self,u: self.need(u,'users.manage') and self.access_page(u),
    '/shifts': lambda self,u: self.need(u,'shifts.manage') and self.shifts_page(u),
    '/overtime': lambda self,u: self.need(u,'overtime.request') and self.overtime_page(u),
    '/leave-balances': lambda self,u: self.need(u,'leave.create') and self.leave_balances_page(u),
    '/payroll/review': lambda self,u: self.need(u,'payroll.view') and self.payroll_actions(u),
    '/import/mapping': lambda self,u: self.need(u,'import.mapping') and self.import_mapping_page(u),
    '/backups': lambda self,u: self.need(u,'backup.manage') and self.backups_page(u),
    '/system': lambda self,u: self.need(u,'settings.manage') and self.system_page(u),
    '/diagnostics/errors': lambda self,u: self.need(u,'system.manage') and self.diagnostics_errors(u),
    '/diagnostics/test': lambda self,u: self.need(u,'system.manage') and self.diagnostics_test(u),
    '/network': lambda self,u: self.need(u,'system.manage') and self.network_page(u),
    '/network/ping': lambda self,u: self.need(u,'system.manage') and self.network_ping(u),
    '/discipline': lambda self,u: self.need(u,'discipline.manage') and self.discipline_page(u),
    '/template/attendance': lambda self,u: self.export_template_attendance(),
    '/template/leaves': lambda self,u: self.export_template_leaves(),
    '/template/employees': lambda self,u: self.export_template_employees(),
    '/supplies': lambda self,u: self.need(u,'supplies.view') and self.supplies_dashboard(u),
    '/supplies/issue': lambda self,u: self.need(u,'supplies.issue') and self.supplies_issue_page(u),
    '/supplies/log': lambda self,u: self.need(u,'supplies.view') and self.supplies_log_page(u),
    '/supplies/catalog': lambda self,u: self.need(u,'supplies.manage') and self.supplies_catalog_page(u),
    '/supplies/quotas': lambda self,u: self.need(u,'supplies.manage') and self.supplies_quotas_page(u),
    '/supplies/log/export.csv': lambda self,u: self.need(u,'supplies.view') and self.supplies_export_csv(u),
    # NOTE: original if-chain also had a second, unreachable 'if path==\'/health\':'
    # branch here (dead code -- '/health' is already handled pre-auth at the top
    # of do_GET and returns before self.require() is ever called). Dropped as
    # dead code; behavior is identical since it could never execute.
}

GET_PREFIX_ROUTES = [
    ('/export/html/', lambda self,u,path: self.need(u,'reports.export') and self.export_html(path.split('/')[-1],u)),
    ('/export/csv/', lambda self,u,path: self.need(u,'reports.export') and self.export_csv(path.split('/')[-1],u)),
    ('/employee/edit/', lambda self,u,path: self.employee_form(u,path.split('/')[-1])),
    ('/employee/profile/', lambda self,u,path: self.employee_profile(u,path.split('/')[-1])),
    ('/export/import-errors/', lambda self,u,path: self.need(u,'import.validate') and self.export_import_errors(path.split('/')[-1])),
    ('/export/pdf/', lambda self,u,path: self.need(u,'reports.export') and self.pdf_report(path.split('/')[-1],u)),
    ('/export/', lambda self,u,path: self.need(u,'reports.export') and self.export_route(path)),
    ('/document/', lambda self,u,path: self.document_download(u,path.split('/')[-1])),
    ('/report/pdf/', lambda self,u,path: self.need(u,'reports.export') and self.pdf_report(path.split('/')[-1],u)),
    ('/supplies/receipt/', lambda self,u,path: self.need(u,'supplies.view') and self.supplies_receipt(u,path.split('/')[-1])),
    ('/import/errors/', lambda self,u,path: self.need(u,'import.validate') and self.import_errors_page(u,path.split('/')[-1])),
    # NOTE: original if-chain repeated '/export/import-errors/' a second time
    # further down; that second check was unreachable (the first one above
    # already returns), so it is not duplicated here.
]

POST_EXACT_ROUTES = {
    '/bridge/authorize': lambda self,u,f: self.bridge_authorize_token(),
    '/password': lambda self,u,f: self.password_save(u,f),
    '/enterprise/save': lambda self,u,f: self.need(u,'employees.edit') and self.enterprise_save(u,f),
    '/matching/review': lambda self,u,f: self.need(u,'employees.edit') and self.matching_review(u,f),
    '/myhr/request': lambda self,u,f: self.need(u,'leave.create') and self.myhr_request(u,f),
    '/requests/action': lambda self,u,f: self.request_action(u,f),
    '/employee/save': lambda self,u,f: self.need(u,'employees.edit') and self.save_employee(u,f),
    '/employee/evaluation/save': lambda self,u,f: self.need(u,'employees.edit') and self.save_evaluation(u,f),
    '/employees/bulk': lambda self,u,f: self.need(u,'employees.edit') and self.bulk_employees(u,f),
    '/export/employees/selected': lambda self,u,f: self.need(u,'reports.export') and self.export_selected_employees(u),
    '/documents/paste': lambda self,u,f: self.need(u,'documents.manage') and self.paste_documents(u),
    '/employees/views/save': lambda self,u,f: self.need(u,'employees.edit') and self.save_employee_view(u,f),
    '/leave/save': lambda self,u,f: self.need(u,'leave.create') and self.save_leave(u,f),
    '/leave/status': lambda self,u,f: self.need(u,'leave.approve') and self.leave_status(u,f),
    '/attendance/import': lambda self,u,f: self.need(u,'attendance.import') and self.do_import_attendance(u),
    '/import/employees': lambda self,u,f: self.need(u,'employees.edit') and self.do_import_employees(u),
    '/import/employees/paste': lambda self,u,f: self.need(u,'employees.edit') and self.do_import_employees_paste(u,f),
    '/settings/save': lambda self,u,f: self.need(u,'settings.manage') and self.save_settings(u,f),
    '/branding/logo': lambda self,u,f: self.need(u,'settings.manage') and self.branding_logo_save(u),
    '/users/save': lambda self,u,f: self.save_user(u,f),
    '/users/toggle': lambda self,u,f: self.toggle_user(u,f),
    '/backup': lambda self,u,f: self.need(u,'backup.manage') and self.backup(u),
    '/notifications/read': lambda self,u,f: self.notifications_read(u,f),
    '/documents/upload': lambda self,u,f: self.need(u,'documents.manage') and self.document_upload(u),
    '/documents/folders/import': lambda self,u,f: self.need(u,'documents.bulk_import') and self.folder_import(u),
    '/documents/folders/import-files': lambda self,u,f: self.need(u,'documents.bulk_import') and self.folder_files_import(u),
    '/roles/save': lambda self,u,f: self.need(u,'roles.manage') and self.roles_save(u,f),
    '/payroll/save': lambda self,u,f: self.need(u,'payroll.manage') and self.payroll_save(u,f),
    '/access/save': lambda self,u,f: self.need(u,'users.manage') and self.access_save(u,f),
    '/shift/save': lambda self,u,f: self.need(u,'shifts.manage') and self.shift_save(u,f),
    '/shift/assign': lambda self,u,f: self.need(u,'shifts.manage') and self.shift_assign(u,f),
    '/overtime/save': lambda self,u,f: self.need(u,'overtime.request') and self.overtime_save(u,f),
    '/overtime/status': lambda self,u,f: self.need(u,'overtime.approve') and self.overtime_status(u,f),
    '/payroll/approve': lambda self,u,f: self.need(u,'payroll.approve') and self.payroll_approve(u,f),
    '/payroll/lock': lambda self,u,f: self.need(u,'payroll.lock') and self.payroll_lock(u,f),
    '/import/mapping/save': lambda self,u,f: self.need(u,'import.mapping') and self.import_mapping_save(u,f),
    '/backup/restore': lambda self,u,f: self.need(u,'backup.manage') and self.backup_restore(u,f),
    '/discipline/save': lambda self,u,f: self.need(u,'discipline.manage') and self.discipline_save(u,f),
    '/supplies/issue/save': lambda self,u,f: self.need(u,'supplies.issue') and self.supplies_issue_save(u,f),
    '/supplies/catalog/save': lambda self,u,f: self.need(u,'supplies.manage') and self.supplies_catalog_save(u,f),
    '/supplies/catalog/toggle': lambda self,u,f: self.need(u,'supplies.manage') and self.supplies_catalog_toggle(u,f),
    '/supplies/quotas/save': lambda self,u,f: self.need(u,'supplies.manage') and self.supplies_quotas_save(u,f),
    '/attendance/save': lambda self,u,f: self.need(u,'attendance.edit') and self.attendance_save(u,f),
    '/attendance/adjust': lambda self,u,f: self.need(u,'attendance.edit') and self.attendance_adjust(u,f),
    '/notifications': lambda self,u,f: self.notifications(u),
    '/documents': lambda self,u,f: self.documents(u),
    '/documents/folders': lambda self,u,f: self.folder_import_page(u),
    '/roles': lambda self,u,f: self.need(u,'roles.manage') and self.roles_page(u),
    '/payroll': lambda self,u,f: self.need(u,'payroll.view') and self.payroll(u),
    '/access': lambda self,u,f: self.need(u,'users.manage') and self.access_page(u),
    '/shifts': lambda self,u,f: self.need(u,'shifts.manage') and self.shifts_page(u),
    '/overtime': lambda self,u,f: self.need(u,'overtime.request') and self.overtime_page(u),
    '/leave-balances': lambda self,u,f: self.need(u,'leave.create') and self.leave_balances_page(u),
    '/payroll/review': lambda self,u,f: self.need(u,'payroll.view') and self.payroll_actions(u),
    '/import/mapping': lambda self,u,f: self.need(u,'import.mapping') and self.import_mapping_page(u),
    '/backups': lambda self,u,f: self.need(u,'backup.manage') and self.backups_page(u),
    '/system': lambda self,u,f: self.need(u,'settings.manage') and self.system_page(u),
    '/diagnostics/errors': lambda self,u,f: self.need(u,'system.manage') and self.diagnostics_errors(u),
    '/diagnostics/test': lambda self,u,f: self.need(u,'system.manage') and self.diagnostics_test(u),
    '/network': lambda self,u,f: self.need(u,'system.manage') and self.network_page(u),
    '/network/ping': lambda self,u,f: self.need(u,'system.manage') and self.network_ping(u),
    '/discipline': lambda self,u,f: self.need(u,'discipline.manage') and self.discipline_page(u),
    '/template/attendance': lambda self,u,f: self.export_template_attendance(),
    '/template/leaves': lambda self,u,f: self.export_template_leaves(),
    '/health': lambda self,u,f: self.send(json.dumps({'ok':True,'version':APP_VERSION,'server_name':setting('server_name') or 'HR-SERVER'},ensure_ascii=False),200,'application/json'),
    '/template/employees': lambda self,u,f: self.export_template_employees(),
}

POST_PREFIX_ROUTES = [
    ('/employee/archive/', lambda self,u,f,seg: self.need(u,'employees.edit') and self.archive_employee(u,seg)),
    ('/employee/restore/', lambda self,u,f,seg: self.need(u,'employees.edit') and self.restore_employee(u,seg)),
    ('/documents/folders/rollback/', lambda self,u,f,seg: self.need(u,'documents.bulk_import') and self.folder_import_rollback(u,seg)),
    ('/document/', lambda self,u,f,seg: self.document_download(u,seg)),
    ('/report/pdf/', lambda self,u,f,seg: self.pdf_report(seg,u)),
]

# --------------------------------------------------------------------------
# CONTENT SECURITY POLICY
#
# The policy used to be script-src 'self' 'unsafe-inline', which permits ANY
# injected inline script and so gives almost no protection against XSS.
#
# Fully removing inline script would mean rewriting 54 on*= handlers and 37
# inline <script> blocks across 8 template modules — a frontend refactor that
# cannot be verified page-by-page here. The standard migration path is used
# instead, and it is a real improvement rather than a cosmetic one:
#
#   * every inline <script> block is stamped with a per-response nonce, so an
#     injected <script> without the nonce will not execute;
#   * the on*= handlers that the page genuinely contains are allowed by their
#     individual SHA-256 hashes via 'unsafe-hashes', so an injected handler
#     with any other body is blocked.
#
# Net effect: arbitrary attacker-supplied script no longer runs, while the
# known handlers keep working. style-src keeps 'unsafe-inline' because inline
# style attributes are pervasive and style injection is a far lower risk; that
# remains the next step, not a claim of completion.
# --------------------------------------------------------------------------
_SCRIPT_OPEN_RE = re.compile(rb'<script(?![^>]*\bsrc=)([^>]*)>', re.I)
_INLINE_HANDLER_RE = re.compile(rb'\son[a-z]+\s*=\s*"([^"]*)"', re.I)
CSP_STRICT = os.environ.get('HR_CSP_STRICT', '1').strip() != '0'

def _csp_prepare(body):
    """Stamp inline scripts with a nonce and collect handler hashes."""
    nonce = base64.b64encode(secrets.token_bytes(16)).decode('ascii')
    stamped = _SCRIPT_OPEN_RE.sub(
        lambda m: b'<script nonce="' + nonce.encode() + b'"' + m.group(1) + b'>', body)
    hashes = set()
    for m in _INLINE_HANDLER_RE.finditer(stamped):
        raw = m.group(1)
        # The browser hashes the attribute value after HTML entity decoding.
        decoded = html_module.unescape(raw.decode('utf-8', 'replace')).encode('utf-8')
        hashes.add("'sha256-" + base64.b64encode(hashlib.sha256(decoded).digest()).decode('ascii') + "'")
    return stamped, nonce, sorted(hashes)

def build_csp(nonce=None, handler_hashes=()):
    if not CSP_STRICT or nonce is None:
        script_src = "script-src 'self' 'unsafe-inline'"
    else:
        parts = ["script-src 'self'", f"'nonce-{nonce}'"]
        if handler_hashes:
            parts.append("'unsafe-hashes'")
            parts.extend(handler_hashes)
        script_src = ' '.join(parts)
    return ("default-src 'self'; img-src 'self' data: blob:; "
            "style-src 'self' 'unsafe-inline'; " + script_src + "; "
            "object-src 'none'; base-uri 'self'; frame-ancestors 'self'; form-action 'self';")

def rehydrate_session(sid):
    """Rebuild an in-memory session from system_sessions after a restart.

    Returns None when the row is missing, revoked, idle past its window, or the
    user is no longer active — the cookie alone is never enough.
    """
    try:
        c=db()
        row=c.execute('SELECT * FROM system_sessions WHERE session_id=? AND revoked=0',(sid,)).fetchone()
        if not row:
            c.close(); return None
        cols=row.keys()
        try:
            idle=int(setting('session_idle_minutes') or 30)
            last=row['last_seen'] or row['created_at']
            if last and datetime.now()-datetime.fromisoformat(last)>timedelta(minutes=idle):
                c.execute('UPDATE system_sessions SET revoked=1 WHERE session_id=?',(sid,))
                c.commit(); c.close(); return None
        except (TypeError,ValueError):
            pass
        user=c.execute('SELECT * FROM users WHERE username=? AND active=1',(row['user_name'],)).fetchone()
        c.close()
        if not user:
            return None
        sess=dict(user)
        sess['_sid']=sid
        # Without the original token every open tab would fail its next POST
        # with a CSRF error, which is barely better than being logged out.
        sess['csrf']=(row['csrf'] if 'csrf' in cols and row['csrf'] else secrets.token_urlsafe(24))
        sess['last_seen']=now()
        sess['permission_version']=user['permission_version'] if 'permission_version' in user.keys() else 0
        SESS[sid]=sess
        hr_log('auth','session_rehydrated',level='INFO',username=row['user_name'])
        return sess
    except sqlite3.Error as e:
        hr_log('auth','session_rehydrate_failed',level='WARNING',exc=e)
        return None
...
class H(BaseHTTPRequestHandler):
    ...
    # ----------------------------------------------------------------------
    # HARDENING: bounded concurrency, socket timeouts, and a real 413.
    #
    # ThreadingHTTPServer spawns one thread per connection with no ceiling, so
    # a flood of slow connections could exhaust RAM/CPU and hold threads open
    # forever (no socket timeout was set anywhere). Both are now bounded.
    # ----------------------------------------------------------------------
    timeout = SOCKET_TIMEOUT_SECS

    def setup(self):
        try: self.request.settimeout(SOCKET_TIMEOUT_SECS)
        except Exception: pass
        return BaseHTTPRequestHandler.setup(self)

    def _drain_unread_body(self, cap=1024*1024):
        # A 413/503 is sent WITHOUT ever reading the client's body, but the
        # client (having already sent it, or being mid-send) is still
        # sitting on that socket. Closing our end immediately, while those
        # bytes are still unread in the kernel's receive buffer, makes many
        # TCP stacks send a hard RST instead of a graceful close — Windows
        # in particular does this eagerly. An RST can race ahead of (or
        # swallow) the response we just wrote, so the client never sees the
        # 413 body at all and just sits there until its own read timeout
        # fires. Draining a bounded amount first lets the normal FIN/close
        # sequence carry the response through. Bounded (and time-boxed) so a
        # spoofed huge Content-Length can't be used to hang this thread.
        try:
    ...
    def _send_status_only(self, status, message):
        try:
    ...
    def handle_one_request(self):
        """Gate every request behind a semaphore and translate size errors."""
        if not HTTP_SEMAPHORE.acquire(timeout=5):
            # Server is saturated: fail fast with 503 instead of queueing
            # another unbounded thread behind an already-overloaded SQLite.
            self.close_connection = True
            metric_inc('rejected_503')
            try:
                self.raw_requestline = self.rfile.readline(65537)
                if self.parse_request():
                    self._send_status_only(503, 'الخادم مشغول حاليًا. حاول مرة أخرى بعد قليل.')
            except Exception:
                pass
            return
        _t0 = time.time()
        try:
            ...
            self.close_connection = True
            metric_inc('rejected_413')
            try: log_error('request-too-large', e, path=getattr(self, "path", ""), ip=self.client_address[0])
            except Exception: pass
            self._send_status_only(413, 'حجم الطلب أكبر من الحد المسموح به.')
        except (socket.timeout, TimeoutError):
            self.close_connection = True
        finally:
            metric_latency(time.time() - _t0)
            try: HTTP_SEMAPHORE.release()
            except Exception: pass
    def user(self):
        # THE MAIN NAVIGATION-SLOWDOWN BUG: this used to open 2 fresh SQLite
        # connections (SELECT users + SELECT system_sessions) AND write
        # (UPDATE system_sessions ... COMMIT) to disk on every single request —
        # every page click, every employee photo <img>, everything — with zero
        # caching. On top of that, require() below used to run a THIRD,
        # fully duplicate "SELECT * FROM users" on every request. That's 3
        # SQLite connections + 1 disk write per click, and dozens more per page
        # when a page embeds several images/requests. Each sqlite3.connect()
        # also re-runs 4 PRAGMA statements, which is slow on Windows in
        # particular (antivirus, slower disk). That combination is what made
        # "just clicking the sidebar" feel like it hangs for a long time.
        #
        # Fix: re-validate a session against the DB at most once every
        # SESSION_REVALIDATE_SECS, and persist last_seen at most once every
        # SESSION_WRITE_SECS. A disabled account or revoked session still takes
        # effect within a few seconds — it is just no longer synchronous with
        # every click.
        ...
        u=SESS.get(sidv)
        if not u:
            # SESSION PERSISTENCE: SESS is in-process, so every restart — a
            # crash, an update, a reboot — logged every user out mid-task even
            # though system_sessions already held the row. Rebuild the session
            # from the database instead of discarding it. The user record is
            # re-read here, so a deactivated account or a changed role cannot
            # be resurrected by an old cookie.
            u=rehydrate_session(sidv)
            if not u: return None
        try:
            idle=int(setting('session_idle_minutes') or 30)
            if datetime.now()-datetime.fromisoformat(u.get('last_seen',now()))>timedelta(minutes=idle):
                SESS.pop(sidv,None)
                cc=db(); cc.execute('UPDATE system_sessions SET revoked=1 WHERE session_id=?',(sidv,)); cc.commit(); cc.close()
                return None
            now_t=time.time()
            if now_t-u.get('_checked_at',0)>=SESSION_REVALIDATE_SECS:
                cc=db(); row=cc.execute('SELECT * FROM users WHERE username=?',(u['username'],)).fetchone(); sessrow=cc.execute('SELECT revoked FROM system_sessions WHERE session_id=?',(sidv,)).fetchone(); cc.close()
                if not row or not row['active'] or not sessrow or sessrow['revoked']:
                    SESS.pop(sidv,None); self._auth_msg='تم تحديث صلاحيات الحساب أو إيقافه. سجّل الدخول مرة أخرى.'; return None
                if int(row['permission_version'] or 1)!=int(u.get('permission_version') or 1) or row['role']!=u.get('role') or row['scope_type']!=u.get('scope_type') or row['scope_value']!=u.get('scope_value') or int(row['must_change_password'] or 0)!=int(u.get('must_change_password') or 0):
                    csrf=u.get('csrf') or secrets.token_urlsafe(24)
                    u=dict(row); u['csrf']=csrf
                u['_checked_at']=now_t
            u['last_seen']=now(); u['_sid']=sidv
            SESS[sidv]=u
            if now_t-u.get('_last_write',0)>=SESSION_WRITE_SECS:
                cc=db(); cc.execute('UPDATE system_sessions SET last_seen=? WHERE session_id=? AND revoked=0',(u['last_seen'],sidv)); cc.commit(); cc.close()
                u['_last_write']=now_t
            return u
        except Exception:
            return None
    def send(self,body,status=200,ctype='text/html; charset=utf-8',headers=None):
        ...
        csp_nonce=None; csp_hashes=()
        if CSP_STRICT and ctype.startswith('text/html'):
            body,csp_nonce,csp_hashes=_csp_prepare(body)
        ...
        # camera=(self) — was fully blocked (camera=()) which silently killed any
        # in-page QR camera scanner regardless of what the page's JS did. The QR
        # scanner is same-origin only, so allow camera for this origin and keep
        # microphone/geolocation blocked.
        self.send_header('Permissions-Policy','camera=(self), microphone=(), geolocation=()')
        self.send_header('Content-Security-Policy',build_csp(csp_nonce,csp_hashes))
        if os.environ.get('HR_HTTPS','').strip()=='1' or setting('https_enabled')=='1':
    ...
    def _raw(self):
        ...
        #
        # HARDENING: the body is now size-checked BEFORE it is read, and read in
        # bounded chunks. Previously a single Content-Length header could make
        # the server allocate an arbitrary amount of RAM (a 2 GB POST was read
        # in full before any handler got a chance to reject it).
        if not hasattr(self,'_raw_body'):
            try: n=int(self.headers.get('Content-Length','0') or 0)
            except (TypeError,ValueError): n=0
            if n<0: n=0
            limit=max_body_for(urlparse(self.path).path)
            if n>limit:
                self._raw_body=b''; self._body_too_large=True
                raise RequestTooLarge(f'body {n} bytes exceeds limit {limit}')
            buf=bytearray(); remaining=n
            while remaining>0:
                chunk=self.rfile.read(min(READ_CHUNK,remaining))
                if not chunk: break
                buf.extend(chunk); remaining-=len(chunk)
            self._raw_body=bytes(buf)
    ...
    def require(self):
        # Used to run a full duplicate "SELECT * FROM users" (a whole extra
        # SQLite connection) on every single request just to re-confirm what
        # self.user() had already just confirmed. self.user() already performs
        # the live active/permission_version/role/scope re-check (throttled,
        # see SESSION_REVALIDATE_SECS above), so trust its result directly.
        self._auth_msg=None
        u=self.user()
        if not u:
            self.send(login_page(getattr(self,'_auth_msg','') or ''),401); return None
        return u
    def forbid(self,u):
    ...
    def bridge_authorize_token(self):
        try:
            ...
            info=BRIDGE_TOKENS.get(tok)
            c=db(); row=c.execute('SELECT used,expires_at,username,session_id FROM bridge_tokens WHERE token=?',(tok,)).fetchone(); c.close()
            ok=bool(info and row and not row['used'] and datetime.now()<datetime.fromisoformat(info['expires_at']) and datetime.now()<datetime.fromisoformat(row['expires_at']) and secrets.compare_digest(info['csrf'],csrf) and secrets.compare_digest(info['username'],row['username']) and secrets.compare_digest(info.get('session_id',''),row['session_id'] or ''))
            if not ok: return self.send(json.dumps({'ok':False,'error':'bridge token expired or invalid'}),403,'application/json')
    ...
    def bridge_upload_token(self):
        try:
            ...
            c=db(); er=c.execute('SELECT 1 FROM employees WHERE emp_code=?',(emp,)).fetchone(); usr=c.execute('SELECT * FROM users WHERE username=? AND active=1',(info['username'],)).fetchone(); sess=c.execute('SELECT revoked FROM system_sessions WHERE session_id=?',(info.get('session_id',''),)).fetchone(); c.close()
            if not er: return self.send(json.dumps({'ok':False,'error':'employee not found'}),404,'application/json')
            if not usr or not sess or sess['revoked'] or not emp_allowed(dict(usr),emp): return self.send(json.dumps({'ok':False,'error':'employee is outside your access scope'}),403,'application/json')
            imported=0
            ...
            c=db(); c.execute('UPDATE bridge_tokens SET used=1 WHERE token=? AND used=0',(token,)); c.commit(); c.close(); BRIDGE_TOKENS.pop(token,None)
...
<div class="full"><button class="btn">حفظ كلمة المرور</button></div></form></div>'''
    ...
    def password_save(self,u,f):
        ...
        if u['username']=='admin':
            try:
                _p=os.path.join(DATA,'INITIAL_ADMIN_PASSWORD.txt')
                if os.path.exists(_p): os.remove(_p)
            except Exception as e: log_error('delete-initial-admin-password',e)
...
<script>async function syncDevice(){{const d=prompt('Computer Name / Device Name',navigator.platform+'-'+location.hostname);try{{const r=await fetch('/device/ping?x='+Date.now(),{{headers:{{'X-HR-Device-Name':d||navigator.platform}}}});const j=await r.json();document.getElementById('deviceStatus').textContent='🟢 Connected · '+j.server+' · '+j.ip+':'+j.port+' · '+j.last_sync;setTimeout(()=>location.reload(),500);}}catch(e){{document.getElementById('deviceStatus').textContent='🔴 SERVER OFFLINE';}}}}</script>'''.format(server=esc(setting('server_name') or 'HR-MAIN'),ip=esc(local_ip()),port=PORT,rows=trs or '<tr><td colspan="6">لا توجد أجهزة مسجلة.</td></tr>')
    ...
    def do_GET(self):
        ...
        if path=='/static/style.css':
            return self.send(CSS,200,'text/css; charset=utf-8',{'Cache-Control':'public,max-age=31536000,immutable'})
        if path=='/login': return self.send(login_page())
        if path in ('/health','/health/live'):
            # LIVE: the process is up. Deliberately touches nothing else, so a
            # locked database can never make the liveness probe fail and trigger
            # a restart loop.
            return self.send(json.dumps({'ok':True,'status':'live','version':APP_VERSION,'server_name':setting('server_name') or 'HR-SERVER'},ensure_ascii=False),200,'application/json',{'Cache-Control':'no-store'})
        if path=='/health/ready':
            # READY: the database is reachable and answers a trivial query.
            try:
                c=db(); c.execute('SELECT 1').fetchone(); c.close()
                return self.send(json.dumps({'ok':True,'status':'ready','version':APP_VERSION},ensure_ascii=False),200,'application/json',{'Cache-Control':'no-store'})
            except Exception as e:
                return self.send(json.dumps({'ok':False,'status':'not-ready','error':str(e)},ensure_ascii=False),503,'application/json',{'Cache-Control':'no-store'})
        if path=='/health/deep':
            # DEEP: everything an operator needs to explain a slow/hanging UI
            # without guessing. Requires an authenticated session.
            u=self.require()
            if not u: return None
            report={'version':APP_VERSION,'uptime_seconds':round(time.time()-METRICS['started_at'],1),
                    'threads':threading.active_count(),'sessions_in_memory':len(SESS),
                    'http':{'max_workers':MAX_HTTP_WORKERS,'requests':METRICS.get('requests',0),
                            'rejected_503':METRICS.get('rejected_503',0),'rejected_413':METRICS.get('rejected_413',0),
                            'slow_requests':METRICS.get('slow_requests',0),'slow_threshold_secs':SLOW_REQUEST_SECS,
                            'latency':metric_percentiles()},
                    'sqlite':{'lock_waits':METRICS.get('db_lock_waits',0),
                              'lock_seconds_total':METRICS.get('db_lock_seconds',0.0)}}
            try:
                report['sqlite']['db_size_bytes']=os.path.getsize(DB) if os.path.exists(DB) else 0
                c=db(); report['sqlite']['journal_mode']=(c.execute('PRAGMA journal_mode').fetchone() or [''])[0]
                report['sqlite']['integrity']=(c.execute('PRAGMA quick_check').fetchone() or [''])[0]
                report['employees']=c.execute('SELECT COUNT(*) n FROM employees').fetchone()['n']
                c.close()
            except Exception as e:
                report['sqlite']['error']=str(e)
            try:
                usage=shutil.disk_usage(DATA); report['disk']={'free_bytes':usage.free,'total_bytes':usage.total}
            except Exception: pass
            try:
                c=db(); last=c.execute('SELECT created_at FROM system_backups ORDER BY id DESC LIMIT 1').fetchone(); c.close()
                report['last_backup']=last['created_at'] if last else None
                report['backups_encrypted']=backups_encrypted()
            except sqlite3.Error as _e:
                hr_log('db','silent_db_failure',level='WARNING',exc=_e,where='server.py:2670')
            report['ok']=report.get('sqlite',{}).get('integrity')=='ok'
            return self.send(json.dumps(report,ensure_ascii=False,indent=2),200,'application/json; charset=utf-8',{'Cache-Control':'no-store'})
        if path=='/branding/logo':
            logo=setting('company_logo')
            p=os.path.join(DATA,logo) if logo else ''
            if p and os.path.exists(p):
                raw=open(p,'rb').read(); self.send(raw,200,mimetypes.guess_type(p)[0] or 'image/png',{'Cache-Control':'public,max-age=3600'})
            else:
                self.send(b'',404,'image/png')
            return
        if path=='/branding/favicon':
            logo=setting('logo_favicon') or setting('company_logo')
            p=os.path.join(DATA,logo) if logo else ''
            if p and os.path.exists(p):
                raw=open(p,'rb').read(); self.send(raw,200,mimetypes.guess_type(p)[0] or 'image/png',{'Cache-Control':'public,max-age=3600'})
            else:
        ...
        if path=='/logout':
            ...
            if sidc:
                ...
                try:
                    ...
                    hr_log('auth','session_revoke_failed',exc=e,session_id=str(sidc.value)[:12])
        ...
        try:
            handler = GET_EXACT_ROUTES.get(path)
            if handler: return handler(self,u)
            for prefix,handler in GET_PREFIX_ROUTES:
                if path.startswith(prefix): return handler(self,u,path)
    ...
    def do_POST(self):
        ...
        if path=='/login':
            ...
            f=self.form(); uname=f.get('username','').strip()
            locked,wait,scope=auth_check_locked(ip,uname)
            if locked:
                auth_event(uname,ip,'LOGIN_BLOCKED',f'locked scope={scope}')
                # Deliberately identical wording for both scopes: revealing that
                # the *account* is locked would confirm the username exists.
                return self.send(login_page(f'محاولات دخول كثيرة. حاول مرة أخرى بعد {wait} ثانية.'))
            c=db(); r=c.execute('SELECT * FROM users WHERE username=? AND active=1',(uname,)).fetchone()
            # Always run a real hash comparison so an unknown username costs the
            # same wall-clock time as a known one (no timing enumeration).
            stored_hash = r['password_hash'] if r else _dummy_password_hash()
            password_ok = checkpw(f.get('password',''), stored_hash)
            if not r or not password_ok:
                c.close()
                auth_record_failure(ip,uname)
                auth_event(uname,ip,'LOGIN_FAILED','bad credentials')
                return self.send(login_page('بيانات الدخول غير صحيحة.'))
            auth_clear_failures(ip,uname)
            auth_event(uname,ip,'LOGIN_OK','')
            ...
            try:
                c2=db(); c2.execute('INSERT INTO system_sessions(csrf,session_id,user_name,full_name,role,created_at,last_seen,ip,device) VALUES(?,?,?,?,?,?,?,?,?)',(sess['csrf'],sid,r['username'],r['full_name'],r['role'],now(),now(),ip,self.headers.get('User-Agent','')[:180])); c2.commit(); c2.close()
            except sqlite3.Error as e:
                hr_log('auth','session_record_failed',exc=e,username=r['username'])
            # HARDENING: mark the session cookie Secure whenever the connection
            # can actually carry it. Setting Secure on a plain-HTTP deployment
            # is not "safer" — the browser simply never sends the cookie back,
            # so login silently breaks. Plaintext exposure is instead prevented
            # up front by serve_http(), which refuses to bind a non-loopback
            # interface without TLS.
            _tls_on = bool(os.environ.get('HR_TLS_CERT','').strip()) or \
                      os.environ.get('HR_HTTPS','').strip()=='1' or \
                      os.environ.get('HR_COOKIE_SECURE','').strip()=='1' or \
                      setting('https_enabled')=='1'
            cookie_secure='; Secure' if _tls_on else ''
            self.redirect('/',{'Set-Cookie':f'sid={sid}; Path=/; HttpOnly; SameSite=Lax{cookie_secure}'}); return
        u=self.require()
        if not u:return
        AUDIT_CTX.ip=self.client_address[0]; AUDIT_CTX.user=u
        ...
        try:
            ...
            handler = POST_EXACT_ROUTES.get(path)
            if handler: return handler(self,u,f)
            for prefix,handler in POST_PREFIX_ROUTES:
                if path.startswith(prefix): return handler(self,u,f,path.split('/')[-1])
        except Exception as e:
...
<div class="card" style="margin-top:16px"><h3>🔔 تنبيهات ذكية</h3>{''.join(f'<p style="padding:9px 0;border-bottom:1px solid #eee">{x}</p>' for x in alerts) or '<p><span class="badge b-ok">لا توجد تنبيهات حرجة الآن</span></p>'}</div>'''
    ...
    def employees(self,u):
        ...
        rows=c.execute(f'SELECT e.emp_code,e.name,e.national_id,e.job,e.department,e.unit,e.status,e.phone,e.hire_date,e.shift_id,(SELECT id FROM documents d WHERE d.emp_code=e.emp_code AND d.category="صورة" AND d.status="current" ORDER BY d.id DESC LIMIT 1) photo_id FROM employees e{where}{scope_sql} ORDER BY e.name COLLATE NOCASE LIMIT 2000',params+scope_params).fetchall()
        ...
        def photo_cell(r):
            if r["photo_id"]:
                return f'<img src="/employee/photo/{esc(r["emp_code"])}?v={r["photo_id"]}" loading="lazy" decoding="async" onerror="this.style.display=&quot;none&quot;" style="width:36px;height:42px;object-fit:cover;border-radius:8px;border:1px solid #e4e7ec;background:#f2f4f7">'
            return '<span style="width:36px;height:42px;border-radius:8px;background:#f2f4f7;border:1px solid #e4e7ec;display:inline-flex;align-items:center;justify-content:center;font-size:11px;color:#98a2b3">—</span>'
        trs=''.join(f'<tr><td><input class="emp-check" type="checkbox" value="{esc(r["emp_code"])}"></td><td>{esc(r["emp_code"])}</td><td><a href="/employee/profile/{esc(r["emp_code"])}" style="display:flex;align-items:center;gap:9px">{photo_cell(r)}<b>{esc(r["name"])}</b></a></td><td>{esc(r["national_id"]) if can(u,"sensitive.view") else "************"}</td><td>{esc(r["job"] or "—")}</td><td>{esc(r["department"] or "—")}</td><td>{esc(r["unit"] or "—")}</td><td><span class="badge {badge.get(r["status"],"b-warn")}">{esc(r["status"])}</span></td><td>{esc(r["phone"] or "—")}</td><td>{esc(r["hire_date"] or "—")}</td><td><a class="btn gray" href="/employee/profile/{esc(r["emp_code"])}">فتح</a></td></tr>' for r in rows)
    ...
    def employee_form(self,u,code=None):
        ...
        is_edit=bool(r)
        def field(k,label,typ='text',req=False,full=False):
            extra=' step="0.01"' if typ=='number' else ''
            rq=' required' if req else ''
            dis=' readonly' if (k=='emp_code' and is_edit) else ''
            cls='field full' if full else 'field'
            return f'<div class="{cls}"><label>{label}{" *" if req else ""}</label><input type="{typ}" name="{k}" value="{v(k)}"{extra}{rq}{dis}></div>'
        status=v('status') or 'على رأس العمل'
        status_html=f'<div class="field"><label>الحالة</label><select name="status"><option {"selected" if status=="على رأس العمل" else ""}>على رأس العمل</option><option {"selected" if status=="موقوف" else ""}>موقوف</option><option {"selected" if status=="منتهي الخدمة" else ""}>منتهي الخدمة</option><option {"selected" if status=="مؤرشف" else ""}>مؤرشف</option></select></div>'
        ...
        shift_html='<div class="field"><label>الوردية</label><select name="shift_id"><option value="">استخدام الوردية الافتراضية</option>'+''.join(f'<option value="{x["id"]}" {"selected" if str(current_shift)==str(x["id"]) else ""}>{esc(x["name"])} ({esc(x["start_time"])} → {esc(x["end_time"])})</option>' for x in shifts)+'</select></div>'

        step1=field('emp_code','م / كود الموظف',req=True)+field('name','الإسم',req=True)+field('national_id','الرقم القومى')+field('birth_date','تاريخ الميلاد','date')+field('phone','رقم التيلفون')+field('address','العنوان',full=True)
        step2=field('employee_group','المجموعه الوظفيه')+field('job','الوظيفه')+field('department','الإدارة')+field('unit','الوحدة')+field('qualification','المؤهل')+status_html+'<div class="full">'+shift_html+'</div>'
        step3=field('contract_date','تاريخ التعاقد','date')+field('contract_amount','مبلغ التعاقد','number')+field('bank_name','إسم البنك')+field('bank_branch','إسم فرع البنك')+field('iban','IBAN')+'<div class="wz-hint full">راجع البيانات المالية قبل الحفظ — هذه البيانات تُستخدم في مسير الرواتب.</div>'

        steps=[('1','بيانات أساسية','الهوية والتواصل',step1),('2','بيانات وظيفية','القسم والوردية',step2),('3','تعاقد ومالية','الراتب والبنك',step3)]
        track=''.join(f'<div class="wz-step{" active" if i==0 else ""}" data-step="{i}"><div class="dot">{n}</div><div class="lbl">{lbl}</div><div class="sub">{sub}</div></div>' for i,(n,lbl,sub,_) in enumerate(steps))
        panels=''.join(f'<div class="wz-panel{" active" if i==0 else ""}" data-panel="{i}">{content}</div>' for i,(_,_,_,content) in enumerate(steps))
        nsteps=len(steps)
        js='''<script>
(function(){
var cur=0,total=%d,steps=document.querySelectorAll('.wz-step'),panels=document.querySelectorAll('.wz-panel'),form=document.getElementById('wzForm');
function show(i){
  panels.forEach(function(p,idx){p.classList.toggle('active',idx===i)});
  steps.forEach(function(s,idx){s.classList.toggle('active',idx===i);s.classList.toggle('done',idx<i)});
  cur=i;
  document.getElementById('wzPrev').style.visibility=(i===0?'hidden':'visible');
  document.getElementById('wzNext').style.display=(i===total-1?'none':'inline-flex');
  document.getElementById('wzSubmit').style.display=(i===total-1?'inline-flex':'none');
  window.scrollTo({top:document.querySelector('.wizard').offsetTop-20,behavior:'smooth'});
}
function validPanel(i){
  var ok=true,fields=panels[i].querySelectorAll('input[required],select[required]');
  fields.forEach(function(f){
    var box=f.closest('.field');
    if(!f.value.trim()){ok=false; if(box)box.classList.add('invalid');}
    else{ if(box)box.classList.remove('invalid');}
  });
  return ok;
}
document.getElementById('wzNext').addEventListener('click',function(){
  if(!validPanel(cur)){return;}
  if(cur<total-1) show(cur+1);
});
document.getElementById('wzPrev').addEventListener('click',function(){ if(cur>0) show(cur-1); });
steps.forEach(function(s,idx){ s.addEventListener('click',function(){ if(idx<=cur || idx===cur+1) { if(idx>cur && !validPanel(cur)) return; show(idx);} }); });
form.addEventListener('submit',function(e){ if(!validPanel(total-1)){ e.preventDefault(); } });
show(0);
})();
</script>'''%nsteps
        body=f'''<div class="top"><div class="title"><h1>{'تعديل موظف' if r else 'إضافة موظف'}</h1><p>أدخل البيانات على {nsteps} خطوات — كل خطوة تُحفظ فورًا عند الضغط على "حفظ البيانات"</p></div><a class="btn gray" href="/employees">عودة</a></div>
<div class="card wizard">
<div class="wz-track">{track}</div>
<form class="form" id="wzForm" method="post" action="/employee/save" novalidate>{csrf_field(u)}
{panels}
<div class="wz-actions"><button type="button" class="btn gray" id="wzPrev">السابق</button><div class="spacer"></div><button type="button" class="btn" id="wzNext">التالي</button><button type="submit" class="btn ok" id="wzSubmit" style="display:none">✓ حفظ البيانات</button></div>
</form>
</div>
{js}'''
    ...
    def employee_profile(self,u,code):
        ...
        status_cls = 'b-gray' if archived else ('b-ok' if e['status']=='نشط' else 'b-warn')
        # Hero header: employee photo + name/status/actions + QR identity, all in
        # one block at the very top of the page (before any tabs/content). The
        # photo and QR slots below are filled in by enterprise_completion.py's
        # profile wrapper (that's where the photo/QR data and upload/QR forms
        # live) — this keeps the base layout in one place while letting that
        # module inject the actual image/actions into the marked slots.
        body=f'''{arch_banner}<div class="card hero-card profile-hero" id="profileHero">
          <div class="ph-photo-wrap" id="heroPhotoSlot"><div class="ph-photo-empty">👤</div></div>
          <div class="ph-info">
            <h1>{esc(e['name'])}</h1>
            <div class="ph-meta">{esc(e['emp_code'])} · {esc(e['job'] or '—')} · {esc(e['department'] or e['unit'] or '—')}</div>
            <div class="ph-status"><span class="badge {status_cls}">{esc(e['status'])}</span></div>
            <div class="ph-actions">{navlinks}<a class="btn" href="/employee/edit/{esc(code)}">تعديل</a>{arch_btn}<a class="btn gray" href="/employees">عودة</a><button class="btn gray" onclick="window.print()">طباعة الملف</button></div>
          </div>
          <div class="ph-qr" id="heroQrSlot"></div>
        </div>
        <div class="card no-print" style="position:sticky;top:8px;z-index:5;margin-top:16px"><div class="actions">{nav}</div></div>
...
<div class="table-wrap" style="margin-top:14px"><h4>تاريخ التقييمات السابقة</h4><table class="table"><thead><tr><th>الفترة</th><th>النتيجة</th><th>Attendance</th><th>Punctuality</th><th>Productivity</th><th>Behavior</th><th>Manager</th><th>بواسطة</th></tr></thead><tbody>{''.join(f'<tr><td>{esc(r["period"])}</td><td><b>{float(r["score"] or 0):g}%</b></td><td>{r["attendance_score"]}%</td><td>{r["punctuality_score"]}%</td><td>{r["productivity_score"]}%</td><td>{r["behavior_score"]}%</td><td>{r["manager_score"]}%</td><td>{esc(r["created_by"])}</td></tr>' for r in evals) or '<tr><td colspan="8">لا توجد تقييمات سابقة</td></tr>'}</tbody></table></div></div>
    ...
    def save_employee(self,u,f):
        ...
        # Scope is enforced on both the current employee and the submitted target scope.
        # This prevents a scoped manager from editing an out-of-scope employee or moving
        # an in-scope employee into another department/unit by crafting a direct POST.
        if old and not emp_allowed(u,code): c.close(); return self.forbid(u)
        if u.get('role') not in ('SuperAdmin','Admin','HR'):
            st=u.get('scope_type',''); sv=u.get('scope_value','')
            if u.get('role')=='Employee' or st=='self': c.close(); return self.forbid(u)
            if st=='department' and (f.get('department') or '').strip()!=sv: c.close(); return self.forbid(u)
            if st=='unit' and (f.get('unit') or '').strip()!=sv: c.close(); return self.forbid(u)
    ...
    def save_leave(self,u,f):
        ...
        if not emp_allowed(u,emp): c.close(); return self.forbid(u)
            ...
            for folder,items in groups.items():
                ...
                if not emp or not emp_allowed(u,emp):
                    ...
                    try:
                        ...
                        hr_log('db','silent_db_failure',level='WARNING',exc=_e,where='server.py:3346')
                    review.append((folder,'لا يوجد تطابق آمن')); continue
                matched+=1
                for relname,data in items:
                    ext=os.path.splitext(relname)[1].lower()
                    if ext not in allowed or len(data)>max_mb*1024*1024 or not validate_file_signature(os.path.basename(relname),data): invalid+=1; continue
...
<div class="field"><label>رابط الخادم للشبكة / QR</label><input name="server_url" placeholder="مثال: http://192.168.1.10:8899" value="{esc(ss.get('server_url',''))}"></div>
<div class="field"><label>المظهر</label><select name="theme"><option value="light" {'selected' if ss.get('theme','light')=='light' else ''}>Light</option><option value="dark" {'selected' if ss.get('theme')=='dark' else ''}>Dark</option></select></div>
<div class="field"><label>لون النظام</label><select name="accent_color"><option value="blue" {'selected' if ss.get('accent_color','blue')=='blue' else ''}>Blue</option><option value="purple" {'selected' if ss.get('accent_color')=='purple' else ''}>Purple</option><option value="emerald" {'selected' if ss.get('accent_color')=='emerald' else ''}>Emerald</option><option value="orange" {'selected' if ss.get('accent_color')=='orange' else ''}>Orange</option></select></div>
...
<div class="full"><button class="btn">حفظ سياسة الدوام</button></div></form></div>'''
    ...
    def branding_logo_save(self,u):
        ...
        # Never delete QR tokens/images, ID-card templates, or branding profiles when
        # the administrator replaces the company logo. Keep each asset class isolated.
        for old in os.listdir(brand):
            if old.startswith(('logo.','logo_','favicon.','login.','sidebar.','report.','transparent.')):
                try:
                    op=os.path.join(brand,old)
                    if os.path.isfile(op): os.remove(op)
                except Exception: pass
        fn='logo'+ext; open(os.path.join(brand,fn),'wb').write(data)
        c=db(); c.execute('INSERT INTO settings(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value',('company_logo',os.path.relpath(os.path.join(brand,fn),DATA))); c.commit(); c.close(); _SETTINGS_CACHE.clear()
        ...
        try:
            ...
            # Shrink the master file itself — /branding/logo is served with no-cache
            # and rendered as a plain (non-lazy) <img> in the sidebar on every single
            # page load, so an unresized upload (up to 5MB) was being re-sent in full
            # on every click, unlike employee photos which already get this treatment.
            im.thumbnail((800,800))
            if ext in ('.jpg','.jpeg'): im.convert('RGB').save(os.path.join(brand,fn),'JPEG',quality=85,optimize=True)
            else: im.save(os.path.join(brand,fn),'PNG',optimize=True)
            ...
            c.commit(); c.close(); _SETTINGS_CACHE.clear()
        except Exception:
            pass
    ...
    def save_settings(self,u,f):
        c=db()
        for k in ['company_name','currency','server_url','theme','accent_color','work_start','work_end','grace_minutes','document_max_mb','late_alert_threshold','monthly_late_limit_minutes','monthly_late_action','monthly_deduction_amount','required_doc_categories','eval_weight_attendance','eval_weight_punctuality','eval_weight_productivity','eval_weight_behavior','eval_weight_manager']:
            c.execute('INSERT INTO settings(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value',(k,f.get(k,'')))
        c.commit();c.close();_SETTINGS_CACHE.clear();audit(u['username'],u['role'],'تعديل','الإعدادات','global');self.redirect('/settings')
    ...
    def document_upload(self,u):
        ...
        if not validate_file_signature(fname,fdata): return self.send(page('خطأ','<div class="card"><div class="alert">محتوى الملف لا يطابق امتداده أو الملف تالف.</div></div>',u,'documents'),400)
    ...
    def roles_save(self,u,f):
        ...
        c.commit(); c.close(); invalidate_role_perms(role); audit(u['username'],u['role'],'تعديل الصلاحيات','الأدوار',role,','.join(perms)); self.redirect('/roles')
    ...
    def discipline_save(self,u,f):
        if not emp_allowed(u,f.get('emp_code')): return self.forbid(u)
    ...
    # ---------------- Supplies / Consumables (المستلمات) ----------------
    def _supplies_depts(self):
        c=db(); depts=[x[0] for x in c.execute("SELECT DISTINCT department FROM employees WHERE department IS NOT NULL AND department<>'' ORDER BY department").fetchall()]; c.close(); return depts

    def _supplies_emp_opts(self,u,selected=''):
        c=db(); emps=c.execute('SELECT emp_code,name,department FROM employees ORDER BY name').fetchall(); c.close()
        emps=[e for e in emps if emp_allowed(u,e['emp_code'])]
        opts=''.join(f'<option value="{esc(e["emp_code"])}" data-dept="{esc(e["department"] or "")}" {"selected" if e["emp_code"]==selected else ""}>{esc(e["emp_code"])} — {esc(e["name"])}</option>' for e in emps)
        return '<option value="">— اختر الموظف —</option>'+opts

    def supplies_dashboard(self,u):
        month=date.today().strftime('%Y-%m'); c=db()
        total_qty=c.execute("SELECT COALESCE(SUM(quantity),0) n FROM supplies_issues WHERE issued_at LIKE ?",(month+'%',)).fetchone()['n']
        total_ops=c.execute("SELECT COUNT(*) n FROM supplies_issues WHERE issued_at LIKE ?",(month+'%',)).fetchone()['n']
        by_dept=c.execute("SELECT department,COALESCE(SUM(quantity),0) qty FROM supplies_issues WHERE issued_at LIKE ? GROUP BY department ORDER BY qty DESC",(month+'%',)).fetchall()
        quotas={r['department']:r['monthly_quota'] for r in c.execute('SELECT department,monthly_quota FROM supplies_quotas').fetchall()}
        top_items=c.execute("SELECT item_name,COALESCE(SUM(quantity),0) qty FROM supplies_issues WHERE issued_at LIKE ? GROUP BY item_name ORDER BY qty DESC LIMIT 6",(month+'%',)).fetchall()
        recent=c.execute("SELECT * FROM supplies_issues ORDER BY id DESC LIMIT 10").fetchall(); c.close()
        def bar(dept,qty):
            q=quotas.get(dept) or 0; pct=min(100,round((qty/q)*100)) if q else 0
            cls='b-red' if q and qty>q else ('b-orange' if pct>=80 else 'b-blue')
            quota_txt=f'من كوتة {int(q) if q==int(q) else q}' if q else 'بدون كوتة محددة'
            return f'<div class="field full"><div style="display:flex;justify-content:space-between"><b>{esc(dept or "—")}</b><span>{qty:g} {quota_txt}</span></div><div style="background:#eee;border-radius:8px;height:10px;overflow:hidden;margin-top:4px"><div style="height:100%;width:{pct}%;background:var(--accent,#2563eb)" class="{cls}"></div></div></div>'
        dept_bars=''.join(bar(r['department'],r['qty']) for r in by_dept) or '<p>لا توجد استلامات هذا الشهر بعد.</p>'
        items_html=''.join(f'<tr><td>{esc(r["item_name"])}</td><td>{r["qty"]:g}</td></tr>' for r in top_items) or '<tr><td colspan="2">لا يوجد</td></tr>'
        recent_html=''.join(f'<tr><td>{esc(r["issued_at"][:16])}</td><td>{esc(r["emp_name"] or r["emp_code"])}</td><td>{esc(r["department"] or "")}</td><td>{esc(r["item_name"])}</td><td>{r["quantity"]:g}</td><td><a href="/supplies/receipt/{esc(r["receipt_no"])}" target="_blank">إيصال</a></td></tr>' for r in recent) or '<tr><td colspan="6">لا توجد عمليات بعد.</td></tr>'
        body=f'''<div class="top"><div class="title"><h1>📦 لوحة المستلمات</h1><p>إحصائيات شهر {esc(month)} — إجمالي {total_qty:g} صنف في {total_ops} عملية استلام.</p></div><div class="actions">{'<a class="btn" href="/supplies/issue">🧾 تسجيل استلام جديد</a>' if can(u,'supplies.issue') else ''}<a class="btn gray" href="/supplies/log">📜 السجل الكامل</a></div></div>
<div class="grid g2"><div class="card"><h3>الاستهلاك حسب القسم (مقابل الكوتة الشهرية)</h3>{dept_bars}</div><div class="card table-wrap"><h3>أكثر الأصناف طلبًا هذا الشهر</h3><table class="table"><thead><tr><th>الصنف</th><th>الكمية</th></tr></thead><tbody>{items_html}</tbody></table></div></div>
<div class="card table-wrap" style="margin-top:16px"><h3>آخر العمليات</h3><table class="table"><thead><tr><th>الوقت</th><th>الموظف</th><th>القسم</th><th>الصنف</th><th>الكمية</th><th>إيصال</th></tr></thead><tbody>{recent_html}</tbody></table></div>'''
        self.send(page('لوحة المستلمات',body,u,'supplies'))

    def supplies_issue_page(self,u):
        c=db(); items=c.execute('SELECT name,unit FROM supplies_items WHERE active=1 ORDER BY name').fetchall(); c.close()
        dl=''.join(f'<option value="{esc(i["name"])}">' for i in items)
        emp_opts=self._supplies_emp_opts(u)
        body=f'''<div class="top"><div class="title"><h1>🧾 تسجيل استلام جديد</h1><p>يتم توليد رقم إيصال تلقائي، وتقدر تطبعه مباشرة على A4 أو A5.</p></div></div>
<div class="card" style="max-width:640px"><form class="form" method="post" action="/supplies/issue/save">{csrf_field(u)}
<div class="field full"><label>الموظف</label><select name="emp_code" id="sup_emp" required onchange="document.getElementById('sup_dept').value=this.options[this.selectedIndex].dataset.dept||''">{emp_opts}</select></div>
<div class="field"><label>القسم (تلقائي)</label><input id="sup_dept" name="department" readonly></div>
<div class="field"><label>الكمية</label><input type="number" step="0.01" name="quantity" value="1" required></div>
<div class="field full"><label>الصنف</label><input name="item_name" list="items_dl" placeholder="اكتب اسم الصنف أو اختر من القائمة" required><datalist id="items_dl">{dl}</datalist></div>
<div class="field"><label>مقاس الورق (لو ينطبق)</label><select name="paper_size"><option value="">غير محدد</option><option>A4</option><option>A5</option></select></div>
<div class="field full"><label>ملاحظات</label><textarea name="notes"></textarea></div>
<div class="full"><button class="btn">حفظ وطباعة الإيصال</button></div></form></div>'''
        self.send(page('تسجيل استلام',body,u,'supplies-issue'))

    def supplies_issue_save(self,u,f):
        emp=f.get('emp_code','').strip()
        if not emp or not emp_allowed(u,emp): return self.forbid(u)
        c=db(); er=c.execute('SELECT name,department FROM employees WHERE emp_code=?',(emp,)).fetchone()
        if not er: c.close(); return self.send(page('خطأ','<div class="card"><div class="alert">الموظف غير موجود.</div></div>',u,'supplies-issue'),400)
        qty=cell_num(f.get('quantity')) or 1; item=f.get('item_name','').strip() or 'غير محدد'
        receipt_no='SUP-'+datetime.now().strftime('%Y%m%d%H%M%S')+'-'+secrets.token_hex(2).upper()
        c.execute('INSERT INTO supplies_issues(receipt_no,emp_code,emp_name,department,item_name,quantity,paper_size,notes,issued_by,issued_at) VALUES(?,?,?,?,?,?,?,?,?,?)',(receipt_no,emp,er['name'],f.get('department') or er['department'],item,qty,f.get('paper_size',''),f.get('notes',''),u['username'],now())); c.commit(); c.close()
        audit(u['username'],u['role'],'تسجيل استلام','المستلمات',emp,f'{item} × {qty:g}')
        self.redirect('/supplies/receipt/'+receipt_no)

    def supplies_receipt(self,u,receipt_no):
        c=db(); r=c.execute('SELECT * FROM supplies_issues WHERE receipt_no=?',(receipt_no,)).fetchone(); c.close()
        if not r or not emp_allowed(u,r['emp_code']): return self.send(page('غير موجود','<div class="card">الإيصال غير موجود.</div>',u),404)
        company=esc(setting('company_name') or 'HR Enterprise'); size=r['paper_size'] or 'A4'
        html=f'''<!doctype html><html lang="ar"><head><meta charset="utf-8"><title>إيصال استلام {esc(receipt_no)}</title>
<style>@page{{size:{size};margin:14mm}} body{{font-family:Tahoma,Arial;direction:rtl;padding:20px}} .box{{border:1px solid #333;border-radius:8px;padding:20px}} table{{width:100%;border-collapse:collapse;margin-top:14px}} td,th{{border:1px solid #ccc;padding:8px;text-align:right}} .sig{{display:flex;justify-content:space-between;margin-top:50px}} .noprint{{margin-bottom:14px}} @media print{{.noprint{{display:none}}}}</style></head>
<body><div class="noprint"><button onclick="window.print()">🖨 طباعة</button> <a href="/supplies">رجوع للوحة المستلمات</a></div>
<div class="box"><h2 style="text-align:center">{company}</h2><h3 style="text-align:center">إيصال استلام مستلمات</h3>
<table><tr><th>رقم الإيصال</th><td>{esc(r["receipt_no"])}</td><th>التاريخ</th><td>{esc(r["issued_at"][:16])}</td></tr>
<tr><th>الموظف</th><td>{esc(r["emp_name"] or r["emp_code"])}</td><th>القسم</th><td>{esc(r["department"] or "")}</td></tr>
<tr><th>الصنف</th><td>{esc(r["item_name"])}</td><th>الكمية</th><td>{r["quantity"]:g}</td></tr>
<tr><th>ملاحظات</th><td colspan="3">{esc(r["notes"] or "")}</td></tr></table>
<div class="sig"><div>توقيع المستلم: ....................</div><div>صرف بواسطة: {esc(r["issued_by"])}</div></div></div>
<script>window.onload=function(){{setTimeout(function(){{window.print()}},300)}}</script></body></html>'''
        self.send(html.encode('utf-8'),200,'text/html; charset=utf-8')

    def supplies_log_page(self,u):
        qs=parse_qs(urlparse(self.path).query); dept=qs.get('department',[''])[0]; emp=qs.get('emp_code',[''])[0]
        sql='SELECT * FROM supplies_issues WHERE 1=1'; params=[]
        if dept: sql+=' AND department=?'; params.append(dept)
        if emp: sql+=' AND emp_code=?'; params.append(emp)
        sql+=' ORDER BY id DESC LIMIT 500'
        c=db(); rows=c.execute(sql,params).fetchall(); c.close()
        rows=[r for r in rows if emp_allowed(u,r['emp_code'])]
        depts=self._supplies_depts()
        dopts=''.join(f'<option value="{esc(d)}" {"selected" if d==dept else ""}>{esc(d)}</option>' for d in depts)
        trs=''.join(f'<tr><td>{esc(r["issued_at"][:16])}</td><td>{esc(r["receipt_no"])}</td><td>{esc(r["emp_name"] or r["emp_code"])}</td><td>{esc(r["department"] or "")}</td><td>{esc(r["item_name"])}</td><td>{r["quantity"]:g}</td><td>{esc(r["paper_size"] or "")}</td><td>{esc(r["issued_by"])}</td><td><a href="/supplies/receipt/{esc(r["receipt_no"])}" target="_blank">إيصال</a></td></tr>' for r in rows)
        body=f'''<div class="top"><div class="title"><h1>📜 سجل المستلمات</h1><p>{len(rows)} عملية.</p></div><div class="actions"><a class="btn gray" href="/supplies/log/export.csv">⬇️ تصدير CSV</a></div></div>
<div class="card"><form class="form" method="get" action="/supplies/log"><div class="field"><label>القسم</label><select name="department"><option value="">الكل</option>{dopts}</select></div><div class="field"><label>كود الموظف</label><input name="emp_code" value="{esc(emp)}"></div><div class="full"><button class="btn">فلترة</button></div></form></div>
<div class="card table-wrap" style="margin-top:16px"><table class="table"><thead><tr><th>الوقت</th><th>رقم الإيصال</th><th>الموظف</th><th>القسم</th><th>الصنف</th><th>الكمية</th><th>المقاس</th><th>بواسطة</th><th></th></tr></thead><tbody>{trs or '<tr><td colspan="9">لا توجد نتائج.</td></tr>'}</tbody></table></div>'''
        self.send(page('سجل المستلمات',body,u,'supplies-log'))

    def supplies_export_csv(self,u):
        qs=parse_qs(urlparse(self.path).query); dept=qs.get('department',[''])[0]
        sql='SELECT * FROM supplies_issues WHERE 1=1'; params=[]
        if dept: sql+=' AND department=?'; params.append(dept)
        sql+=' ORDER BY id DESC LIMIT 5000'
        c=db(); rows=c.execute(sql,params).fetchall(); c.close(); rows=[r for r in rows if emp_allowed(u,r['emp_code'])]
        out=io.StringIO(newline=''); w=csv.writer(out); w.writerow(['رقم الإيصال','التاريخ','كود الموظف','الموظف','القسم','الصنف','الكمية','المقاس','بواسطة','ملاحظات'])
        for r in rows: w.writerow([r['receipt_no'],r['issued_at'],r['emp_code'],r['emp_name'],r['department'],r['item_name'],r['quantity'],r['paper_size'],r['issued_by'],r['notes']])
        raw=('\ufeff'+out.getvalue()).encode('utf-8'); self.send(raw,200,'text/csv; charset=utf-8',{'Content-Disposition':'attachment; filename="supplies_log.csv"'})

    def supplies_catalog_page(self,u):
        c=db(); items=c.execute('SELECT * FROM supplies_items ORDER BY active DESC,name').fetchall(); c.close()
        trs=''.join(f'<tr><td>{esc(r["name"])}</td><td>{esc(r["unit"])}</td><td>{esc(r["notes"] or "")}</td><td>{"✅ نشط" if r["active"] else "🚫 موقوف"}</td><td><form method="post" action="/supplies/catalog/toggle" style="display:inline">{csrf_field(u)}<input type="hidden" name="id" value="{r["id"]}"><button class="btn gray">{"إيقاف" if r["active"] else "تفعيل"}</button></form></td></tr>' for r in items)
        body=f'''<div class="top"><div class="title"><h1>🗂 كتالوج أصناف المستلمات</h1><p>الأصناف النشطة تظهر في قائمة الاختيار عند تسجيل استلام جديد.</p></div></div>
<div class="grid g2"><div class="card"><h3>إضافة / تعديل صنف</h3><form class="form" method="post" action="/supplies/catalog/save">{csrf_field(u)}<div class="field full"><label>اسم الصنف</label><input name="name" required></div><div class="field"><label>الوحدة</label><input name="unit" value="قطعة"></div><div class="field full"><label>ملاحظات</label><input name="notes"></div><div class="full"><button class="btn">حفظ</button></div></form></div>
<div class="card table-wrap"><h3>الأصناف الحالية</h3><table class="table"><thead><tr><th>الاسم</th><th>الوحدة</th><th>ملاحظات</th><th>الحالة</th><th></th></tr></thead><tbody>{trs or '<tr><td colspan="5">لا توجد أصناف بعد.</td></tr>'}</tbody></table></div></div>'''
        self.send(page('كتالوج المستلمات',body,u,'supplies-catalog'))

    def supplies_catalog_save(self,u,f):
        name=f.get('name','').strip()
        if not name: return self.redirect('/supplies/catalog')
        c=db()
        c.execute('INSERT INTO supplies_items(name,unit,notes,active,created_by,created_at) VALUES(?,?,?,1,?,?) ON CONFLICT(name) DO UPDATE SET unit=excluded.unit,notes=excluded.notes,active=1',(name,f.get('unit') or 'قطعة',f.get('notes',''),u['username'],now()))
        c.commit(); c.close(); audit(u['username'],u['role'],'إضافة/تعديل صنف','كتالوج المستلمات',name,''); self.redirect('/supplies/catalog')

    def supplies_catalog_toggle(self,u,f):
        try: iid=int(f.get('id'))
        except Exception: return self.redirect('/supplies/catalog')
        c=db(); c.execute('UPDATE supplies_items SET active=1-active WHERE id=?',(iid,)); c.commit(); c.close()
        audit(u['username'],u['role'],'تفعيل/إيقاف صنف','كتالوج المستلمات',str(iid),''); self.redirect('/supplies/catalog')

    def supplies_quotas_page(self,u):
        month=date.today().strftime('%Y-%m'); depts=self._supplies_depts()
        c=db(); quotas={r['department']:r['monthly_quota'] for r in c.execute('SELECT department,monthly_quota FROM supplies_quotas').fetchall()}
        used={r['department']:r['qty'] for r in c.execute("SELECT department,COALESCE(SUM(quantity),0) qty FROM supplies_issues WHERE issued_at LIKE ? GROUP BY department",(month+'%',)).fetchall()}
        c.close()
        rows=''.join(f'''<tr><td>{esc(d)}</td><td>{used.get(d,0):g}</td><td><form method="post" action="/supplies/quotas/save" style="display:flex;gap:6px;align-items:center">{csrf_field(u)}<input type="hidden" name="department" value="{esc(d)}"><input type="number" step="0.01" name="monthly_quota" value="{quotas.get(d,0):g}" style="width:120px"><button class="btn">حفظ</button></form></td></tr>''' for d in depts)
        body=f'''<div class="top"><div class="title"><h1>🎚 كوتة الأقسام الشهرية</h1><p>حدد الحد الأقصى شهريًا لكل قسم. المستخدم في شهر {esc(month)} معروض للمقارنة.</p></div></div>
<div class="card table-wrap"><table class="table"><thead><tr><th>القسم</th><th>المستخدم هذا الشهر</th><th>الكوتة الشهرية</th></tr></thead><tbody>{rows or '<tr><td colspan="3">لا توجد أقسام.</td></tr>'}</tbody></table></div>'''
        self.send(page('كوتة الأقسام',body,u,'supplies-quotas'))

    def supplies_quotas_save(self,u,f):
        dept=f.get('department','').strip()
        if not dept: return self.redirect('/supplies/quotas')
        c=db(); c.execute('INSERT INTO supplies_quotas(department,monthly_quota,updated_by,updated_at) VALUES(?,?,?,?) ON CONFLICT(department) DO UPDATE SET monthly_quota=excluded.monthly_quota,updated_by=excluded.updated_by,updated_at=excluded.updated_at',(dept,cell_num(f.get('monthly_quota')) or 0,u['username'],now())); c.commit(); c.close()
        audit(u['username'],u['role'],'تعديل كوتة','المستلمات',dept,f.get('monthly_quota','')); self.redirect('/supplies/quotas')
    ...
    def payroll_save(self,u,f):
        ...
        # A SuperAdmin may override a locked period by design, but an override
        # that is indistinguishable from an ordinary save defeats the purpose of
        # locking: the audit trail would show 'حفظ' and nothing else. Record it
        # explicitly so a reviewer can find every edit made after a lock.
        lock_override = bool(old and old['locked_at'])
        if lock_override:
            hr_log('app','payroll_lock_override',level='WARNING',
                   username=u['username'],role=u.get('role'),emp_code=emp,period=period,
                   locked_at=old['locked_at'],previous_net=old['net'])
    ...
    def payroll_lock(self,u,f):
        c=db(); r=c.execute('SELECT * FROM payroll WHERE id=?',(f.get('id'),)).fetchone()
        if not r: c.close(); return self.redirect('/payroll/review')
        if not emp_allowed(u,r['emp_code']): c.close(); return self.forbid(u)
        if r['status']!='معتمدة': c.close(); return self.redirect('/payroll/review')
    ...
    def backup_restore(self,u,f):
        ...
        try:
            if source.lower().endswith('.zip'):
                ...
                plain=source; decrypted=None
                if is_encrypted_backup(source):
                    decrypted=os.path.join(DATA,'_restore_plain.tmp')
                    decrypt_backup_file(source,decrypted); plain=decrypted
                try:
                    # safe_extract_zip validates every member (absolute paths,
                    # traversal, symlinks, entry count, total size, compression
                    # ratio) before a single byte is written to disk.
                    with zipfile.ZipFile(plain,'r') as z: safe_extract_zip(z,tmp)
                finally:
                    if decrypted and os.path.exists(decrypted):
                        try: os.remove(decrypted)
                        except Exception: pass
                dbsrc=os.path.join(tmp,'database.db')
                if not os.path.exists(dbsrc): raise ValueError('النسخة لا تحتوي database.db')
                # Prove the restored database actually opens before replacing the
                # live one; a corrupt restore used to overwrite production first
                # and fail afterwards.
                _probe=sqlite3.connect(dbsrc)
                try:
                    if (_probe.execute('PRAGMA integrity_check').fetchone() or [''])[0]!='ok':
                        raise ValueError('قاعدة البيانات داخل النسخة تالفة (integrity_check).')
                    _probe.execute('SELECT COUNT(*) FROM users').fetchone()
                finally:
                    _probe.close()
                shutil.copy2(dbsrc,DB)
                def _restore_tree(arc_name, target):
                    """Replace one asset tree from the archive.

                    Older backups contain only employee_files/; qr/ and
                    branding/ are absent. In that case the existing directory
                    is left untouched rather than wiped, so restoring a legacy
                    archive cannot destroy QR images it never carried.
                    """
                    srcdir=os.path.join(tmp,arc_name)
                    if not os.path.isdir(srcdir):
                        print(f'[HR] restore: archive has no {arc_name}/ — leaving existing files in place')
                        return 0
                    shutil.rmtree(target,ignore_errors=True); os.makedirs(target,exist_ok=True)
                    n=0
                    for root,dirs,files in os.walk(srcdir):
                        rel=os.path.relpath(root,srcdir); dest=os.path.join(target,'' if rel=='.' else rel)
                        os.makedirs(dest,exist_ok=True)
                        for fn in files:
                            shutil.copy2(os.path.join(root,fn),os.path.join(dest,fn)); n+=1
                    return n
                restored_counts={
                    'employee_files': _restore_tree('employee_files',EMPFILES),
                    'qr':             _restore_tree('qr',os.path.join(DATA,'qr')),
                    'branding':       _restore_tree('branding',BRAND),
                }
                print('[HR] restore: '+', '.join(f'{k}={v}' for k,v in restored_counts.items()))
...
# RETENTION
#
# audit, error_logs, security_login_events, system_sessions, bridge_tokens and
# device telemetry all grew without bound. On a hospital-sized deployment these
# reach millions of rows, at which point every page that touches them — and the
# nightly backup that copies them — gets slower until the UI appears to hang.
#
# Audit is deliberately the longest retained (compliance) and is never deleted
# below one year.
# --------------------------------------------------------------------------
RETENTION_DAYS = {
    'audit':                 int(os.environ.get('HR_RETAIN_AUDIT',    '730')),
    'error_logs':            int(os.environ.get('HR_RETAIN_ERRORS',   '90')),
    'security_login_events': int(os.environ.get('HR_RETAIN_LOGINS',   '365')),
    'system_sessions':       int(os.environ.get('HR_RETAIN_SESSIONS', '90')),
    'device_events':         int(os.environ.get('HR_RETAIN_DEVICE',   '90')),
    'notifications':         int(os.environ.get('HR_RETAIN_NOTIFS',   '180')),
}
# Column names differ per table across schema generations (audit/error_logs use
# `ts`, sessions/notifications use `created_at`), so the timestamp column is
# discovered at runtime instead of being hardcoded per table.
_RETENTION_TIME_CANDIDATES = ('ts','created_at','last_seen','created','event_time')

def _retention_time_column(conn, table):
    try:
        cols = {r[1] for r in conn.execute(f'PRAGMA table_info({table})')}
    except Exception:
        return None
    for candidate in _RETENTION_TIME_CANDIDATES:
        if candidate in cols:
            return candidate
    return None

def apply_retention():
    """Delete rows past their retention horizon. Safe to call repeatedly."""
    removed={}
    for table,days in RETENTION_DAYS.items():
        if days<=0: continue
        if table=='audit': days=max(days,365)   # never trim audit below one year
        cutoff=(date.today()-timedelta(days=days)).isoformat()
        try:
            c=db()
            if not c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name=?",(table,)).fetchone():
                c.close(); continue
            col=_retention_time_column(c,table)
            if not col:
                c.close(); continue
            cur=c.execute(f'DELETE FROM {table} WHERE {col} IS NOT NULL AND {col}<?',(cutoff,))
            removed[table]=cur.rowcount if cur.rowcount and cur.rowcount>0 else 0
            c.commit(); c.close()
        except sqlite3.OperationalError as e:
            log_error('retention:'+table,e,username='system',method='SCHEDULE',path='/system')
        except Exception as e:
            log_error('retention:'+table,e,username='system',method='SCHEDULE',path='/system')
    # One-time / short-lived tokens: used or expired rows have no further value.
    try:
        c=db()
        if c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='bridge_tokens'").fetchone():
            c.execute("DELETE FROM bridge_tokens WHERE used=1 OR (expires_at IS NOT NULL AND expires_at<?)",(now(),))
        if c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='auth_failures'").fetchone():
            # Drop stale counters whose lock has long expired.
            c.execute("DELETE FROM auth_failures WHERE (locked_until IS NULL OR locked_until<?) AND last_fail IS NOT NULL AND last_fail<?",
                      (now(),(date.today()-timedelta(days=30)).isoformat()))
        c.commit(); c.close()
    except Exception as e:
        log_error('retention:tokens',e,username='system',method='SCHEDULE',path='/system')
    return removed

def cleanup_orphan_files():
    """Remove leftover temp artefacts from interrupted QR/export/backup runs.

    QR generation writes .tmp-* files and atomically renames them. If the
    process died between the two steps the temp file stayed forever; likewise
    for job_results archives whose job record has already been evicted.
    """
    cleaned=0
    now_ts=time.time()
    for folder,age_secs in ((os.path.join(DATA,'qr'),3600),
                            (os.path.join(DATA,'job_results'),24*3600),
                            (DATA,3600)):
        if not os.path.isdir(folder): continue
        try:
            for fn in os.listdir(folder):
                if not (fn.startswith('.tmp-') or fn.startswith('_restore') or fn.endswith('.tmp')
                        or fn.endswith('.staging') or fn.endswith('.verify.tmp')):
                    continue
                fp=os.path.join(folder,fn)
                try:
                    if os.path.isfile(fp) and (now_ts-os.path.getmtime(fp))>age_secs:
                        os.remove(fp); cleaned+=1
                except OSError:
                    pass
        except OSError as e:
            log_error('cleanup-orphans',e,username='system',method='SCHEDULE',path='/system')
    return cleaned

def prune_memory_caches():
    """Bound the in-memory dictionaries that previously grew without limit."""
    try:
        # Expired one-time bridge tokens.
        for k in [k for k,v in list(BRIDGE_TOKENS.items())
                  if isinstance(v,dict) and v.get('expires') and time.time()>float(v['expires'])]:
            BRIDGE_TOKENS.pop(k,None)
        if len(BRIDGE_TOKENS)>5000:
            for k in list(BRIDGE_TOKENS)[:len(BRIDGE_TOKENS)-5000]: BRIDGE_TOKENS.pop(k,None)
        # Sessions whose last_seen is far past the idle window.
        limit=int(os.environ.get('HR_SESSION_IDLE_DAYS','7'))
        cutoff=(datetime.now()-timedelta(days=limit)).isoformat(timespec='seconds')
        for sid,sess in list(SESS.items()):
            if isinstance(sess,dict) and (sess.get('last_seen') or '') < cutoff:
                SESS.pop(sid,None)
        LOGIN_ATTEMPTS.clear()   # superseded by the persistent auth_failures table
    except Exception as e:
        log_error('prune-memory-caches',e,username='system',method='SCHEDULE',path='/system')
    ...
    while True:
        try:
            ...
            if last_prune_day!=today:
                try:
                    c=db(); cutoff=(date.today()-timedelta(days=180)).isoformat()
                    c.execute("DELETE FROM notifications WHERE read_at IS NOT NULL AND created_at<?",(cutoff,)); c.commit(); c.close()
                    apply_retention()
                    cleanup_orphan_files()
                    last_prune_day=today
                except Exception as e: log_error('prune_notifications',e,username='system',method='SCHEDULE',path='/system')
            enabled=setting('backup_auto_enabled')=='1'; target=setting('backup_time') or '23:00'
...
def acquire_instance_lock():
    """Prevent two local HR servers from opening the same SQLite database."""
    global _INSTANCE_LOCK_HANDLE
    path=os.path.join(DATA,'server.instance.lock')
    fh=open(path,'a+b')
    try:
...
def release_instance_lock():
    global _INSTANCE_LOCK_HANDLE
    if not _INSTANCE_LOCK_HANDLE: return
    try:
        if sys.platform.startswith('win'):
            import msvcrt
            _INSTANCE_LOCK_HANDLE.seek(0); msvcrt.locking(_INSTANCE_LOCK_HANDLE.fileno(), msvcrt.LK_UNLCK, 1)
        else:
            import fcntl
            fcntl.flock(_INSTANCE_LOCK_HANDLE.fileno(), fcntl.LOCK_UN)
    except Exception: pass
    try: _INSTANCE_LOCK_HANDLE.close()
    except Exception: pass
    _INSTANCE_LOCK_HANDLE=None
...
def main():
    ...
    migrate_legacy_data_location(); server_identity()
    global PORT
    # IMPORTANT: discover the existing LAN server BEFORE touching SQLite.
    # This prevents a second desktop client from opening the shared DB and producing
    # the recurring Windows "database is locked" startup crash.
    ...
    if not acquire_instance_lock():
        # Another local server owns this data directory. Do not open the DB again.
        show_native_error('HR Enterprise — البرنامج يعمل بالفعل','يوجد تشغيل آخر لنظام HR Enterprise يستخدم نفس قاعدة البيانات.\nأغلق النسخة الأخرى أو استخدمها بدل تشغيل نسخة ثانية.')
        return
    try:
        init(); migrate_documents_to_fs(); auto_alerts()
    except Exception:
        release_instance_lock(); raise
    ...
    try: cleanup_orphan_files()
    except Exception as e: log_error('startup-orphan-cleanup',e)
    threading.Thread(target=discovery_responder,daemon=True).start()
    threading.Thread(target=maintenance_loop,daemon=True).start()
    url_scheme = 'https' if _tls_policy(HOST)[0] else 'http'
    url=f'{url_scheme}://{HOST if HOST not in ("0.0.0.0","::") else local_ip()}:{PORT}'
    hr_log('startup','server_started',level='INFO',version=APP_VERSION,
           url=url,mode=('auto' if AUTO_MODE else ('network' if NETWORK_MODE else 'standalone')),
           data_dir=DATA,tls=bool(os.environ.get('HR_TLS_CERT','').strip()),
           backups_encrypted=backups_encrypted(),max_workers=MAX_HTTP_WORKERS,
           log_level=LOG_LEVEL)
...
def _self_signed_cert_paths():
    """Return (cert_path, key_path) for an auto-generated self-signed TLS
    certificate, generating one on first run if it does not already exist.

    This exists so the Network Server never has to choose between "refuse to
    start" and "serve HR payroll/national-ID data in plaintext" just because
    nobody has configured a real certificate yet. The certificate is
    2048-bit RSA, valid ~2 years, covers 127.0.0.1/localhost and the
    machine's LAN IP, and is regenerated automatically only if missing.
    Browsers will show a one-time "not trusted" warning per device for this
    self-signed cert; that is expected for an internal appliance and is a
    strictly better default than unencrypted HTTP. Operators who have a real
    certificate should set HR_TLS_CERT / HR_TLS_KEY, which always takes
    priority over this auto-generated one.
    """
    cert_path = os.path.join(DATA, 'auto_tls_cert.pem')
    key_path = os.path.join(DATA, 'auto_tls_key.pem')
    ...
    try:
        ...
        name = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, u'hr-enterprise.local')])
        san = [x509.DNSName(u'localhost'), x509.IPAddress(ipaddress.IPv4Address('127.0.0.1'))]
        try:
        ...
        os.makedirs(DATA, exist_ok=True)
        tmp_key = key_path + '.tmp'; tmp_cert = cert_path + '.tmp'
        ...
        os.replace(tmp_key, key_path); os.replace(tmp_cert, cert_path)
        # os.chmod(0o600) only toggles the read-only bit on NTFS; restrict the
        # private key with a real ACL instead.
        protect_secret_file(key_path)
        print('='*72)
        print('[HR] No HR_TLS_CERT/HR_TLS_KEY configured — generated a self-signed')
        print('[HR] certificate for this LAN-exposed server (data/auto_tls_cert.pem).')
        print('[HR] Browsers will show a one-time "not trusted" warning per device;')
        print('[HR] this is expected for an internal appliance. Traffic is encrypted.')
        print('[HR] To use a real certificate instead, set HR_TLS_CERT / HR_TLS_KEY.')
        print('='*72)
        return cert_path, key_path
    except Exception as e:
        log_error('self-signed-cert-generation', e)
        return None, None

def _tls_policy(host):
    """Decide whether this process will serve TLS and whether the cert is
    operator-supplied ('manual') or auto-generated. Pure/no side effects, so
    it can be called before the startup banner to pick the right URL scheme
    without generating anything twice."""
    cert = os.environ.get('HR_TLS_CERT', '').strip()
    key = os.environ.get('HR_TLS_KEY', '').strip()
    if cert or key:
        return True, True
    exposed = host not in ('127.0.0.1', '::1', 'localhost')
    if not exposed:
        return False, False
    if os.environ.get('HR_ALLOW_PLAINTEXT_LAN', '').strip() == '1':
        return False, False
    if os.environ.get('HR_NO_AUTO_TLS', '').strip() == '1':
        return False, False
    return True, False

def _tls_context():
    """Build an SSLContext for an operator-supplied certificate only.

    HR_TLS_CERT / HR_TLS_KEY point at a PEM certificate and private key. When
    both are present the server speaks real HTTPS instead of merely emitting
    an HSTS header over plaintext, which is what https_enabled=1 used to do.
    The auto-generated self-signed fallback (see _self_signed_cert_paths) is
    handled separately in serve_http(), since it is not "manual" TLS.
    """
    cert=os.environ.get('HR_TLS_CERT','').strip(); key=os.environ.get('HR_TLS_KEY','').strip()
    if not cert or not key: return None
    if not (os.path.exists(cert) and os.path.exists(key)):
        print('[WARNING] HR_TLS_CERT/HR_TLS_KEY set but the files do not exist; refusing to start in plaintext.')
        raise SystemExit(2)
...
def serve_http(host,port):
    """Start the HTTP(S) listener, refusing unsafe network exposure.

    Resolution order for a LAN-exposed (non-loopback) host:
      1. HR_TLS_CERT/HR_TLS_KEY (operator's real certificate) — always wins.
      2. Auto-generated self-signed certificate (see _self_signed_cert_paths)
         — the default, so the server is never forced to choose between
         refusing to start and serving payroll/national-ID data in plaintext.
      3. HR_NO_AUTO_TLS=1 opts out of #2 and restores the strict old
         behaviour: refuse to start unless HR_ALLOW_PLAINTEXT_LAN=1.
    Loopback hosts (127.0.0.1/::1/localhost) are unaffected and keep serving
    plain HTTP, matching the single-user desktop EXE's existing behaviour.
    """
    httpd=ThreadingHTTPServer((host,port),H)
    httpd.daemon_threads=True
    will_tls, manual = _tls_policy(host)
    ctx=None
    if will_tls:
        if manual:
            ctx=_tls_context()
        else:
            c,k=_self_signed_cert_paths()
            if c and k:
                ctx=ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
                ctx.minimum_version=ssl.TLSVersion.TLSv1_2
    ...
    if ctx is not None:
        ...
        print(f'[HR] TLS enabled ({"operator certificate" if manual else "auto self-signed"}) — serving HTTPS on {host}:{port}')
    else:
        exposed = host not in ('127.0.0.1','::1','localhost')
        if exposed and os.environ.get('HR_ALLOW_PLAINTEXT_LAN','').strip()!='1':
            # Refuse to serve HR payroll/national-ID data over plaintext on a
            # LAN interface unless the operator has explicitly accepted it
            # (e.g. because an HTTPS reverse proxy terminates in front, or
            # HR_NO_AUTO_TLS=1 was set and no self-signed cert was allowed).
            print('='*72)
            print('[HR] REFUSING TO START: bound to',host,'without TLS.')
            print('[HR] Either set HR_TLS_CERT + HR_TLS_KEY, or put an HTTPS reverse')
            print('[HR] proxy in front and bind to 127.0.0.1, or explicitly accept the')
            print('[HR] risk with HR_ALLOW_PLAINTEXT_LAN=1 on an isolated network.')
            print('='*72)
            raise SystemExit(2)
        if exposed:
            print('[HR] WARNING: serving plaintext HTTP on',host,'- HR_ALLOW_PLAINTEXT_LAN=1 was set.')
    httpd.serve_forever()
...
def _device_ping_v9(self,u):
    ...
    try:
        ...
        hr_log('db','silent_db_failure',level='WARNING',exc=_e,where='server.py:4894')
...
def _v9_alerts_snapshot(u):
    ...
    try:
        ...
        hr_log('db','silent_db_failure',level='WARNING',exc=_e,where='server.py:4905')
    return base

def extra_nav_links(user):
    """The v9 feature links, as a reusable fragment.

    These used to be injected by a third page() wrapper that replaced the
    global name. Whether a given route saw them depended on whether its module
    captured `page` before or after that wrapper installed itself, which is why
    /id-cards showed four sidebar entries that /users did not. Both shells now
    call this one function, so every page gets the same navigation.
    """
    if not (user and can(user,'employees.view')):
        return ''
...
def nav_append(out,fragment):
    """Append sidebar links, skipping any whose href the nav already contains.

    Six modules each append their own links to </nav>. Once the final layer
    supplies a curated navigation that already lists some of them, a blind
    append produces duplicates — "أجهزة البصمة" and "Evaluation Intelligence"
    appeared twice. Deduplicating by href keeps every module's links reachable
    without repeating an entry.
    """
    if not fragment or '</nav>' not in out:
        return out
    nav=out.split('</nav>',1)[0]
    keep=[]
    parts=re.findall(r'(<details\b.*?</details>|<a\b[^>]*?href="([^"]*)"[^>]*>.*?</a>)',fragment,re.S)
    if not parts:
        return out.replace('</nav>',fragment+'</nav>',1)
    for chunk,href in parts:
        if chunk.startswith('<details'):
            # Keep only the links in the group that are not already listed,
            # and drop the group entirely if that leaves it empty.
            inner=re.findall(r'<a\b[^>]*?href="([^"]*)"[^>]*>.*?</a>',chunk,re.S)
            fresh=[h for h in inner if f'href="{h}"' not in nav]
            if not fresh:
                continue
            for h in inner:
                if h not in fresh:
                    chunk=re.sub(r'<a\b[^>]*?href="'+re.escape(h)+r'"[^>]*>.*?</a>','',chunk,count=1,flags=re.S)
            keep.append(chunk)
            continue
        if href and f'href="{href}"' in nav:
            continue
        keep.append(chunk)
    return out.replace('</nav>',''.join(keep)+'</nav>',1) if keep else out

def inject_extra_nav(out,user):
    return nav_append(out,extra_nav_links(user))

def _v9_page(title,body,user,active='dashboard'):
    return inject_extra_nav(_V9_ORIG_PAGE(title,body,user,active),user)

def _v9_routes():
    global _V9_ORIG_PAGE,_V9_ORIG_ALERTS
    _V9_ORIG_PAGE=_default_page; _V9_ORIG_ALERTS=hr_alerts_snapshot
    ...
    def get(self):
        ...
        if p=='/import/excel-grid':
            u=self.require(); return self.excel_grid(u) if u else None
...
import enterprise_completion as _enterprise_completion
_enterprise_completion.install_enterprise(globals())
# V12 enterprise UX/data completion
import v12_enterprise as _v12
_v12.install(globals())

import production_ops as _production_ops
_production_ops.install_production_ops(globals())

# ZKTeco integration — Phase 1 (schema-only, no connector/UI yet)
import zkteco_core as _zkteco_core
_zkteco_core.install_zkteco(globals())

# ZKTeco integration — Phase 2 (connector + sync engine; dedup index finalized,
# no scheduler/UI yet — sync_device() must be called explicitly, e.g. from a
# future admin action or CLI script)
import zkteco_sync as _zkteco_sync
_zkteco_sync.install_zkteco_sync(globals())

# ZKTeco integration — Phase 4 (in-app UI: devices, sync, unmatched mapping,
# attendance records). Wraps H.do_GET/H.do_POST and page() the same way
# v10/v11/v12/enterprise_completion/production_ops already do; adds no new
# tables/migrations (reuses Phase 1's zk_* tables and the existing
# employees.zk_user_id unique index).
import zkteco_ui as _zkteco_ui
_zkteco_ui.install_zkteco_ui(globals())

# ZKTeco Hospital Operations Layer — background sync, attendance processing,
# device monitoring, provisioning, reconciliation, and unified center.
import zkteco_hospital as _zkteco_hospital
_zkteco_hospital.install_zkteco_hospital(globals())

# V13 security scope + HR UX completion
import v13_security_ux as _v13
_v13.install(globals())

# V14 final production hardening: GET boundaries, profile 360, onboarding, permission matrix.
import v14_final_hardening as _v14
_v14.install(globals())

# Final stability layer: install BEFORE main() so the real server starts with the
# final routing/navigation guards already active.
import stable_final as _stable_final
_stable_final.install_stable_final(globals())

# V18 production layer: configurable HR policies, permission requests, universal
# Excel/clipboard center, deterministic admin password recovery.
import v18_production as _v18
_v18.install_v18(globals())

# =====================================================================
# TEMPORARY AUTHENTICATION BYPASS
# ---------------------------------------------------------------------
# The app opens directly as the system administrator, with no login
# screen, so users/roles/permissions can be set up from inside.
# To RE-ENABLE login later: set AUTH_DISABLED_DEFAULT = False below and
# rebuild (or run with the environment variable HR_DISABLE_AUTH=0).
# WARNING: while this is on, ANYONE who can reach the server over the
# network gets full admin access.
# =====================================================================
AUTH_DISABLED_DEFAULT = True
AUTH_DISABLED = os.environ.get('HR_DISABLE_AUTH', '1' if AUTH_DISABLED_DEFAULT else '0').strip() != '0'
_NO_AUTH_CSRF = secrets.token_urlsafe(24)
_NO_AUTH_CACHE = {'u': None, 't': 0.0}

def _no_auth_user():
    now_t = time.time()
    cached = _NO_AUTH_CACHE['u']
    if cached is not None and now_t - _NO_AUTH_CACHE['t'] < 30:
        return dict(cached)
    base = {}
    try:
        c = db()
        ...
        if not r:
            r = c.execute("SELECT * FROM users WHERE role IN ('SuperAdmin','Admin') ORDER BY id LIMIT 1").fetchone()
        c.close()
        if r: base = dict(r)
    except Exception as e:
        log_error('no-auth-user', e)
    base.setdefault('username', 'admin')
    base.setdefault('full_name', 'مدير النظام')
    base.setdefault('permission_version', 1)
    if base.get('role') not in ('SuperAdmin', 'Admin'):
        base['role'] = 'SuperAdmin'
    base.update(active=1, must_change_password=0, scope_type='all', scope_value='',
                csrf=_NO_AUTH_CSRF, _sid='no-auth-session', last_seen=now(),
                _checked_at=now_t, _last_write=now_t, _no_auth=True)
    base.pop('password_hash', None)
    _NO_AUTH_CACHE['u'] = base; _NO_AUTH_CACHE['t'] = now_t
    return dict(base)

def _install_no_auth():
    ...
    H.do_GET = _get
    H.do_POST = _post
    print('[HR] WARNING: authentication is DISABLED (temporary). Everyone opens as admin.')
    try: hr_log('auth', 'authentication_disabled', level='WARNING')
    except Exception: pass
