...
'.brand-info b{display:block;overflow:hidden;text-overflow:ellipsis}'
...
def _role_perms(role):
    perms=_ROLE_PERMS_CACHE.get(role)
    if perms is None:
...
def emp_allowed(u,code):
    ...
    if u.get('role')=='Employee' or u.get('scope_type')=='self':return code==(u.get('scope_value') or u.get('username'))
...
class H(BaseHTTPRequestHandler):
    ...
    def send(self,body,status=200,ctype='text/html; charset=utf-8',headers=None):
        ...
        self.send_header('Content-Security-Policy',"default-src 'self'; img-src 'self' data: blob:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; object-src 'none'; base-uri 'self'; frame-ancestors 'self'; form-action 'self';")
...
<div class="full"><button class="btn">حفظ سياسة الدوام</button></div></form></div>'''
    ...
    def roles_save(self,u,f):
        ...
        c.commit(); c.close(); _ROLE_PERMS_CACHE.pop(role,None); audit(u['username'],u['role'],'تعديل الصلاحيات','الأدوار',role,','.join(perms)); self.redirect('/roles')
