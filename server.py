...
APP_VERSION='11.2.6 HR Production Hardened'
...
def _bootstrap_admin_password():
    ...
    Existing installations are never reset because the INSERT below is
    conditional. New installations may provide HR_BOOTSTRAP_PASSWORD; if it is
    absent, a cryptographically random password is generated and written to a
    local first-run file that must be deleted after the password is changed.
    """
    ...
    print('[HR] Initial admin password generated. Change it immediately and delete:',path)
    return value
...
<script>async function syncDevice(){{const d=prompt('Computer Name / Device Name',navigator.platform+'-'+location.hostname);try{{const r=await fetch('/device/ping?x='+Date.now(),{{headers:{{'X-HR-Device-Name':d||navigator.platform}}}});const j=await r.json();document.getElementById('deviceStatus').textContent='🟢 Connected · '+j.server+' · '+j.ip+':'+j.port+' · '+j.last_sync;setTimeout(()=>location.reload(),500);}}catch(e){{document.getElementById('deviceStatus').textContent='🔴 SERVER OFFLINE';}}}}</script>'''.format(server=esc(setting('server_name') or 'HR-MAIN'),ip=esc(local_ip()),port=PORT,rows=trs or '<tr><td colspan="6">لا توجد أجهزة مسجلة.</td></tr>')
    ...
    def do_POST(self):
        ...
        if path=='/login':
            ...
            cookie_secure='; Secure' if (os.environ.get('HR_COOKIE_SECURE','').strip()=='1' or os.environ.get('HR_HTTPS','').strip()=='1' or setting('https_enabled')=='1') else ''
