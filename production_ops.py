...
def install_production_ops(g):
    ...
    def issue_qr(emp,user,regenerate=False):
        ...
        c=db()
        qr_cols={r['name'] for r in c.execute('PRAGMA table_info(qr_identities)').fetchall()}
        issued=now()
        if old:
            if 'token' in qr_cols:
                # Legacy NOT NULL `token` column: store a non-secret marker.
                # This is the THIRD writer of this table (the others are in
                # enterprise_completion._qr_write and stable_final's bulk path);
                # each had to be fixed separately.
                c.execute('UPDATE qr_identities SET token=?,token_hash=?,issued_at=?,revoked_at=NULL,status="active",created_by=?,regenerated_from=?,image_path=? WHERE emp_code=?',('redacted-'+th[:16],th,issued,user['username'],old['token_hash'],rel,emp))
            else:
                c.execute('UPDATE qr_identities SET token_hash=?,issued_at=?,revoked_at=NULL,status="active",created_by=?,regenerated_from=?,image_path=? WHERE emp_code=?',(th,issued,user['username'],old['token_hash'],rel,emp))
        else:
            if 'token' in qr_cols:
                c.execute('INSERT INTO qr_identities(emp_code,token,token_hash,issued_at,status,created_by,regenerated_from,image_path) VALUES(?,?,?,?,?,?,?,?)',(emp,'redacted-'+th[:16],th,issued,'active',user['username'],None,rel))
            else:
                c.execute('INSERT INTO qr_identities(emp_code,token_hash,issued_at,status,created_by,regenerated_from,image_path) VALUES(?,?,?,?,?,?,?)',(emp,th,issued,'active',user['username'],None,rel))
        c.commit(); c.close(); g['audit'](user['username'],user['role'],'QR_CREATED' if not old else 'QR_REGENERATED','QR Identity',emp,'active')
        return True

    # ----------------------------------------------------------------------
    # Export hardening helpers.
    # ----------------------------------------------------------------------
    ...
    def csv_safe(value):
        ...
        An employee whose name, notes or address begins with = + - or @ used to
        be written verbatim into employee_accounts.csv / the export workbook.
        Excel then evaluates it as a formula (=HYPERLINK, =cmd|..., DDE), which
        turns "export the staff list" into code execution on the HR PC.
        """
    ...
    def _checkpoint(done=None, message=None):
        """Bridge to the job engine's cooperative cancellation (installed later)."""
        fn = g.get('job_checkpoint')
        if fn:
            fn(done, message)

    def _export_temp_path(prefix):
        d = os.path.join(DATA, 'job_results')
        os.makedirs(d, exist_ok=True)
        return os.path.join(d, f'{prefix}_{secrets.token_hex(8)}.zip')
    ...
    def export_package(user, include_credentials=True):
        ...
        c=db(); emps=c.execute("SELECT * FROM employees WHERE status<>'مؤرشف' ORDER BY name").fetchall()
        # STREAMING: the archive is built directly on disk instead of io.BytesIO().
        # A full export (documents + photos + QR for every employee) used to be
        # assembled entirely in RAM before a single byte reached the client.
        out_path=_export_temp_path('export'); credentials=[]
        _checkpoint(0,'جاري تجهيز التصدير')
        with zipfile.ZipFile(out_path,'w',zipfile.ZIP_DEFLATED) as z:
            ...
            for _idx,e in enumerate(emps):
                # Cancellation/deadline is checked once per employee, so a
                # cancelled or timed-out export actually stops here instead of
                # running to completion behind a UI that already said "timeout".
                _checkpoint(_idx,'تصدير الموظفين')
                emp=e['emp_code']; uname=''; role=''; active=''; qrstatus=''; issued=''
                ur=c.execute('SELECT username,role,active FROM users WHERE employee_code=? ORDER BY id DESC LIMIT 1',(emp,)).fetchone(); qr=c.execute('SELECT status,issued_at,image_path FROM qr_identities WHERE emp_code=? AND status="active" ORDER BY id DESC LIMIT 1',(emp,)).fetchone(); docs=c.execute('SELECT id,file_name,category,storage_path,data FROM documents WHERE emp_code=? AND status="current" ORDER BY id',(emp,)).fetchall()
    ...
    def bulk_provision(user):
        ...
        results=[]; errors=[]
        for _bidx,r in enumerate(rows):
            _checkpoint(_bidx,'تجهيز حسابات الموظفين')
            emp=r['emp_code']
        ...
        out_path=_export_temp_path('provision')
        with zipfile.ZipFile(out_path,'w',zipfile.ZIP_DEFLATED) as z:
            if Workbook:
                wb=Workbook(); ws=wb.active; ws.title='Employees'
                ws.append(['Employee ID','Name','Username','Temporary Password','Role','QR Status'])
                for row in results: ws.append([csv_safe(v) for v in row])
                x=io.BytesIO(); wb.save(x); z.writestr('employee_accounts.xlsx',x.getvalue())
            s=io.StringIO(); w=csv.writer(s); w.writerow(['Employee ID','Name','Username','Temporary Password','Role','QR Status'])
            w.writerows([[csv_safe(v) for v in row] for row in results])
            z.writestr('employee_accounts.csv',s.getvalue().encode('utf-8-sig'))
            z.writestr('READ_ME_CREDENTIALS.txt',
                ('This archive contains one-time temporary passwords in cleartext.\n'
                 'Distribute them over a secure channel, require an immediate password\n'
                 'change, then DELETE this archive. Do not store it on a shared drive\n'
                 'and do not keep it in the backups folder.\n').encode('utf-8'))
            for r in results:
...
<div class="grid g3" style="margin-top:16px"><div class="card"><h3>🔐 Bulk Users</h3><p>إنشاء/تجديد حسابات الموظفين دفعة واحدة.</p><form method="post" action="/employee/operations/users">{csrf(u)}<button class="btn gray">إنشاء Users فقط</button></form></div><div class="card"><h3>🔳 Bulk QR</h3><p>إنشاء QR لكل موظف بدون QR.</p><form method="post" action="/employee/operations/qr">{csrf(u)}<button class="btn gray">إنشاء QR فقط</button></form></div><div class="card"><h3>📦 Full Export</h3><p>بيانات + مستندات + صور + QR + Excel.</p><form method="post" action="/employee/operations/export" style="display:inline">{csrf(u)}<button class="btn gray">Export Full Package</button></form></div><div class="card"><h3>📁 Employee Folders</h3><p>إنشاء مجلد مستقل لكل موظف لرفع وحفظ المستندات.</p><form method="post" action="/employee/operations/folders" style="display:inline">{csrf(u)}<button class="btn gray">إنشاء كل المجلدات</button></form></div></div>'''
    ...
    def get(self):
        ...
        return old_get(self)
    def post(self):
        # Keep the heavy export callable discoverable for the centralized job layer.
        _legacy_export_callable = export_package
        p=urlparse(self.path).path
        ...
        if p in ('/employee/operations/provision','/employee/operations/users','/employee/operations/qr'):
            ...
            if p=='/employee/operations/provision':
                try:
                    path,n,errors=bulk_provision(u)
                    # The archive now lives on disk; stream it out and delete it
                    # rather than holding the whole thing in memory.
                    try:
                        with open(path,'rb') as _fh: data=_fh.read()
                    finally:
                        try: os.remove(path)
                        except OSError: pass
                    return self.send(data,200,'application/zip',{'Content-Disposition':'attachment; filename="HR_Bulk_Employee_Provisioning.zip"'})
            ...
            if p=='/employee/operations/users':
                ...
                out=io.StringIO(); w=csv.writer(out); w.writerow(['Employee ID','Username','Temporary Password','Status']); w.writerows([[csv_safe(v) for v in row] for row in result]); return self.send(out.getvalue().encode('utf-8-sig'),200,'text/csv; charset=utf-8',{'Content-Disposition':'attachment; filename="Employee_Accounts.csv"'})
            if p=='/employee/operations/qr':
                ...
                out=io.StringIO(); w=csv.writer(out); w.writerow(['Employee ID','Status','Error']); [w.writerow([csv_safe(r['emp_code']),'created','']) for r in rows if r['emp_code'] not in dict(errors)]; [w.writerow([csv_safe(a),'failed',csv_safe(b)]) for a,b in errors]; return self.send(out.getvalue().encode('utf-8-sig'),200,'text/csv; charset=utf-8',{'Content-Disposition':'attachment; filename="QR_Bulk_Result.csv"'})
    ...
    def page_ops(title,body,user,active='dashboard'):
        ...
        if user and can(user,'employees.view'):
            ...
            if marker not in out:
                ...
                out=g['nav_append'](out,link) if '</nav>' in out else out.replace('</aside>',link+'</aside>',1)
        return out
    g['page']=page_ops
