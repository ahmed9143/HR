...
def install_production_ops(g):
    ...
    def issue_qr(emp,user,regenerate=False):
        ...
        if old:
            if 'token' in qr_cols:
                c.execute('UPDATE qr_identities SET token=?,token_hash=?,issued_at=?,revoked_at=NULL,status="active",created_by=?,regenerated_from=?,image_path=? WHERE emp_code=?',(token,th,issued,user['username'],old['token_hash'],rel,emp))
            else:
            ...
            if 'token' in qr_cols:
                c.execute('INSERT INTO qr_identities(emp_code,token,token_hash,issued_at,status,created_by,regenerated_from,image_path) VALUES(?,?,?,?,?,?,?,?)',(emp,token,th,issued,'active',user['username'],None,rel))
            else:
