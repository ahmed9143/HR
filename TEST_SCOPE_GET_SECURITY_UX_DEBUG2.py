...
with tempfile.TemporaryDirectory(prefix='hr_scope_get_') as td:
    ...
    try:
        ...
        for path in ['/contracts','/training']:
            ...
            if 'Blocked' in b or 'EB' in b: failures.append(f'{path}: leaked DeptB employee; snippet='+b[b.find('EB')-100:b.find('EB')+150] if 'EB' in b else f'{path}: leaked Blocked')
        st,b=req(om,base,'/id-cards')
        if st!=200: failures.append(f'/id-cards: expected 200 got {st}')
        if 'EB' in b or 'Blocked' in b: failures.append('/id-cards leaked DeptB employee; snippet='+b[b.find('EB')-100:b.find('EB')+150] if 'EB' in b else '/id-cards leaked Blocked')
        ...
        st,b=req(oa,base,'/qr/generate','POST',{'_csrf':csrf_a,'emp_code':'EB'})
        if st not in (302,303): failures.append(f'admin qr generate: expected redirect got {st}; body='+b[:500])
