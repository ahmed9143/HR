...
def install_zkteco(g):
    ...
    try:
        ...
        CREATE TABLE IF NOT EXISTS zk_jobs(
            id TEXT PRIMARY KEY, kind TEXT, device_id TEXT, state TEXT DEFAULT 'queued',
            total INTEGER DEFAULT 0, done INTEGER DEFAULT 0, success INTEGER DEFAULT 0,
            failed INTEGER DEFAULT 0, message TEXT, error TEXT,
            started_at TEXT, finished_at TEXT, updated_at TEXT
        );
        CREATE INDEX IF NOT EXISTS idx_zk_jobs_state ON zk_jobs(state);
        ...
        ''')
        # Self-healing migration: these two columns were added after zk_devices
        # first shipped, and CREATE TABLE IF NOT EXISTS above is a no-op against
        # an existing table, so an install upgrading from an older build needs
        # them added explicitly (same pattern as qr_identities in
        # enterprise_completion.py). Both have safe defaults so make_adapter()
        # keeps working unmodified against rows created before either existed.
        dev_cols={r['name'] for r in c.execute('PRAGMA table_info(zk_devices)').fetchall()}
        dev_required_cols={
            'connect_timeout_seconds':'INTEGER DEFAULT 5',
            'retry_count':'INTEGER DEFAULT 2',
        }
        for col,coltype in dev_required_cols.items():
            if col not in dev_cols:
                c.execute(f'ALTER TABLE zk_devices ADD COLUMN {col} {coltype}')
        c.commit()

        # SECURITY MIGRATION: zk_devices.comm_password was stored in cleartext.
        # Encrypt any surviving plaintext value in place. Devices keep working
        # because make_adapter() decrypts on use; only the at-rest form changes.
        try:
            encrypt_secret = g['encrypt_secret']; is_encrypted = g['is_encrypted_secret']
            stale = c.execute("SELECT id,comm_password FROM zk_devices "
                              "WHERE comm_password IS NOT NULL AND comm_password<>''").fetchall()
            migrated = 0
            for row in stale:
                if is_encrypted(row['comm_password']):
                    continue
                c.execute('UPDATE zk_devices SET comm_password=? WHERE id=?',
                          (encrypt_secret(row['comm_password']), row['id']))
                migrated += 1
            if migrated:
                c.commit()
                print(f'[HR] SECURITY MIGRATION: encrypted {migrated} ZKTeco device password(s) at rest.')
        except KeyError as e:
            # encrypt_secret is defined in server.py; a missing name means the
            # import order changed and must be fixed, not silently ignored.
            g['log_error']('zk-comm-password-migration-missing-helper', e)
        except Exception as e:
            g['log_error']('zk-comm-password-migration', e)
    finally:
        c.close()
