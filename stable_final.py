...
import hashlib
import os, re, json, csv, io, threading, time, traceback, secrets, zipfile, hashlib
...
def install_stable_final(g):
                ...
                try:
                    ...
                    for typ,emp,token,rel,final_path,was_active in prepared:
                        if typ=='revoke':
                            ...
                            if old:
                                if 'token' in qr_cols:
                                    conn.execute('UPDATE qr_identities SET token=?,token_hash=?,issued_at=?,revoked_at=NULL,status="active",created_by=?,regenerated_from=?,image_path=? WHERE emp_code=?',(token,th,issued,user['username'],old_token,rel,emp))
                                else:
                                ...
                                if 'token' in qr_cols:
                                    conn.execute('INSERT INTO qr_identities(emp_code,token,token_hash,issued_at,status,created_by,regenerated_from,image_path) VALUES(?,?,?,?,?,?,?,?)',(emp,token,th,issued,'active',user['username'],None,rel))
                                else:
                ...
                if committed:
                    for typ,emp,token,rel,final_path,was_active in prepared:
                        if typ=='upsert':
                            ...
                            try: os.replace(tmp_path,final_path)
                            except OSError: pass
                            finish_item(jid,True)
                        elif typ=='revoke':
                            finish_item(jid,True)
    ...
    def hospital_page(title,body,user,active='dashboard'):
        ...
        company=esc(g['setting']('company_name') or 'HR Hospital')
        groups=[]
        def add(label,items):
        ...
        search_box='''<div class="gsearch"><form action="/search" method="get"><input id="gsearch" name="q" placeholder="بحث سريع: اسم، كود، مستند…" autocomplete="off"><kbd>Ctrl K</kbd></form><script>document.addEventListener('keydown',function(e){if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();var el=document.getElementById('gsearch');if(el){el.focus();el.select();}}});</script></div>'''
