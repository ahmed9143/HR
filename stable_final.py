...
def install_stable_final(g):
    ...
    # the change screen itself, logout, the login page, and the assets those
    # pages need to render. Everything else is refused until the change.
    _PW_CHANGE_ALLOWED=('/password','/logout','/login','/static/','/branding/',
                        '/health','/favicon','/device/ping')

    def _forced_password_change(self):
        """Central enforcement of must_change_password for every GET route.

        The check used to live inside individual handlers, so a route owned by
        a layer that never added it — /id-cards in v13_security_ux — was fully
        reachable while the user was still on the first-run default password.
        Enforcing it once here, at the outermost layer, covers every route
        regardless of which module serves it.
        """
        p=urlparse(self.path).path
        if any(p==a or p.startswith(a) for a in _PW_CHANGE_ALLOWED):
            return False
        try:
            u=self.user()
        except Exception as e:
            g['hr_log']('auth','forced_change_check_failed',level='WARNING',exc=e)
            return False
    ...
    def get(self):
        p=urlparse(self.path).path
        if _forced_password_change(self):
            return None
